...
const uint32_t hw_uart_divisor[HW_UART_BAUDRATE_MAX]  = { 0x0006820B, 0x00034105,
                                                          0x0001a00b, 0x0000d005,
                                                          0x00008a0e, 0x00006803,
                                                          0x00004507, 0x00003401,
                                                          0x0000220c, 0x00001106,
                                                          0x0000080b, 0x0000070D,
                                                          0x00000405, 0x00000400,
                                                          0x00000203, 0x00000200,
                                                          0x00000100, 0x00000105};

//===================== Read/Write functions ===================================

uint8_t hw_uart_read(const HW_UART_ID uart)
{
        // Wait until received data are available
        while (hw_uart_read_buf_empty(uart));

        // Read element from the receive FIFO
        return hw_uart_rxdata_getf(uart);
}

void hw_uart_write(const HW_UART_ID uart, uint8_t data)
{
...
void hw_uart_write_buffer(const HW_UART_ID uart, const void *data, uint16_t len)
{
...
__STATIC_INLINE void hw_uart_enable_rx_int(const HW_UART_ID uart, bool enable)
{
...
__STATIC_INLINE void hw_uart_enable_tx_int(const HW_UART_ID uart, bool enable)
{
        GLOBAL_INT_DISABLE();
...
HW_UART_CONFIG_ERR hw_uart_send(const HW_UART_ID uart, const void *data, uint16_t len, hw_uart_tx_callback cb,
                                                                                void *user_data)
{
        ASSERT_WARNING(data != NULL);
 ...
 */
static bool hw_uart_drain_rx(const HW_UART_ID uart, UART_Data *ud, int len)
{
        /*
...
/*
 * Read number of bytes from UART synchronously
 * This function waits to receive  len bytes from UART.
 * This function does not use interrupts, nor DMA, it just waits for all data to arrive.
 */
static void hw_uart_read_buffer(HW_UART_ID uart, void *data, uint16_t len)
{
        ASSERT_WARNING(data != NULL);
...
void hw_uart_set_soft_fifo(const HW_UART_ID uart, uint8_t *buf, uint8_t size)
{
        UART_Data *ud = UARTDATA(uart);
 ...
 * This function does not use interrupts, nor DMA, it just waits for all data to arrive.
 */
static void hw_uart_read_buffer(const HW_UART_ID uart, void *data, uint16_t len)
{
        ASSERT_WARNING(data != NULL);
        uint8_t *p = data;
...
HW_UART_CONFIG_ERR hw_uart_receive(const HW_UART_ID uart, void *data, uint16_t len, hw_uart_rx_callback cb,
                                                                                void *user_data)
{
        ASSERT_WARNING(data != NULL);
        ...
        if (ud->rx_dma_buf_size > 0) {
                ASSERT_WARNING(len < ud->rx_dma_buf_size);

                uint16_t new_int, cur_idx;
                bool data_ready = false;

                ASSERT_WARNING(ud->rx_dma_active == false);
...
void hw_uart_receive_error_checking(const HW_UART_ID uart, void *data, uint16_t len, hw_uart_rx_callback cb,
...
static void hw_uart_irq_stop_receive(const HW_UART_ID uart)
{
        UART_Data *ud = &uart_data[UARTIX(uart)];
...
#if (dg_configUART_RX_CIRCULAR_DMA == 1)
uint16_t hw_uart_copy_dma_rx_to_user_buffer(const HW_UART_ID uart)
{
        UART_Data *ud = &uart_data[UARTIX(uart)];
...
uint16_t hw_uart_abort_receive(const HW_UART_ID uart)
{
        UART_Data *ud = &uart_data[UARTIX(uart)];
...
uint16_t hw_uart_abort_send(const HW_UART_ID uart)
{
        UART_Data *ud = &uart_data[UARTIX(uart)];
...
__STATIC_INLINE void hw_uart_rx_isr(const HW_UART_ID uart)
{
        UART_Data *ud = UARTDATA(uart);
...
__STATIC_INLINE void hw_uart_rx_timeout_isr(const HW_UART_ID uart)
{
        UART_Data *ud = UARTDATA(uart);
...
__STATIC_INLINE void hw_uart_error_isr(const HW_UART_ID uart) {
        UART_Data *ud = UARTDATA(uart);
        if (ud->err_cb) {
...
void UART_Interrupt_Handler(const HW_UART_ID uart)
{
        HW_UART_INT int_id;
...
{
        __IOM uint32_t *reg = sclk ? &CRG_COM->SET_CLK_COM_REG : &CRG_COM->RESET_CLK_COM_REG;
        if (uart == HW_UART1) {
                *reg = REG_MSK(CRG_COM, CLK_COM_REG, UART_CLK_SEL);
        } else if (uart == HW_UART2) {
                *reg = REG_MSK(CRG_COM, CLK_COM_REG, UART2_CLK_SEL);
        }
        else {
                ASSERT_WARNING(0); /*there are only 2 HW_UART for this device */
        }
}

HW_UART_BAUDRATE hw_uart_baudrate_get(const HW_UART_ID uart)
{
        ...
        ASSERT_WARNING(HW_UART_REG_GETF(uart, LCR, UART_DLAB) == 0);

        for (baud_rate = 0; baud_rate < HW_UART_BAUDRATE_MAX; baud_rate++ ) {
                if (divisor == hw_uart_divisor[baud_rate]) {
                    break;
                }
        }
        /* baud rates greater or equal to HW_UART_BAUDRATE_MAX are invalid */
        ASSERT_WARNING(baud_rate < HW_UART_BAUDRATE_MAX);

        sys_clk_is_t sys_clk = hw_clk_get_sysclk();
        if (sys_clk == SYS_CLK_IS_DBLR && baud_rate == HW_UART_BAUDRATE_3000000) {
                /* invalid settings for HW_UART_BAUDRATE_3000000 */
                ASSERT_WARNING(((uart == HW_UART1) && (REG_GETF(CRG_COM, CLK_COM_REG, UART_CLK_SEL))) ||
                        ((uart == HW_UART2) && (REG_GETF(CRG_COM, CLK_COM_REG, UART2_CLK_SEL))));
        }
        return baud_rate;
}

void hw_uart_baudrate_set(const HW_UART_ID uart, HW_UART_BAUDRATE baud_rate)
{
        bool sclk = false;
        ASSERT_WARNING((uart == HW_UART1) || (uart == HW_UART2));
        ASSERT_WARNING(baud_rate < HW_UART_BAUDRATE_MAX);
        bool is_doubler = (hw_clk_get_sysclk() == SYS_CLK_IS_DBLR) ? true : false ;
        sclk = false; /* Use DivN */
        if (baud_rate == HW_UART_BAUDRATE_3000000) {
                /* Requested baud rate is 3MBps. In this case, a high speed system clock is required. */
                ASSERT_WARNING(is_doubler);
                sclk = true;   /* Use Div1 */
        }
        ...
        UBA(uart)->UART2_DLF_REG = 0xFF & hw_uart_divisor[baud_rate];
        // Set low byte of baud rate
        UBA(uart)->UART2_RBR_THR_DLL_REG = 0xFF & (hw_uart_divisor[baud_rate] >> 8);
        // Set high byte of baud rare
        UBA(uart)->UART2_IER_DLH_REG = 0xFF & (hw_uart_divisor[baud_rate] >> 16);
...
uint8_t hw_uart_fifo_en_getf(const HW_UART_ID uart)
{
...
uint8_t hw_uart_tx_fifo_tr_lvl_getf(const HW_UART_ID uart)
{
...
void hw_uart_configure_dma_channels(const HW_UART_ID uart, const uart_config *uart_init)
{
        ...
        ASSERT_WARNING(tx_channel >= HW_DMA_CHANNEL_0 && tx_channel <= HW_DMA_CHANNEL_INVALID);

        /* Only specific DMA channels are allowed (or HW_DMA_CHANNEL_INVALID for no DMA) */
        ASSERT_WARNING(rx_channel >= HW_DMA_CHANNEL_0 && rx_channel <= HW_DMA_CHANNEL_INVALID);
        ...
        if (tx_channel == HW_DMA_CHANNEL_INVALID && rx_channel == HW_DMA_CHANNEL_INVALID) {
                ...
                if (tx_channel != HW_DMA_CHANNEL_INVALID && rx_channel != HW_DMA_CHANNEL_INVALID) {//not both invalid
                        ASSERT_WARNING(tx_channel != rx_channel);//not equal
                        ASSERT_WARNING(tx_channel>>1 == rx_channel>>1);//on same pair
                }
                if (tx_channel != HW_DMA_CHANNEL_INVALID) {
                        ASSERT_WARNING(tx_channel & 1);//odd number
                }
                if (rx_channel != HW_DMA_CHANNEL_INVALID) {
                        ASSERT_WARNING((rx_channel & 1) == 0);//even number
                }
...
void hw_uart_enable_rx_circular_dma(const HW_UART_ID uart)
{
        UART_Data *ud = UARTDATA(uart);

        ASSERT_WARNING(ud->rx_dma_buf_size > 0);
...
void hw_uart_copy_rx_circular_dma_buffer(const HW_UART_ID uart, uint8_t *buf, uint16_t len)
{
...
static HW_UART_CONFIG_ERR fifo_dma_burst_not_match(const uart_config *uart_init)
{
        ASSERT_WARNING(uart_init != NULL);
...
HW_UART_CONFIG_ERR hw_uart_init(HW_UART_ID uart, const uart_config *uart_init)
{
        ASSERT_WARNING(uart_init != NULL);
...
void hw_uart_reinit(const HW_UART_ID uart, const uart_config *uart_init)
{
        ASSERT_WARNING(uart_init != NULL);
        ...
        if (ud->rx_cb && ud->rx_len != ud->rx_ix) {
                ...
                if (ud->rx_len > 1 && uart_init->use_dma && uart_init->rx_dma_channel != HW_DMA_CHANNEL_INVALID) {
                        /* Do Nothing here for UART.
                           DMA interrupt driven */
                } else {
                        // Direct UART Interrupt driven and handling
                        hw_uart_enable_rx_int(uart, true);
                }
#else
                // Direct UART Interrupt driven and handling
...
void hw_uart_deinit(const HW_UART_ID uart)
{
        GLOBAL_INT_DISABLE();
        ...
        //ASSERT_WARNING(hw_pd_com_is_up());
...
void hw_uart_cfg_get(const HW_UART_ID uart, uart_config *uart_cfg)
{
        ASSERT_WARNING(uart_cfg != NULL);
...
HW_UART_ERROR hw_uart_error_getf(const HW_UART_ID uart)
{
...
uint8_t hw_uart_cts_getf(const HW_UART_ID uart)
{
        // Get CTS bit from Modem Control Register
        return (uint8_t) HW_UART_REG_GETF(uart, MSR, UART_CTS);
}

uint8_t hw_uart_delta_cts_getf(const HW_UART_ID uart)
{
