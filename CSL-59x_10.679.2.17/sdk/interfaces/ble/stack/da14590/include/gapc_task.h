...
enum gapc_state_id
{
    ...
    /// Link Layer (Connection Parameters or PHY) Update Operation on-going
...
#if (RWBLE_SW_VERSION_MAJOR >= 9) && defined(F_2M)
...
#if (RWBLE_SW_VERSION_MAJOR >= 9) && defined(F_2M)
    /// Send an LE read PHY command to the controller
    GAPC_LE_READ_PHY,
    /// Send an LE set PHY command to the controller
    GAPC_LE_SET_PHY,
#endif // (RWBLE_SW_VERSION_MAJOR >= 9) && defined(F_2M)
...
#ifdef F_PCLE
    /// Send an LE Enhanced Read Transmit Power Level command to controller
    GAPC_LE_RD_TX_PWR_LVL_ENH,
    /// Send an LE Read Remote Transmit Power Level command to controller
    GAPC_LE_RD_REM_TX_PWR_LVL,
    /// Send LE Set Path Loss Reporting Parameters command to controller
    GAPC_LE_SET_PATH_LOSS_REPORT_PARAMS,
    /// Send an LE Set Path Loss Reporting Enable command to controller
    GAPC_LE_SET_PATH_LOSS_REPORT_EN,
    /// Send an LE Set Transmit Power Reporting Enable command to controller
    GAPC_LE_SET_TX_PWR_REPORT_EN,
#endif // F_PCLE
#endif //(RWBLE_SW_VERSION_MAJOR >= 9)
...
struct gapc_peer_att_info_ind
{
    ...
    /// - GAPC_DEV_RPA_ONLY: Device Resolvable Private Address Only (ESR10 - valid if RWBLE_SW_VERSION >= VERSION_8_1)
    uint8_t  req;
    /// Attribute handle
    uint16_t handle;
...
#define GAPC_NO_PREFERRED_TX 1
#define GAPC_NO_PREFERRED_RX 2

#define GAPC_PREFERRED_PHY_1M    1
#define GAPC_PREFERRED_PHY_2M    2
#define GAPC_PREFERRED_PHY_CODED 4

#define GAPC_PHY_OPTION_TX_PREFERENCE_MASK 0x3

enum gapc_le_phy_option_tx_preference
{
    gapc_phy_option_no_preferred_coding = 0,
    gapc_phy_option_preferred_coding_s2 = 1,
    gapc_phy_option_preferred_coding_s8 = 2,
};

struct gapc_le_set_phy_cmd
{
    /// GAP request type:
    /// - GAPC_LE_SET_PHY : Set LE PHY
    uint8_t operation;
    ...
    /// Use the GAPC_NO_PREFERRED_... masks for this field
    uint8_t all_phys;
    ...
    /// Use the GAPC_PREFERRED_PHY_... masks for this field
    uint8_t tx_phys;
    ...
    ///  Isolate this field with GAPC_PHY_OPTION_TX_PREFERENCE_MASK and then use values
    ///  from the enum gapc_le_phy_option_tx_preference
    uint16_t phy_options;
};

enum gapc_le_phy_rate
{
...
#ifdef F_CH2
//5.0 CH2
/// Parameters of the @ref GAPC_LE_CH_SEL_IND message
struct gapc_le_ch_sel_ind
{
    /// The channel selection algorithm used
    uint8_t ch_sel;
};
#endif
#ifdef F_PCLE
/// Parameters of the @ref GAPC_LE_RD_TX_PWR_LVL_ENH_IND message
struct gapc_le_rd_tx_pwr_lvl_enh_ind
{
    /// Status code coming from the controller
    uint8_t status;
    /// PHY
...
/// Parameters of the @ref GAPC_LE_TX_PWR_REPORT_IND message
struct gapc_le_tx_pwr_lvl_report_ind
{
    /// Status code coming from the controller
    uint8_t status;
    ///Reason of event
    ...
    ///Change in power level of transmitter (dB)
    int8_t  delta;
};

/// Parameters of the @ref GAPC_LE_PATH_LOSS_THRES_IND message
struct gapc_le_path_loss_thres_ind
{
...
/// Parameters of the @ref GAPC_LE_RD_TX_PWR_LVL_ENH_CMD message
struct gapc_le_rd_tx_pwr_lvl_enh_cmd
{
    /// GAP request type:
    /// - GAPC_LE_RD_TX_PWR_LVL_ENH : LE Enhanced Read (local) Transmit Power Level
    uint8_t operation;
    /// Value 0x01: LE 1M PHY
    /// Value 0x02: LE 2M PHY
    /// Value 0x03: LE Coded PHY with S=8 data coding
    /// Value 0x04: LE Coded PHY with S=2 data coding
    uint8_t phy;
};

/// Parameters of the @ref GAPC_LE_RD_REM_TX_PWR_LVL_CMD message
struct gapc_le_rd_rem_tx_pwr_lvl_cmd
{
    /// GAP request type:
    /// - GAPC_LE_RD_REM_TX_PWR_LVL : LE Read Remote Transmit Power Level
    uint8_t operation;
    /// Value 0x01: LE 1M PHY
    /// Value 0x02: LE 2M PHY
    /// Value 0x03: LE Coded PHY with S=8 data coding
...
/// Parameters of the @ref GAPC_LE_SET_PATH_LOSS_REPORT_PARAMS_CMD message
struct gapc_le_set_path_loss_report_params_cmd
{
    /// GAP request type:
    /// - GAPC_LE_SET_PATH_LOSS_REPORT_PARAMS : LE Set Path Loss Reporting Parameters
    uint8_t  operation;
    /// Value 0xXX: High threshold for the path loss (Units: dB)
    uint8_t  high_thres;
    /// Value 0xXX: Hysteresis value for the high threshold (Units: dB)
    uint8_t  high_hyst;
    /// Value 0xXX: Low threshold for the path loss (Units: dB)
    uint8_t  low_thres;
    /// Value 0xXX: Hysteresis value for the low threshold (Units: dB)
    uint8_t  low_hyst;
    /// Value 0xXXXX: Minimum time in number of connection events to be observed once the
    /// path crosses the threshold before an event is generated.
    uint16_t min_time_spent;
};

/// Parameters of the @ref GAPC_LE_SET_PATH_LOSS_REPORT_EN_CMD message
struct gapc_le_set_path_loss_report_en_cmd
{
    /// GAP request type:
    /// - GAPC_LE_SET_PATH_LOSS_REPORT_EN : LE Set Path Loss Reporting Enable
    uint8_t operation;
    /// Value 0x00: Reporting disabled
    /// Value 0x01: Reporting enabled
    uint8_t enable;
};

/// Parameters of the @ref GAPC_LE_SET_TX_PWR_REPORT_EN_CMD message
struct gapc_le_set_tx_pwr_report_en_cmd
{
    /// GAP request type:
    /// - GAPC_LE_SET_TX_PWR_REPORT_EN : LE Set Transmit Power Reporting Enable
    uint8_t operation;
    /// Value 0x00: Disable local transmit power reports
    /// Value 0x01: Enable local transmit power reports
    uint8_t loc_en;
    /// Value 0x00: Disable remote transmit power reports
    /// Value 0x01: Enable remote transmit power reports
    uint8_t rem_en;
};
#endif // F_PCLE
#endif // (RWBLE_SW_VERSION_MAJOR >= 9)

/// Parameters of the @ref GAPC_SET_TX_PWR_CMD message
struct gapc_set_tx_pwr_cmd
{
        /// GAP request type:
        /// - GAPC_SET_TX_PWR : Set TX power
        uint8_t operation;
        /// Power level
        uint8_t  pwr_lvl;
};
