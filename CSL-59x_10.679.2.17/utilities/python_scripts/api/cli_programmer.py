...
class CliProgrammer(Application):
    ...
    def write_partition(self, part_name, address, bin_path, **kwargs):
        cmd = ['write_partition', str(part_name), str(address), bin_path]
        return self.run(args=cmd, **kwargs)
    ...
    def erase_eflash(self, address, size, **kwargs):
        cmd = ['erase_eflash', str(address), str(size)]
        return self.run(args=cmd, **kwargs)

    def write_eflash_bytes(self, address, *data, **kwargs):
        cmd = ['write_eflash_bytes', str(address), ' '.join(map(str, data))]
        return self.run(args=cmd, **kwargs)

    def write_eflash(self, address, bin_path, size=None, **kwargs):
        cmd = ['write_eflash', str(address), bin_path]
        if size:
            cmd.append(str(size))
        return self.run(args=cmd, **kwargs)

    def read_eflash(self, address, size, **kwargs):
        cmd = ['read_eflash', str(address), "-", str(size)]
        return self.run(args=cmd, **kwargs)

    def chip_erase_eflash(self, **kwargs):
        cmd = ['chip_erase_eflash']
        return self.run(args=cmd, **kwargs)
