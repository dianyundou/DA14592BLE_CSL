...
static ini_line_t parse_line(const char *line, char *key, char *value)
{
        ...
        case '[':
                ...
                if (last_char - ptr >= MAX_KEY_LENGTH) {
                        return INI_LINE_INVALID;
                }
                memcpy(value, ptr, last_char - ptr);
                value[last_char - ptr] = '\0';
                strncpy(key, value, MAX_KEY_LENGTH - 1);
        ...
        default:
                ...
                /* Check that parsed key will fit */
                if (last_char - ptr + 1 >= MAX_KEY_LENGTH) {
                        return INI_LINE_INVALID;
                }
                ...
                /* Check that parsed value will fit */
                if (ptr_end - ptr - 1 >= MAX_VALUE_LENGTH) {
                        return INI_LINE_INVALID;
                }
