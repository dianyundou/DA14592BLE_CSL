...
static void opth_help(void)
{
        ...
        printf("    read_eflash <address> <file> <size>\n");
        printf("        Reads <size> bytes from eFLASH memory starting at <address> into <file>.\n");
        printf("        If <file> is specified as either '-' or '--', data is output to stdout \n"
                "        as hexdump.\n");
        printf("        hexdump is either 16-bytes (-) or 32-bytes (--) wide\n");
        printf("    write_eflash <address> <file> [<size>]\n");
        printf("        Writes up to <size> bytes of <file> into eFLASH at <address>.\n");
        printf("        If <size> is omitted, complete file is written.\n");
        printf("    write_eflash_bytes <address> <data1> [<data2> [...]]\n");
        printf("        Writes bytes specified on command line into eFLASH at <address>.\n");
        printf("    copy_eflash <address_ram> <address_eflash> <size>\n");
        printf("        Copies <size> bytes from RAM memory starting at <address_ram> to eFLASH \n"
                "        at <address_eflash>.\n");
        printf("    is_empty_eflash [<start_address size>]\n");
        printf("        Checks if <size> bytes of eFLASH contain only 0xFF values, starting\n"
                "        from <start_address>. Default value of <size> is 256 KB and default value of\n"
                "        <start_address> is 0.\n");
        printf("    erase_eflash <address> <size>\n");
        printf("        Erases <size> bytes of eFLASH starting at <address>.\n");
        printf("        Note: Actual area erased may be larger due to size of erase block.\n");
        printf("    chip_erase_eflash\n");
        printf("        Erases whole eFLASH memory.\n");
        ...
        printf("        Checks if <size> bytes of QSPI flash contain only 0xFF values, starting\n"
        ...
        printf("        Reads <length> 32-bit words from the write-protectable area of eFLASH at <address>.\n");
        printf("    write_otp <address> <length> [<data> [<data> [...]]]\n");
        printf("        Writes <length> words to the write-protectable area of eFLASH at <address>.\n");
        printf("        <data> are 32-bit words to be written. If less than <length> words are \n"
                "        specified, remaining words are assumed to be 0x00.\n");
        printf("        If write-protection in the sector of the specified location has been enabled,\n"
                "        writing will fail.\n");
        printf("        Note: This command does not enable write-protection after writing. This has to be\n"
                "        done manually afterwards, by writing the proper command in the Configuration Script.\n");
        printf("    boot <binary_file>\n");
        printf("        Boots application binary using 1st stage bootloader (ROM booter).\n"
               "        If the application is too big (more than 64kB) and serial interface is\n"
               "        used, then the \"run\" command should be used instead.\n");
        printf("    run <binary_file>\n");
        printf("        Runs application binary using 2nd stage bootloader (uartboot).\n");
        ...
        printf("> cli_programmer COM40 boot test_api.bin\n\n");
        ...
        printf("> cli_programmer -b C:\\Users\\user\\SDK\\sdk\\bsp\\system\\loaders\\uartboot\\Release\\uartboot.bin "
        ...
        printf("Write 6 bytes specified in command line to QSPI flash/RAM at address 0x80000\n");
        printf("> cli_programmer gdbserver write_qspi_bytes 0x80000 0x11 0x22 0x33 0x44 0x55 0x66\n\n");
        printf("Read 16 bytes from eFLASH address 0x10\n");
        printf("> cli_programmer gdbserver read_eflash 0x00000010 -- 16\n\n");
}
