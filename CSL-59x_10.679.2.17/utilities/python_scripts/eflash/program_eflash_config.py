...
DEFAULT_CONFIG_PATH = os.path.join(os.path.join(os.path.dirname(__file__), 'program_eflash.xml'))
DEFAULT_FLASH_CONFIGS = os.path.join(PROJECT_ROOT, "eflash", "flash_configurations.xml")
MIN_FW_ADDRESS = 0x2000

PRODUCT_IDS_DICT = OrderedDict((
    ('DA1459x-00', ProductId.DA1459x_00),
))


class ProgramEFLASHConfig(object):
    PROGRAM_EFLASH_TAG = 'program_eflash'
    ...
    def load(self):
        ...
        try:
            ...
            root = ElemTree.Element(self.PROGRAM_EFLASH_TAG)
        ...
        if product_header:
            ...
            #flash_write_config_command = None
    ...
    def is_valid(self):
        ...
        if self.product_id == ProductId.DA1459x_00:
        ...
        for item in fconfig:
            try:
                if self.flash_burstcmda_reg_value == \
                    int(fconfig[item][ProgramEFLASHConfig.FLASH_BURSTCMDA_REG_VALUE_TAG], 16) \
                    and self.flash_name == \
                    str(fconfig[item][ProgramEFLASHConfig.FLASH_NAME_TAG]) \
                    and self.flash_size == \
                    int(fconfig[item][ProgramEFLASHConfig.FLASH_SIZE_TAG], 16) \
                    and self.flash_burstcmdb_reg_value == \
                    int(fconfig[item][ProgramEFLASHConfig.FLASH_BURSTCMDB_REG_VALUE_TAG], 16):
                        return True
            except:
    ...
    def get_config_info(self):
        ...
        if self.product_id == ProductId.DA1459x_00 and self.product_id is not None:
            buf += 'Product Header:\n'
            ...
            buf += '\tflash_size: {}K\n'.format(
                int(self.flash_size / 1024) if self.flash_size is not None else 'N/A')
...
def program_eflash_config(product_id=None, flash_config_path=None, header_config_path=None,
                        flash_configuration=None, default_img_addr=False):


    config = ProgramEFLASHConfig(header_config_path)
    ...
    if config.product_id == ProductId.DA1459x_00:
        ...
        config.flash_size = int(configuration[ProgramEFLASHConfig.FLASH_SIZE_TAG], 16)
        config.flash_burstcmda_reg_value = \
            int(configuration[ProgramEFLASHConfig.FLASH_BURSTCMDA_REG_VALUE_TAG], 16)
        config.flash_burstcmdb_reg_value = \
            int(configuration[ProgramEFLASHConfig.FLASH_BURSTCMDB_REG_VALUE_TAG], 16)
        config.flash_write_config_command = ()
        ...
        if default_img_addr:
            ...
            config.active_image_address = \
                _setup_address(ProgramEFLASHConfig.ACTIVE_FW_IMAGE_ADDRESS_TAG,
                               'Insert Active FW image address (hex)')

            ui.print_title('Update FW Image Address\n')
            config.update_image_address = \
                _setup_address(ProgramEFLASHConfig.UPDATE_FW_IMAGE_ADDRESS_TAG,
...
if __name__ == '__main__':
    ui.print_header('PROGRAM EFLASH CONFIGURATOR')
    run_script(program_eflash_config, parse_args)
