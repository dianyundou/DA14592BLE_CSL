 ...
 *
 * Copyright (c) 2015-2023 Modified by Renesas Electronics Corporation and/or its affiliates.
 *
 ****************************************************************************************
 */
...
//5.0 2M
#ifdef F_2M
#define HCI_LE_READ_PHY_CMD_OPCODE                      0x2030
#define HCI_LE_SET_DEFAULT_PHY_CMD_OPCODE               0x2031
#define HCI_LE_SET_PHY_CMD_OPCODE                       0x2032
#define HCI_LE_RX_TEST_ENH_CMD_OPCODE                   0x2033
#define HCI_LE_TX_TEST_ENH_CMD_OPCODE                   0x2034
#endif

#ifdef F_PCLE
#define HCI_LE_TX_TEST_V3_CMD_OPCODE                    0x2050
#endif

#define HCI_LE_READ_TRANSMIT_POWER_CMD_OPCODE           0x204B

#ifdef F_PCLE
#define HCI_LE_RD_RF_PATH_COMPENSATION_CMD_OPCODE       0x204C
#define HCI_LE_WR_RF_PATH_COMPENSATION_CMD_OPCODE       0x204D
#endif // F_PCLE

#define HCI_LE_GENERATE_DHKEY_V2_CMD_OPCODE             0x205E

//5.2 Power Control LE
#ifdef F_PCLE
#define HCI_LE_RD_TX_PWR_LVL_ENH_CMD_OPCODE             0x2076
#define HCI_LE_RD_REM_TX_PWR_LVL_CMD_OPCODE             0x2077
#define HCI_LE_SET_PATH_LOSS_REPORT_PARAMS_CMD_OPCODE   0x2078
#define HCI_LE_SET_PATH_LOSS_REPORT_EN_CMD_OPCODE       0x2079
#define HCI_LE_SET_TX_PWR_REPORT_EN_CMD_OPCODE          0x207A
#define HCI_LE_TX_TEST_V4_CMD_OPCODE                    0x207B
#endif // F_PCLE
...
#define HCI_DBG_TEST_SET_TX_PW_CMD_OPCODE               0xFC13
#define HCI_DBG_TX_TEST_ENH_CMD_OPCODE                  0xFC14
#define HCI_DBG_RX_TEST_ENH_CMD_OPCODE                  0xFC15
#define HCI_DBG_RX_TEST_STATS_GET_CMD_OPCODE            0xFC16
#define HCI_DBG_START_CALIBRATION_CMD_OPCODE            0xFC17
#define HCI_DBG_GET_CAL_RESULT_CMD_OPCODE               0xFC18
#define HCI_DBG_SET_EVT_RPRT_STAT_CMD_OPCODE            0xFC19
...
#ifdef F_CH2
//5.0 CH2
#define HCI_DBG_WR_CH_SEL_CMD_OPCODE                    0xFC44
#endif // F_CH2

#define HCI_DBG_SET_CONN_TX_PW_CMD_OPCODE               0xFC45
#ifdef F_PCLE
#define HCI_DBG_LE_SET_REM_TX_PWR_LVL_CMD_OPCODE        0xFC46
#endif // F_PCLE
...
#ifdef F_CH2
//5.0 CH2
#define HCI_LE_CH_SEL_EVT_SUBCODE                       0x14
#endif // F_CH2
#ifdef F_2M
//5.0 2M
#define HCI_LE_PHY_UPDATE_CMP_EVT_SUBCODE               0x0C
#endif // F_2M
#ifdef F_PCLE
#define HCI_LE_PATH_LOSS_THRES_EVT_SUBCODE              0x20
#define HCI_LE_TX_PWR_REPORT_EVT_SUBCODE                0x21
#endif // F_PCLE
#endif // (RWBLE_SW_VERSION_MAJOR >= 9)
#endif // (RWBLE_SW_VERSION_MAJOR >= 8)

///Debug Events Subcodes
#define HCI_DBG_RX_REPORT_EVT_SUBCODE                   0x01
#define HCI_DBG_RF_CALIB_EN_EVT_SUBCODE                 0x02
#define HCI_DBG_RF_CALIB_DIS_EVT_SUBCODE                0x03
#define HCI_DBG_SYS_PROC_EVT_SUBCODE                    0xFF
...
struct hci_acl_data_tx
{
	...
	#if (BLE_EMB_PRESENT)
    /// Pointer to the Tx descriptor containing the Tx Data
    struct co_buf_tx_node *desc;

    /// Pointer to the buffer containing the Tx Data
    uint8_t* buffer;

    /// How many bytes have been processed so far
    uint16_t index;

    /// Number of descriptors used for this packet
    uint8_t desc_cnt;
    #else // (BLE_HOST_PRESENT)
    /// Pointer to the data buffer
    uint8_t* buffer;
...
struct hci_le_set_adv_param_cmd
{
    ...
    ///         0x00: Public
    ///         0x01: Host Random
    ///         0x02: Controller RPA or Public
    ///         0x03: Controller RPA or Host Random
    uint8_t        own_addr_type;
    ///Peer_Address_Type:
    ///         0x00: Public Device or Identity Address,
    ///         0x01: Random Device or Random Static Identity Address,
...
struct hci_le_create_con_cmd
{
    ...
    ///         0x02: Public Identity Address (Corresponds to peer's RPA).
    ///         0x03: Random (static) Identity Address (Corresponds to peer's RPA)
...
#ifdef F_2M
//5.0 2M
/// HCI LE set default phy
struct hci_le_set_default_phy_cmd
{
    ///all_phys
    uint8_t         all_phys;
    ///tx_phys
...
struct hci_le_set_phy_cmd
{
    ...
    ///rx_phys
    uint8_t         rx_phys;
    ///PHY_options, used for LE CODED PHY
    uint16_t        phy_options;
};

///HCI LE Enhanced Receiver Test Command parameters structure
struct hci_le_rx_test_enh_cmd
{
    ///RX frequency for Rx test
    uint8_t        rx_freq;
    ///RX PHY for Rx test
    uint8_t        rx_phy;
    ///RX modulation index for Rx test
    uint8_t        rx_mod;
};

///HCI LE Enhanced Transmitter Test Command parameters structure
struct hci_le_tx_test_enh_cmd
{
    ///TX frequency for Tx test
    uint8_t        tx_freq;
    ///TX test data length
...
#ifdef F_PCLE
///HCI LE Transmitter Test Command [v3] parameters structure
struct hci_le_tx_test_v3_cmd
{
    ...
    ///  Values: 0x02 to 0x4B (all other values are reserved for future use)
    uint8_t     switching_pattern_length;
    ///List of Antenna IDs in the pattern
    uint8_t     antenna_ids[0x4B];
};

///HCI LE Transmitter Test Command [v4] parameters structure
struct hci_le_tx_test_v4_cmd
{
    ...
    ///  Values:
    ///     0xXX: Set transmitter to the specified or the nearest transmit power level
...
///HCI LE Enhanced Read Transmit Power Level Command structure
struct hci_le_rd_tx_pwr_lvl_enh_cmd
{
    ///connection handle
    uint16_t       conhdl;
    ///PHY
...
/// HCI command complete event structure for the LE Enhanced Read Transmit Power Level Command
struct hci_le_rd_tx_pwr_lvl_enh_cmd_cmp_evt
{
    ///Status for command reception
    uint8_t        status;
    ///Connection handle
...
///HCI LE Read Remote Transmit Power Level Command structure
struct hci_le_rd_rem_tx_pwr_lvl_cmd
{
    ///connection handle
    uint16_t       conhdl;
    ///PHY
...
///LE Set Path Loss Reporting Parameters Command structure
struct hci_le_set_path_loss_report_params_cmd
{
    ///connection handle
    uint16_t       conhdl;
    ///high threshold (dBm)
...
///LE Set Path Loss Reporting Enable Command structure
struct hci_le_set_path_loss_report_en_cmd
{
    ///connection handle
    uint16_t       conhdl;
    ///enable
...
///LE Set Transmit Power Reporting Enable Command structure
struct hci_le_set_tx_pwr_report_en_cmd
{
    ///connection handle
    uint16_t       conhdl;
...
/// LE Write RF Path Compensation Command structure
struct hci_le_wr_rf_path_compensation_cmd
{
    /// RF TX Path Compensation Value
    int16_t          rf_path_tx; // value in units of 0.1dB, i.e. 128.0 dB = 0x0500
    /// RF RX Path Compensation Value
    int16_t          rf_path_rx; // value in units of 0.1dB
};
#endif // F_PCLE
...
struct hci_rd_adv_chnl_tx_pw_cmd_cmp_evt
{
    ...
    /// On a DA1469x based controller, different TX power levels can be configured
    /// for advertising, scanning and initiating. This completion event shall return
    /// the TX power level configured for advertising.
...
struct hci_dbg_evt
{
    ...
    ///Data length
    uint8_t             len;
    ///Buffer to hold the report data
    uint8_t             report[64];
};

///HCI DBG RX report structure
struct hci_dbg_rx_report_subevt
{
    ///Pointer to the received payload data
    uint8_t             *payload;
...
#ifdef F_CH2
//5.0 CH2
/// HCI LE channel selection algorithm event structure
struct hci_le_ch_sel_evt
{
    ///LE Subevent code
    uint8_t             subcode;
    ///Connection handle
    uint16_t            conhdl;
    ///Chennel selection algorithm, 0 = Algorithm #1 (legacy), 1 = Algorithm #2
    uint8_t             ch_sel;
};
#endif // F_CH2
...
#ifdef F_2M
//5.0 2M
/// HCI command complete event structure for LE Read PHY Command
struct hci_le_read_phy_cmd_cmp_evt
{
    ///Status of command reception
    uint8_t            status;
    ///Connection handle
...
/// HCI command complete event structure for HCI LE PHY Update Complete Command
...
#ifdef F_PCLE
/// HCI LE path loss threshold event
struct hci_le_path_loss_thres_evt
{
    ///LE Subevent code
    uint8_t             subcode;
    ///Connection handle
    uint16_t            conhdl;
    ///Current path loss value
    uint8_t             curr_path_loss;
    ///Zone entered
    uint8_t             zone_enter;
};

/// HCI LE transmit power reporting event
struct hci_le_tx_pwr_report_evt
{
    ///LE Subevent code
    uint8_t             subcode;
    ///Status of received command
    ...
    ///Connection handle
    uint16_t            conhdl;
    ///Reason of event and device (local or remote)
    uint8_t             reason;
    ///PHY
    uint8_t             phy;
    ///Value of TX power level (dBm)
    int8_t              tx_pwr_lvl;
    ///TX power level min or max
    uint8_t             tx_pwr_lvl_flag;
    ///Change in power level of transmitter (dBm)
    int8_t              delta;
};

///HCI command complete event structure for the LE Read RF Path Compensation command
struct hci_le_rd_rf_path_compensation_cmd_cmp_evt
{
    /// Status of the command reception
    uint8_t     status;
    /// RF TX Path Compensation Value
    int16_t     rf_tx_path_compens;
    /// RF RX Path Compensation Value
    int16_t     rf_rx_path_compens;
};

///HCI command complete event structure for the LE Write RF Path Compensation command
struct hci_le_wr_rf_path_compensation_cmd_cmp_evt
{
    /// Status of the command reception
    uint8_t     status;
    /// RF TX Path Compensation Value
...
///HCI Debug set Tx power in Test Mode - vendor specific
struct hci_dbg_set_test_tx_pw_cmd
{
    /// Tx power
    uint8_t pwr_level;
...
#define MODULATED_INFINITE_PACK_TEST            (1)
#define MODULATED_INFINITE_NONPACK_TEST         (2)
#define MODULATED_FINITE_PACK_TEST              (3)
#define UNMODULATED_INFINITE_NONPACK_TEST       (4)

///HCI Debug start enhanced Tx Test - vendor specific
struct hci_dbg_tx_test_enh_cmd
{
    /// Tx channel
    uint8_t tx_freq;
    /// Tx data length
    ...
    /// Test type
    uint8_t test_type;
    /// Tx burst of N packets
    uint16_t tx_packet_num;
    /// Tx interval in usec
    uint32_t tx_interval_us;
};

///HCI Debug start enhanced Rx Test - vendor specific
struct hci_dbg_rx_test_enh_cmd
{
    /// Rx channel
    uint8_t rx_freq;
    /// PHY (1M or 2M)
    ...
    /// Tx data length
    uint8_t test_data_len;
    /// Tx side packet interval in usec
    uint32_t rx_interval_us;
    /// Rx window size applied at "event interval" in usec
    uint16_t normal_winsz;
    /// Rx window size applied to the 1st reception ("scan" mode) in slots
    uint16_t first_winsz;
};

///Common structure for Command Complete Event of HCI Debug Read Rx Test counters - vendor specific
struct hci_dbg_rx_test_stats_get_cmd_cmp_evt
{
    ///Status
    uint8_t status;
    ///Total packets received
    uint16_t rx_packets_total;
    ///Packets with SYNC error
    uint16_t rx_packets_syncerr;
    ///Packets with LEN error
    uint16_t rx_packets_lenerr;
    ///Packets with CRC error
    uint16_t rx_packets_crcerr;
    ///RSSI
    uint16_t rssi;
};

///HCI Debug start enhanced Rx Test - vendor specific
struct hci_dbg_start_calibration_cmd
{
    ///Calibration type
...
///HCI Debug set event reporting status - vendor specific
struct hci_dbg_set_evt_reporting_cmd
{
    ///Status
    uint8_t enable;
};
...
#if (DA14690)
///HCI Debug Set ADV/SCAN/INITIATE/RFTEST TX Power Level Command parameters
struct hci_dbg_set_tx_pw_cmd
{
    /// Air Operation: ADV/SCAN/INITIATE/RFTEST
    uint8_t airop;
    /// Power level
    uint8_t pw_lvl;
};

///HCI Debug Set Connection TX Power Level Command parameters
struct hci_dbg_set_conn_tx_pw_cmd
{
    /// Connection handle
    uint16_t conhdl;
    /// Power level
...
#ifdef F_PCLE
///HCI Debug Set Remote Transmit Power Level Command structure
struct hci_dbg_le_set_rem_tx_pwr_lvl_cmd
{
    ///connection handle
    uint16_t       conhdl;
    ///PHY
...
#ifdef F_CH2
//5.0 CH2
///HCI Debug Channel Selection #2 supported command
struct hci_dbg_wr_ch_sel_cmd
{
    ///channel selection 0=disable 1=enable
    uint8_t    ch_sel;
};

///HCI Debug Channel Selection #2 supported command complete event
struct hci_dbg_wr_ch_sel_cmd_cmp_evt
{
    /// status
    uint8_t status;
};
...
/// HCI command complete event structure for the Generate DHKey Command (for both V1 and V2)
...
/// HCI command complete event structure for the Read Tx Power Command
struct hci_le_read_transmit_power_cmd_cmp_evt
{
    /// Status of the command reception
    uint8_t     status;
