...
/* LED blink rate (as interval in msec) */
#ifndef LED_CONTROL_BLINK_RATE_MS
#define LED_CONTROL_BLINK_RATE_MS       ( 500 )
#endif
...
void led_control_init(OS_TASK task_hdl)
{
        ...
        led_blink_tim = OS_TIMER_CREATE("led", OS_MS_2_TICKS(LED_CONTROL_BLINK_RATE_MS),
...
void led_control_set_mode(LED_CONTROL_MODE mode, uint16_t blink_cnt)
{
        ...
        case LED_CONTROL_MODE_START_BLINK:
                ...
                OS_TIMER_CHANGE_PERIOD(led_blink_tim, OS_MS_2_TICKS(LED_CONTROL_BLINK_RATE_MS),
                        OS_TIMER_FOREVER);
                /* no break */
    ...
    if (notif & LED_CONTROL_NOTIF) {
            if (led_mode == LED_CONTROL_MODE_BLINK) {
                    if (led_blink_toggles == 0) {
                            led_control_set_mode(LED_CONTROL_MODE_SET_OFF, 0);
                    } else {
