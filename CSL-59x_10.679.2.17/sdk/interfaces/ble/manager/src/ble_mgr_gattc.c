...
static void send_browse_gtl_cmd(const att_uuid_t *uuid, uint16_t conn_idx, uint16_t start_hdl,
                                                                                uint16_t end_hdl)
{
        struct gattc_sdp_svc_disc_cmd *gcmd;
        ble_mgr_common_stack_msg_t *gmsg;

        gmsg = ble_gtl_alloc_with_conn(GATTC_SDP_SVC_DISC_CMD, TASK_ID_GATTC, conn_idx,
        ...
        if (uuid) {
                ...
                uuid_dg2rw(uuid, &gcmd->uuid_len, gcmd->uuid);
        } else {
                gcmd->operation = GATTC_SDP_DISC_SVC_ALL;
        }

        gcmd->start_hdl = start_hdl;
        gcmd->end_hdl = end_hdl;
...
void ble_mgr_gattc_browse_cmd_handler(void *param)
{
        ...
        /* Setup and send GTL message */
        send_browse_gtl_cmd(cmd->uuid, conn_idx, 1, 65535);
...
void ble_mgr_gattc_browse_range_cmd_handler(void *param)
{
        const ble_mgr_gattc_browse_range_cmd_t *cmd = param;
        ble_mgr_gattc_browse_range_rsp_t *rsp;
        ble_error_t ret = BLE_ERROR_FAILED;
        device_t *dev;
        ...
        /* Setup and send GTL message */
        send_browse_gtl_cmd(cmd->uuid, cmd->conn_idx, cmd->start_h, cmd->end_h);

        ret = BLE_STATUS_OK;

done:
        ble_msg_free(param);
        rsp = ble_msg_init(BLE_MGR_GATTC_BROWSE_RANGE_CMD, sizeof(*rsp));
        rsp->status = ret;
        ble_mgr_response_queue_send(&rsp, OS_QUEUE_FOREVER);
}
