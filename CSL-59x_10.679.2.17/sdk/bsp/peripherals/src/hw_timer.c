...
// Timer, Timer2, Timer3 are in PD_TIM and can be active during sleep. Callback needs to be retained
...
// Timer4 are in PD_COM can be active during sleep. Callback needs to be retained
...
void hw_timer_init(HW_TIMER_ID id, const timer_config *cfg)
{
        ...
        /* Reset control register and set clock source */
        TBA(id)->TIMER_CTRL_REG =  BITS32(TIMER, TIMER_CTRL_REG, TIM_SYS_CLK_EN, cfg->clk_src);
...
void hw_timer_configure_pwm(HW_TIMER_ID id, const timer_config_pwm *cfg)
{
        ...
        if (cfg->pwm_active_in_sleep) {
                if (id == HW_TIMER) {
                        /* For TIMER, only P0_12 supports PWM during sleep */
                        if (cfg->port == HW_GPIO_PORT_0 && cfg->pin == HW_GPIO_PIN_12) {
                                REG_SET_BIT(CRG_TOP, CLK_TMR_REG, TMR_PWM_AON_MODE);
                        } else {
                                ASSERT_WARNING(0);
                        }
                } else if (id == HW_TIMER2) {
                        /* For TIMER2, only P0_10 supports PWM during sleep */
                        if (cfg->port == HW_GPIO_PORT_0 && cfg->pin == HW_GPIO_PIN_10) {
                ...
                if (id == HW_TIMER) {
                        REG_CLR_BIT(CRG_TOP, CLK_TMR_REG, TMR_PWM_AON_MODE);
                } else if (id == HW_TIMER2) {
                        REG_CLR_BIT(CRG_TOP, CLK_TMR_REG, TMR2_PWM_AON_MODE);
                }
        }
}
