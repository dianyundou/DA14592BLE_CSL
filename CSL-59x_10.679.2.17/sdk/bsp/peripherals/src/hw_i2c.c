...
#if defined(HW_I2C3)
#define I2C_ID_IS_VALID(id)     ((id == HW_I2C3) || (id == HW_I2C2) || (id == HW_I2C1))
#elif defined(HW_I2C2)
#define I2C_ID_IS_VALID(id)     ((id == HW_I2C2) || (id == HW_I2C1))
#else
#define I2C_ID_IS_VALID(id)     (id == HW_I2C1)
#endif
...
{
        ASSERT_ERROR(I2C_ID_IS_VALID(id));
...
__STATIC_INLINE IRQn_Type get_irq_type(const HW_I2C_ID id)
{
        ASSERT_ERROR(I2C_ID_IS_VALID(id));
...
{
        IRQn_Type irq_type = get_irq_type(id);
...
bool hw_i2c_is_clk_enabled(const HW_I2C_ID id)
{
        ...
        return (!!(REG_GETF(CRG_COM, CLK_COM_REG, I2C_ENABLE)));
}

__STATIC_INLINE uint8_t get_speed_for_register(HW_I2C_SPEED speed)
{
        switch (speed) {
...
__STATIC_INLINE void set_hs_master_code(const HW_I2C_ID id)
{
        ASSERT_WARNING(hw_i2c_is_enabled(id) == false);
...
__STATIC_INLINE void configure_master_control(HW_I2C_ID id, uint8_t speed_val, HW_I2C_ADDRESSING addr_mode)
{
        uint32_t tmp = IBA(id)->I2C_CON_REG;
        HW_I2C_REG_SET_FIELD(I2C_CON, I2C_MASTER_MODE, tmp, 1);
        HW_I2C_REG_SET_FIELD(I2C_CON, I2C_SLAVE_DISABLE, tmp, 1);
        HW_I2C_REG_SET_FIELD(I2C_CON, I2C_SPEED, tmp, speed_val);
        HW_I2C_REG_SET_FIELD(I2C_CON, I2C_10BITADDR_MASTER, tmp, (addr_mode == HW_I2C_ADDRESSING_10B) ? 1 : 0);
        IBA(id)->I2C_CON_REG = tmp;
}

#if (HW_I2C_SLAVE_SUPPORT == 1)
__STATIC_INLINE void configure_slave_control(HW_I2C_ID id, uint8_t speed_val, HW_I2C_ADDRESSING addr_mode)
{
        uint32_t tmp = IBA(id)->I2C_CON_REG;
        HW_I2C_REG_SET_FIELD(I2C_CON, I2C_MASTER_MODE, tmp, 0);
        HW_I2C_REG_SET_FIELD(I2C_CON, I2C_SLAVE_DISABLE, tmp, 0);
        HW_I2C_REG_SET_FIELD(I2C_CON, I2C_SPEED, tmp, speed_val);
        HW_I2C_REG_SET_FIELD(I2C_CON, I2C_10BITADDR_SLAVE, tmp, (addr_mode == HW_I2C_ADDRESSING_10B) ? 1 : 0);
        IBA(id)->I2C_CON_REG = tmp;
}
#endif
...
void hw_i2c_configure(HW_I2C_ID id, const i2c_config *cfg)
{
        ...
        uint8_t speed = get_speed_for_register(cfg->speed);

        if (cfg->mode == HW_I2C_MODE_MASTER) {
                configure_master_control(id, speed, cfg->addr_mode);
                set_hs_master_code(id);
                hw_i2c_set_target_address(id, cfg->address);
        } else {
#if (HW_I2C_SLAVE_SUPPORT == 1)
                configure_slave_control(id, speed, cfg->addr_mode);
                hw_i2c_set_slave_address(id, cfg->address);
                hw_i2c_set_slave_callback(id, cfg->event_cb);
#else
                ASSERT_WARNING(0);
#endif /* HW_I2C_SLAVE_SUPPORT */
        }
...
#if (HW_I2C_SLAVE_SUPPORT == 1)
void hw_i2c_setup_slave(HW_I2C_ID id, HW_I2C_ADDRESSING addr_mode, uint16_t addr, hw_i2c_event_cb cb)
{
        uint32_t tmp = IBA(id)->I2C_CON_REG;
        HW_I2C_REG_SET_FIELD(I2C_CON, I2C_MASTER_MODE, tmp, 0);
        HW_I2C_REG_SET_FIELD(I2C_CON, I2C_SLAVE_DISABLE, tmp, 0);
