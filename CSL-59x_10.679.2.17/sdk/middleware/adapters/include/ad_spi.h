 ...
 *
 * \note The spi_cs configuration prevails over the driver's CS pad and mode configuration.
 *       The first CS of the spi_cs list is configured with \sa ad_spi_open().
 *       \sa ad_spi_controller_conf_t hw_spi_config_t
 */
typedef struct {
        ...
        const uint8_t cs_cnt;           /**< Number of configured CS pins */
        ad_io_conf_t* spi_cs;           /**< List of CS's signals  configuration */
}  ad_spi_io_conf_t;
 ...
 */
typedef struct {
        hw_spi_config_t spi;            /**< Low level driver configuration */
...
typedef struct {
        ...
        const ad_spi_driver_conf_t      *drv;       /**< Driver configuration - CS-related configuration is discarded */
 ...
 * \sa ad_spi_open()
 *
 * \note No change in I/O configuration is done.
 *
 */
 ...
 * \param [in] csid   The index in the CS list
 *
