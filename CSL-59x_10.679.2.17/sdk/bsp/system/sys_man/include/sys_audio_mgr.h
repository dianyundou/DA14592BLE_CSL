...
#define MAX_NUM_OF_PATHS         4      // the max number of concurrently supported audio data paths according to device family
...
typedef struct {
        ...
        hw_sdadc_interrupt_cb cb;         /**< Data buffer package ready callback */
 ...
 */
typedef struct {
        audio_path_t audio_path[MAX_NUM_OF_PATHS]; /**< Audio data paths*/
} sys_audio_path_t;
...
{
        for (uint8_t idx = 0; idx < MAX_NUM_OF_PATHS; idx++) {
