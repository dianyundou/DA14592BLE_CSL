 ...
 *
 * Copyright (C) 2016-2018 Renesas Electronics Corporation and/or its affiliates.
 ...
 *
 * Called when the HRS Client receives Heart Rate Measurement notification from server.
 * The rr_values have variable length in the received data. To prevent the memory leak
 * caused by OS_MALLOC (measurement) this callback uses OS_FREE()
 *
 * \param [in] hrs_client       HRS Client instance
 * \param [in] measurement      heart rate measurements
 *
 */
typedef void (* hrs_client_heart_rate_measurement_notif_cb_t) (ble_client_t *hrs_client,
 ...
 *
 */
typedef void (* hrs_client_get_event_state_completed_cb_t) (ble_client_t *hrs_client,
 ...
 *
 */
typedef void (* hrs_client_set_event_state_completed_cb_t) (ble_client_t *hrs_client,
                                                                hrs_client_event_t event,
                                                                att_error_t status);
 ...
 *
 */
typedef void (* hrs_client_read_body_sensor_location_cb_t) (ble_client_t *hrs_client,
                                                        att_error_t status,
 ...
 *
 */
typedef void (* hrs_client_reset_energy_expended_completed_cb_t) (ble_client_t *hrs_client,
                                                                                att_error_t status);
 ...
 *
 * \param [in] cb               HRS Client application callbacks
 ...
 * callbacks
 *
 * Function semantics is very similar to hrs_client_init(). The difference is that internal
 * data is initialized by the buffered context and the HRS Client is automatically added to active
 * clients collection.
 *
 ...
 *
 * \return HRS Client instance when initialized properly, NULL otherwise
 */
