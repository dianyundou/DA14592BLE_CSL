...
#define UUID_SUOTA_SERVICE              (dg_configBLE_UUID_SUOTA_SERVICE)
...
static bool ready_for_suota(suota_service_t *suota, uint16_t conn_idx)
{
        if (suota->cb && suota->cb->suota_ready) {
                return suota->cb->suota_ready(conn_idx);
        }
...
{
        if ((status == SUOTA_ERROR) && (suota->notified_app_status_mask & SUOTA_ERROR)) {
                /* Make sure that SUOTA_ERROR will be notified at most once */
                return;
        }

        suota->notified_app_status_mask |= status;

        if (suota->cb && suota->cb->suota_status) {
                suota->cb->suota_status(suota->conn_idx, status, error_code);
        }
}
...
void suota_send_client_status_notifications(suota_service_t *suota, uint16_t conn_idx)
{
        ...
        while (queue_length(&suota->client_status_notif_q)) {
                ...
                OS_ASSERT(client_status_notif != NULL);
...
bool safe_product_header_write(nvms_t ph_part, size_t offset, const uint8_t *ph, uint16_t ph_size)
{
        int retry_cnt;
...
static bool set_active_img_ptr(suota_service_t *suota)
{
        ...
        if (!safe_product_header_write(product_header_partition, 0, ph_buffer, ph_size)) {
                OS_FREE(ph_buffer);
                return false;
        }

        /* Update backup product header */
        if (!safe_product_header_write(product_header_partition, PH_BACKUP_OFFSET, ph_buffer, ph_size)) {
                OS_FREE(ph_buffer);
                return false;
        }
...
static void prepare_flash(suota_service_t *suota, size_t write_size)
{
        ...
        sector_size = ad_nvms_erase_size(suota->nvms);
        erase_size = end_addr - suota->flash_erase_addr + 1;

        /* Check if region [suota->flash_erase_addr, suota->flash_erase_addr + sector_size] is
        ...
        suota->flash_erase_addr |= (sector_size - 1);
...
static bool validate_img_hdr(suota_1_1_image_header_da1469x_t *hdr)
{
...
static uint32_t get_code_size(suota_1_1_image_header_da1469x_t *hdr)
{
        return hdr->size;
}

static uint32_t get_exec_location(suota_1_1_image_header_da1469x_t *hdr)
{
        return hdr->pointer_to_ivt;
}

static bool validate_img_size(suota_service_t *suota)
{

        /* SUOTA 1.1 header + header extension + application code */
        return get_exec_location(&suota->header) + get_code_size(&suota->header) <=
                                                                ad_nvms_get_size(suota->nvms);
}
...
static bool suota_state_w4_header(suota_service_t *suota)
{
        ...
        if (!validate_img_hdr(&suota->header)) {
                suota->error_cb(suota, SUOTA_INVAL_IMG_HDR);
                return false;
        }

        if (!validate_img_size(suota)) {
...
static bool suota_state_w4_header_ext(suota_service_t *suota)
{
        ...
        if (suota->recv_hdr_ext_len == get_exec_location(&suota->header) - sizeof(suota->header)) {
...
static bool suota_state_w4_image_data(suota_service_t *suota)
{
        ...
        if (write_count == suota->buffer_len) {
                ...
                int read_count = ad_nvms_read(suota->nvms, suota->flash_write_addr, suota->buffer, suota->buffer_len);
                if (read_count == suota->buffer_len) {
                        suota->flash_write_addr += write_count;
                        suota->image_crc = suota_update_crc(suota->image_crc, suota->buffer, write_count);
                        suota->recv_image_len += write_count;
                        if (suota->recv_image_len == get_code_size(&suota->header)) {
...
static bool process_patch_data(suota_service_t *suota, const uint8_t *data, size_t len, size_t *consumed)
{
        ...
        case SUOTA_STATE_W4_HEADER:
                expected_len = sizeof(suota->header);
                break;
        case SUOTA_STATE_W4_HEADER_EXT:
                expected_len = get_exec_location(&suota->header) - sizeof(suota->header) -
        ...
        case SUOTA_STATE_W4_IMAGE_DATA:
                ...
                expected_len = get_code_size(&suota->header) - suota->recv_image_len;
                if (expected_len > SUOTA_BUFFER_SIZE) {
                        expected_len = SUOTA_BUFFER_SIZE;
                }
                break;
...
static nvms_t open_suota_fw_partition(uint32_t* product_header_address)
{
        nvms_t product_header_partition;
                ...
                uint16_t length, const uint8_t *value)
{
        ...
        case SPOTAR_IMG_SPI_FLASH:
                if (!ready_for_suota(suota, conn_idx)) {
                        suota_notify_client_status(suota, conn_idx, SUOTA_SRV_EXIT);
                        return ATT_ERROR_OK;
                }

                if (!suota->buffer) {
                        /* Buffer does NOT exist so allocate it */
                ...
                suota->notified_app_status_mask = 0;
        ...
        case SPOTAR_MEM_SERVICE_EXIT:
                if (suota->conn_idx == conn_idx) {
                        ...
                        /*
                         * If SUOTA_START has not been accompanied with SUOTA_DONE,
                         * notify application with SUOTA_ERROR.
                         */
                        if ((suota->notified_app_status_mask & SUOTA_START) &&
                                !(suota->notified_app_status_mask & SUOTA_DONE)) {
                                suota_notify_app_status(suota, SUOTA_ERROR, 0);
                        }
                }
...
static void handle_disconnected_evt(ble_service_t *svc, const ble_evt_gap_disconnected_t *evt)
{
         ...
         * even if the remote device does not send REBOOT command but disconnects
         * instead
         */
        trigger_reboot();
}
...
ble_service_t *suota_init(const suota_callbacks_t *cb)
{
        ...
        nvms = open_suota_fw_partition(&product_header_address);
        if (!nvms) {
                return NULL;
        }
        ...
        suota->nvms = nvms;
        suota->product_header_address = product_header_address;
        ...
        ble_gatts_add_descriptor(&uuid, ATT_PERM_RW, 2, 0, &suota->suota_status_ccc_h);
