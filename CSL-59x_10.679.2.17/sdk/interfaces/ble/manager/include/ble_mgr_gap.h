...
enum ble_cmd_gap_opcode {
        ...
        BLE_MGR_GAP_ADV_SET_PERMUTATION_CMD,
        ...
        BLE_MGR_GAP_PEER_VERSION_GET_CMD,
        BLE_MGR_GAP_PEER_FEATURES_GET_CMD,
        ...
        BLE_MGR_GAP_PHY_SET_CMD,
#endif /* (dg_configBLE_2MBIT_PHY == 1) */
        BLE_MGR_GAP_TX_POWER_SET_CMD,
        BLE_MGR_GAP_CONN_TX_POWER_SET_CMD,
#ifdef F_PCLE
        BLE_MGR_GAP_LOCAL_TX_POWER_GET_CMD,
        BLE_MGR_GAP_REMOTE_TX_POWER_GET_CMD,
        BLE_MGR_GAP_PATH_LOSS_REPORT_PARAMS_SET_CMD,
        BLE_MGR_GAP_PATH_LOSS_REPORT_EN_CMD,
        BLE_MGR_GAP_TX_PWR_REPORT_EN_CMD,
        BLE_MGR_GAP_RF_PATH_COMPENSATION_SET_CMD,
#endif /* F_PCLE */
...
/** GAP set advertising permutation command message structure */
typedef struct {
        ble_mgr_msg_hdr_t   hdr;
        uint8_t             permutation_index;
} ble_mgr_gap_adv_set_perm_id_cmd_t;

/** GAP set advertising permutation response message structure */
typedef struct {
        ble_mgr_msg_hdr_t   hdr;
        ble_error_t         status;
...
/** GAP get peer version command message structure */
typedef struct {
        ble_mgr_msg_hdr_t   hdr;
        uint16_t            conn_idx;
} ble_mgr_gap_peer_version_get_cmd_t;

/** GAP get peer version response message structure */
typedef struct {
        ble_mgr_msg_hdr_t   hdr;
        ble_error_t         status;
...
/** GAP get peer features command message structure */
typedef struct {
        ble_mgr_msg_hdr_t   hdr;
        uint16_t            conn_idx;
} ble_mgr_gap_peer_features_get_cmd_t;

/** GAP get peer features response message structure */
typedef struct {
        ble_mgr_msg_hdr_t   hdr;
        ble_error_t         status;
...
#if (dg_configBLE_2MBIT_PHY == 1)
/** GAP PHY set command message */
typedef struct {
        ble_mgr_msg_hdr_t   hdr;
        uint16_t            conn_idx;
        uint8_t             tx_phy;
        uint8_t             rx_phy;
} ble_mgr_gap_phy_set_cmd_t;

/** GAP PHY set response message */
typedef struct {
        ble_mgr_msg_hdr_t   hdr;
        ble_error_t         status;
} ble_mgr_gap_phy_set_rsp_t;

void ble_mgr_gap_phy_set_cmd_handler(void *param);
#endif /* (dg_configBLE_2MBIT_PHY == 1) */

/** GAP TX power set command message structure */
typedef struct {
        ble_mgr_msg_hdr_t   hdr;
...
/** GAP TX power set response message structure */
typedef struct {
        ble_mgr_msg_hdr_t   hdr;
        ble_error_t         status;
...
/** GAP connection TX power set command message structure */
typedef struct {
        ble_mgr_msg_hdr_t   hdr;
        uint16_t            conn_idx;
        gap_tx_power_t      tx_power;
...
/** GAP connection TX power set response message structure */
typedef struct {
        ble_mgr_msg_hdr_t   hdr;
        ble_error_t         status;
...
#ifdef F_PCLE
/** GAP local transmit power level get command message structure */
typedef struct {
        ble_mgr_msg_hdr_t   hdr;
        uint16_t            conn_idx;
        uint8_t             phy;
} ble_mgr_gap_local_tx_power_get_cmd_t;
/** GAP local transmit power level get response message structure */
typedef struct {
        ble_mgr_msg_hdr_t   hdr;
        ble_error_t         status;
} ble_mgr_gap_local_tx_power_get_rsp_t;
...
typedef struct {
        ...
        uint16_t            conn_idx;
        uint8_t             phy;
} ble_mgr_gap_rd_rem_tx_pwr_lvl_cmd_t;

/** GAP Read Remote Transmit Power Level response message structure */
typedef struct {
        ble_mgr_msg_hdr_t   hdr;
        ble_error_t         status;
} ble_mgr_gap_rd_rem_tx_pwr_lvl_rsp_t;

void ble_mgr_gap_rd_rem_tx_pwr_lvl_cmd_handler(void *param);

/** GAP Set Path Loss Reporting Parameters command message structure */
typedef struct {
        ble_mgr_msg_hdr_t   hdr;
        uint16_t            conn_idx;
        uint8_t             high_thres;
...
} ble_mgr_gap_set_path_loss_report_params_cmd_t;

/** GAP Set Path Loss Reporting Parameters response message structure */
typedef struct {
        ble_mgr_msg_hdr_t   hdr;
        ble_error_t         status;
} ble_mgr_gap_set_path_loss_report_params_rsp_t;

void ble_mgr_gap_set_path_loss_report_params_cmd_handler(void *param);

/** GAP Set Path Loss Reporting Enable command message structure */
typedef struct {
        ble_mgr_msg_hdr_t   hdr;
        uint16_t            conn_idx;
        uint8_t             enable;
} ble_mgr_gap_set_path_loss_report_en_cmd_t;

/** GAP Set Path Loss Reporting Enable response message structure */
typedef struct {
        ble_mgr_msg_hdr_t   hdr;
        ble_error_t         status;
} ble_mgr_gap_set_path_loss_report_en_rsp_t;

void ble_mgr_gap_set_path_loss_report_en_cmd_handler(void *param);

/** GAP Set Transmit Power Reporting Enable command message structure */
typedef struct {
        ble_mgr_msg_hdr_t   hdr;
        uint16_t            conn_idx;
        uint8_t             loc_en;
        uint8_t             rem_en;
} ble_mgr_gap_set_tx_pwr_report_en_cmd_t;

/** GAP Set Transmit Power Reporting Enable response message structure */
typedef struct {
        ble_mgr_msg_hdr_t   hdr;
        ble_error_t         status;
} ble_mgr_gap_set_tx_pwr_report_en_rsp_t;

void ble_mgr_gap_set_tx_pwr_report_en_cmd_handler(void *param);

/** GAP RF path compensation set command message structure */
typedef struct {
        ble_mgr_msg_hdr_t   hdr;
        int16_t             rf_tx_path_compens; // value in units of 0.1dB
        int16_t             rf_rx_path_compens; // value in units of 0.1dB
} ble_mgr_gap_rf_path_compensation_set_cmd_t;

/** GAP RF path compensation set response message structure */
typedef struct {
        ble_mgr_msg_hdr_t   hdr;
        ble_error_t         status;
} ble_mgr_gap_rf_path_compensation_set_rsp_t;
