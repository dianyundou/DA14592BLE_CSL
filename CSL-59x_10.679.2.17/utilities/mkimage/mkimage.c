...
static void usage(const char* my_name)
{
        fprintf(stderr,
                ...
                "#7 da1459x          - generate DA1459x device image file in secure or non-secure mode\n"
                "\n"
                "\n"
                "Usage case #1:\n"
                ...
                "\n"
                "Usage case #6:\n"
                "mkimage da1469x <in_file> <version_file> [<private_key> <key_idx> <sym_key>\n"
                "              <sym_key_idx> [nonce <nonce_hex>] [rev <cmd>]]\n"
                "\n"
                "parameters:\n"
                "  in_file         input binary file which will be converted to output image\n"
                "  version_file    version file which contains version, timestamp and\n"
                ...
                "Usage case #7:\n"
                "mkimage da1459x <in_file> <version_file> <out_file> <fw_version>\n"
                "               [sign <private_key> <key_idx>] [min_fw <min_fw_version>] [rev <cmd>]\n"
                "\n"
                "parameters:\n"
                "  in_file         input binary file which will be converted to output image\n"
                "  version_file    version file which contains version, timestamp and\n"
                "                  housekeeping information (e.g. sw_version.h)\n"
                "  out_file        output image file\n"
                "  fw_version      fw_version for image. default set to 000000.\n"
                "  sign            use image signature (security)\n"
                "  private_key     private key for signature generation\n"
                "  key_idx         index of public key (stored in device)\n"
                "  min_fw          set minimum version\n"
                "  min_fw_version  version number to be set\n"
                "  rev             keys revocation command\n"
                "  cmd             indexes of the keys which should be revoked. If more than one\n"
                "                  key is passed, parameter should be given in quotation marks\n"
                "                  (look at an example). If index is preceded with 'u' then it\n"
                "                  concerns user data key. Otherwise signature public key will be revoked\n"
                "example:\n"
                "  da1459x pxp_reporter.bin sw_version.h output.img 1\n"
                "  da1459x pxp_reporter.bin sw_version.h output.img 2\n"
                "      sign 8E05FA7509F4D3B8F96B08DEFAA204A9BCEFF67AD28306B6D4A2DBAB3C238DCA 0\n"
                "      min_fw 1 rev \"1 2 s1\"\n"
                );
...
static int generate_asym_key(int argc, const char* argv[])
{
        ...
        for (i = 0; i < num; i++) {
                ...
                /*
                 *   If elliptic curve is ELLIPTIC_CURVE_CURVE25519 convert public key from little to big endian.
                 *   For all other curves drop out the 0x04 prefix
                 */
                if ( ec == MKIMAGE_ELLIPTIC_CURVE_CURVE25519 ) {
                        for (uint32_t i = 0, j=pub_key_len-1; i < (pub_key_len/2); i++,j--) {
                                temp = pub_key[j];
                                pub_key[j] = pub_key[i];
                                pub_key[i] = temp;
                        }
                } else if ( ec != MKIMAGE_ELLIPTIC_CURVE_EDWARDS25519 ) {
                        pub_key_len = pub_key_len - 1;
                        memcpy(pub_key, pub_key + 1, pub_key_len);
                }
...
static int create_da1469x_image(int argc, const char *argv[])
{
        ...
        pub_key_idx = (uint8_t)strtoul(argv[6], &end_ptr, 10);
        ...
        sym_key_idx = (uint8_t)strtoul(argv[8], &end_ptr, 10);
...
static int create_da1459x_image(int argc, const char *argv[])
{
        /*
         * Common mandatory arguments:
         *  0:        application name (e.g mkimage)
         *  1:        option (da1459x), not used in this function but is needed by main to select which create_da14xxx_image function to call.
         *  2:        input file path
         *  3:        version file path
         *  4:        output file path
         *  5:        fw_version
         * Secure-boot image mandatory arguments:
         *  i:        sign (key word) - signing of fw
         *  i + 1:    private key (signature generation)
         *  i + 2:    public key index (signature verification)
         * Optional arguments:
         *  n:        min_ver (key word) - minimum fw version
         *  n + 1:    min_ver - minimum fw version value
         *  m:        rev (key word) - position depends on that the min_fw was passed or not
         *  m + 1:    revocation command - position depends on that the min_fw was passed or not
         *  i, j, n, m are even integers > 5
         */

        uint32_t fw_version = 0;
        bool sign_image = false;
        uint8_t priv_key[32];
        mkimage_security_data_da1459x_t data = { 0 };
        mkimage_da1459x_key_id_t rev_array[100];
        mkimage_device_adm_data_da1459x_t opt_data = { 0 };
        mkimage_status_t lib_status;
        int status = EXIT_FAILURE;
        uint8_t *in_buf = NULL, *ver_buf = NULL, *out_buf = NULL;
        size_t in_size, ver_size, out_size;
        int  out_fd = -1;
        char *end_ptr = NULL;
        char  *arg_dup = NULL, *token = NULL;


        if (argc < ID_TOTAL_ARGS) {
                /* Not enough arguments. */
                usage(argv[0]);
                goto done;
        }
        ...
        /* fw_version part. */
        fw_version = strtoul(argv[ID_FW_VERSION], &end_ptr, 10);
        if (*end_ptr != '\0' || end_ptr == argv[ID_FW_VERSION]) {
                fprintf(stderr, "invalid fw_version\r\n");
                goto done;
        }

        /* Parse optional args. */
        for (int i = ID_TOTAL_ARGS; i < argc; i++) {
                /* We've got additional args -
                 * either min_fw, rev or both,
                 * plus legacy way of passing private key
                 * with keyword "sign". */
                /* sign part */
                if (!strcmp(argv[i], "sign")) {
                        ...
                        if (parse_hex_string(argv[i + 1], priv_key, 32)) {
                                fprintf(stderr, "invalid private key hex-string\r\n");
                                goto done;
                        }

                        /* Warning - copying pointer! */
                        data.priv_key = priv_key;

                        /* public_key_idx part. */
                        /* Get public key index */
                        data.pub_key_idx = (uint8_t)strtoll(argv[i + 2], &end_ptr, 10);

                        if (*end_ptr != '\0' || end_ptr == argv[i + 2]) {
                                fprintf(stderr, "invalid public key index\r\n");
                                goto done;
                        }
                ...
                if (!strcmp(argv[i], "min_ver")) {
                        ...
                        opt_data.minimum_fw_version = strtoul(argv[i + 1], &end_ptr, 10);

                        if (*end_ptr != '\0' || end_ptr == argv[i + 1]) {
                                fprintf(stderr, "invalid minimal version\r\n");
                                goto done;
                        }
                ...
                if (!strcmp(argv[i], "rev")) {
                        ...
                        arg_dup = strdup(argv[i + 1]);
                        token = strtok(arg_dup, " ");
                        int j = 0;

                        for (; token && j < 100; j++) {
                                if (token[0] == 's') {
                                        rev_array[j].type = MKIMAGE_DA1459X_KEY_TYPE_USER_DATA;
                                        ++token;
                                } else {
                                        rev_array[j].type = MKIMAGE_DA1459X_KEY_TYPE_SIGNATURE;
                                }

                                rev_array[j].id = strtoul(token, NULL, 0);
                                if (rev_array[j].id > 7) {
                                        fprintf(stderr, "revocation key id must be in range 0-7\r\n");
                                        goto done;
                                }
                        ...
                        opt_data.key_rev_array = !!j ? rev_array : NULL;

                        i++;
                        continue;
                }
        }
        /* End of parsing optional args. */

        /* Read input file. */
        if (!read_whole_file(argv[ID_INPUT_FILE], O_RDONLY | O_BINARY, &in_size, &in_buf) ||
                !in_size || !in_buf) {
                fprintf(stderr, "cannot read file - %s\r\n", argv[ID_INPUT_FILE]);
                goto done;
        }
        ...
        /* Read version file. */
        if (!read_whole_file(argv[ID_VERSION_FILE], O_RDONLY | O_BINARY, &ver_size, &ver_buf) ||
                !ver_size || !ver_buf) {
                fprintf(stderr, "cannot read file - %s\r\n", argv[ID_VERSION_FILE]);
                status = EXIT_FAILURE;
                goto done;
        }

        /* Create image. */
        lib_status = mkimage_create_da1459x_image(
                in_size, in_buf,
                ver_size, ver_buf,
                fw_version,
                sign_image ? &data : NULL,
                &opt_data, &out_buf, &out_size);

        if (lib_status != MKIMAGE_STATUS_OK) {
                fprintf(stderr, "cannot create da1459x image - %s\r\n",
                        mkimage_status_message(lib_status));
                goto done;
        }

        /* Open output file. */
        out_fd = open(argv[ID_OUTPUT_FILE], O_RDWR | O_CREAT | O_TRUNC | O_BINARY, S_IRUSR | S_IWUSR);
        if (out_fd < 0) {
                fprintf(stderr, "cannot open file - %s\r\n", argv[ID_OUTPUT_FILE]);
                goto done;
        }
...
int main(int argc, const char* argv[])
{
        ...
        else if (!strcmp(argv[1], "da1459x"))
                res = create_da1459x_image(argc, argv);
        else
                usage(argv[0]);
