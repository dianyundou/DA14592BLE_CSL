...
def nvic_cmd():
    cmd = "echo __NVIC_REGISTERS_START__ " + "\\n" + "\n"
    cmd += "nvic-reg \n"
    cmd += "echo __NVIC_REGISTERS_END__ " + "\\n" + "\n"
    return cmd
...
def cmac_cache_hex_dump_cmd(xml_reg_file):
    target_device = get_target_device_from_xml(xml_reg_file)
    if target_device == DEVICE_DA1459X:
        cmd = "echo __CMAC_CACHE_HEX_DUMP_START__ " + "\\n" + "\n"
        cmd += "dmp-cmac_cache_memory \n"
        cmd += "echo __CMAC_CACHE_HEX_DUMP_END__ " + "\\n" + "\n"
    else:
        cmd = ""
    return cmd
...
def create_gdb_cmd_file(elf_file, off_mem_snapshot_file, xml_reg_file, cpu):
    ...
    if is_on_line_debugging:
        file_handle_gdb_cmds.write(arm_registers_cmd() +
                                   ...
                                   cmac_cache_hex_dump_cmd(xml_reg_file) + # This command should be performed last
                                                                           # because it may modify some registers
                                   footer_cmd())
