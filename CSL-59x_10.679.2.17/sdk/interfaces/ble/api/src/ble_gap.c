...
{
        ble_dev_params_t *params = ble_mgr_dev_params_acquire();
        uint8_t dev_name_len = strlen(params->dev_name);

        if (dev_name_len == 0) {
                return BLE_ERROR_INVALID_PARAM;
        }

        if (*length >= BLE_GAP_DEVNAME_LEN_MAX) {
                *length = BLE_GAP_DEVNAME_LEN_MAX + 1;
        }

        if (*length == 0 || *length < dev_name_len) {
                *length = dev_name_len;
        }
...
{
        ble_mgr_gap_adv_set_perm_id_cmd_t *cmd;
        ble_mgr_gap_adv_set_perm_id_rsp_t *rsp;
        ble_error_t ret = BLE_ERROR_FAILED;

        /* Create new command and fill it */
        cmd = alloc_ble_msg(BLE_MGR_GAP_ADV_SET_PERMUTATION_CMD, sizeof(*cmd));
        cmd->permutation_index = permutation_index;

        if (!ble_cmd_execute(cmd, (void **) &rsp, ble_mgr_gap_adv_set_perm_id_cmd_handler)) {
                                 ...
                                 uint8_t *scan_rsp_data_len, uint8_t *scan_rsp_data)
{
        ...
        if (*adv_data_len == 0) {
                ...
                *adv_data_len = (*adv_data_len < params->adv_data_length) ? *adv_data_len : params->adv_data_length;
        }

        if (*scan_rsp_data_len == 0) {
                *scan_rsp_data_len = params->scan_rsp_data_length;
        } else {
                *scan_rsp_data_len = (*scan_rsp_data_len < params->scan_rsp_data_length) ? *scan_rsp_data_len : params->scan_rsp_data_length;
        }

        /* Copy no more than the input buffer length (has to be non-zero) */
        if (*adv_data_len) {
                memcpy(adv_data, params->adv_data, *adv_data_len);
        }

        if (*scan_rsp_data_len) {
                memcpy(scan_rsp_data, params->scan_rsp_data, *scan_rsp_data_len);
        }
...
{
        ble_mgr_gap_peer_version_get_cmd_t *cmd;
        ble_mgr_gap_peer_version_get_rsp_t *rsp;
        ble_error_t ret = BLE_ERROR_FAILED;

        /* Create new command and fill it */
        cmd = alloc_ble_msg(BLE_MGR_GAP_PEER_VERSION_GET_CMD, sizeof(*cmd));
        cmd->conn_idx = conn_idx;

        if (!ble_cmd_execute(cmd, (void **) &rsp, ble_mgr_gap_peer_version_get_cmd_handler)) {
                return BLE_ERROR_BUSY;
        }
...
{
        ble_mgr_gap_peer_features_get_cmd_t *cmd;
        ble_mgr_gap_peer_features_get_rsp_t *rsp;
        ble_error_t ret = BLE_ERROR_FAILED;

        /* Create new command and fill it */
        cmd = alloc_ble_msg(BLE_MGR_GAP_PEER_FEATURES_GET_CMD, sizeof(*cmd));
        cmd->conn_idx = conn_idx;

        if (!ble_cmd_execute(cmd, (void **) &rsp, ble_mgr_gap_peer_features_get_cmd_handler)) {
                return BLE_ERROR_BUSY;
        }
...
ble_error_t ble_gap_get_connected(uint8_t *length, uint16_t **conn_idx)
{
        struct active_conn_idx_data data = {
                .length = 0,
                .conn_idx = OS_MALLOC(dg_configBLE_CONNECTIONS_MAX * sizeof(**conn_idx)),
        };
        ...
        *level = GAP_SEC_LEVEL_1;
        ...
        if (dev->encrypted) {
                *level = GAP_SEC_LEVEL_2;
                ...
                if (dev->mitm) {
                        #if (dg_configBLE_SECURE_CONNECTIONS == 1)
                        if ((dev->secure) && (dev->remote_ltk->key_size == BLE_ENC_KEY_SIZE_MAX)) {
...
__STATIC_INLINE void filter_do_copy(const device_t *dev, struct filter_cb_data *fd)
{
        /*
...
ble_error_t ble_gap_phy_get(uint16_t conn_idx, uint8_t *tx_phy, uint8_t *rx_phy)
{
        ...
        if (conn_idx == BLE_CONN_IDX_INVALID) {
                ...
                if (params) {
                        /* Copy parameters */
                        *tx_phy = params->tx_phy_pref_default;
                        *rx_phy = params->rx_phy_pref_default;
                        ret = BLE_STATUS_OK;
                }
                ...
                if (dev) {
                        /* Get PHY attributes from connected device structure */
                        *tx_phy = dev->tx_phy;
                        *rx_phy = dev->rx_phy;
                        ret = BLE_STATUS_OK;
                } else {
                        /* Device has disconnected or conn_idx is invalid */
...
{
        ble_mgr_gap_phy_set_cmd_t *cmd;
        ble_mgr_gap_phy_set_rsp_t *rsp;
        ble_error_t ret = BLE_ERROR_FAILED;

        /* Create new command and fill it */
        cmd = alloc_ble_msg(BLE_MGR_GAP_PHY_SET_CMD, sizeof(*cmd));
        cmd->conn_idx = conn_idx;
        cmd->tx_phy = tx_phy;
        cmd->rx_phy = rx_phy;

        if (!ble_cmd_execute(cmd, (void **) &rsp, ble_mgr_gap_phy_set_cmd_handler)) {
                return BLE_ERROR_BUSY;
        }
...
{
        ble_mgr_gap_tx_power_set_cmd_t *cmd;
        ...
        /* Create new command and fill it */
        cmd = alloc_ble_msg(BLE_MGR_GAP_TX_POWER_SET_CMD, sizeof(*cmd));
        cmd->air_operation = air_operation;
        cmd->tx_power = tx_power;

        if (!ble_cmd_execute(cmd, (void **) &rsp, ble_mgr_gap_tx_power_set_cmd_handler)) {
                return BLE_ERROR_FAILED;
        }
...
{
        ble_mgr_gap_conn_tx_power_set_cmd_t *cmd;
        ...
        /* Create new command and fill it */
        cmd = alloc_ble_msg(BLE_MGR_GAP_CONN_TX_POWER_SET_CMD, sizeof(*cmd));
        cmd->conn_idx = conn_idx;
        cmd->tx_power = tx_power;

        if (!ble_cmd_execute(cmd, (void **) &rsp, ble_mgr_gap_conn_tx_power_set_cmd_handler)) {
                return BLE_ERROR_FAILED;
        }
...
{
        ble_mgr_gap_local_tx_power_get_cmd_t *cmd;
        ble_mgr_gap_local_tx_power_get_rsp_t *rsp;
        ble_error_t ret = BLE_ERROR_FAILED;

        /* Create new command and fill it */
        cmd = alloc_ble_msg(BLE_MGR_GAP_LOCAL_TX_POWER_GET_CMD, sizeof(*cmd));
        cmd->conn_idx = conn_idx;
        cmd->phy = phy;

        if (!ble_cmd_execute(cmd, (void **) &rsp, ble_mgr_gap_local_tx_power_get_cmd_handler)) {
                return BLE_ERROR_FAILED;
        }
...
{
        ble_mgr_gap_rd_rem_tx_pwr_lvl_cmd_t *cmd;
        ble_mgr_gap_rd_rem_tx_pwr_lvl_rsp_t *rsp;
        ble_error_t ret = BLE_ERROR_FAILED;

        /* Create new command and fill it */
        cmd = alloc_ble_msg(BLE_MGR_GAP_REMOTE_TX_POWER_GET_CMD, sizeof(*cmd));
        cmd->conn_idx = conn_idx;
        cmd->phy = phy;

        if (!ble_cmd_execute(cmd, (void **) &rsp, ble_mgr_gap_rd_rem_tx_pwr_lvl_cmd_handler)) {
                return BLE_ERROR_FAILED;
        }
...
{
        ble_mgr_gap_set_path_loss_report_params_cmd_t *cmd;
        ble_mgr_gap_set_path_loss_report_params_rsp_t *rsp;
        ble_error_t ret = BLE_ERROR_FAILED;

        /* Create new command and fill it */
        cmd = alloc_ble_msg(BLE_MGR_GAP_PATH_LOSS_REPORT_PARAMS_SET_CMD, sizeof(*cmd));
        cmd->conn_idx = conn_idx;
        cmd->high_thres = high_thres;
        cmd->high_hyst = high_hyst;
        cmd->low_thres = low_thres;
        cmd->low_hyst = low_hyst;
        cmd->min_time_spent = min_time_spent;

        if (!ble_cmd_execute(cmd, (void **) &rsp, ble_mgr_gap_set_path_loss_report_params_cmd_handler)) {
                return BLE_ERROR_FAILED;
        }
...
{
        ble_mgr_gap_set_path_loss_report_en_cmd_t *cmd;
        ble_mgr_gap_set_path_loss_report_en_rsp_t *rsp;
        ble_error_t ret = BLE_ERROR_FAILED;

        /* Create new command and fill it */
        cmd = alloc_ble_msg(BLE_MGR_GAP_PATH_LOSS_REPORT_EN_CMD, sizeof(*cmd));
        cmd->conn_idx = conn_idx;
        cmd->enable = enable;

        if (!ble_cmd_execute(cmd, (void **) &rsp, ble_mgr_gap_set_path_loss_report_en_cmd_handler)) {
                return BLE_ERROR_FAILED;
        }
...
{
        ble_mgr_gap_set_tx_pwr_report_en_cmd_t *cmd;
        ble_mgr_gap_set_tx_pwr_report_en_rsp_t *rsp;
        ble_error_t ret = BLE_ERROR_FAILED;

        /* Create new command and fill it */
        cmd = alloc_ble_msg(BLE_MGR_GAP_TX_PWR_REPORT_EN_CMD, sizeof(*cmd));
        cmd->conn_idx = conn_idx;
        cmd->loc_en = local_enable;
        cmd->rem_en = remote_enable;

        if (!ble_cmd_execute(cmd, (void **) &rsp, ble_mgr_gap_set_tx_pwr_report_en_cmd_handler)) {
                return BLE_ERROR_FAILED;
        }
...
{
        ble_mgr_gap_rf_path_compensation_set_cmd_t *cmd;
        ble_mgr_gap_rf_path_compensation_set_rsp_t *rsp;
        ble_error_t ret = BLE_ERROR_FAILED;

        /* Create new command and fill it */
        cmd = alloc_ble_msg(BLE_MGR_GAP_RF_PATH_COMPENSATION_SET_CMD, sizeof(*cmd));
        cmd->rf_tx_path_compens = rf_tx_path_compens;
        cmd->rf_rx_path_compens = rf_rx_path_compens;

        if (!ble_cmd_execute(cmd, (void **) &rsp, ble_mgr_gap_rf_path_compensation_set_cmd_handler)) {
                return BLE_ERROR_FAILED;
        }
