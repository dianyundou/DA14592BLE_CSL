...
static int send_initial_code(const uint8_t *buf, uint32_t size)
{
        ...
        if (size < max_bootloader_size / 2 ) {
                ...
                uint8_t extended_mode_header[6] = { SOH, 0x0, 0x0, (uint8_t) size, (uint8_t) (size >> 8), 0x1 };
...
int protocol_cmd_erase_qspi(uint32_t address, size_t size)
{
        ...
        if (!strncmp(chip_rev, CHIP_REV_590XX, CHIP_REV_STRLEN)) {
                /*
                 * Consider eFLASH and QSPI FLASH erase timings: erasing eFLASH takes more time -
                 * about 263ms for each page. Add 200ms to be sure.
                 */
                err = wait_for_ack(200 + 263 * (size / EFLASH_PAGE_SIZE + 1));
        } else {
...
int protocol_cmd_chip_erase_qspi(void)
{
        ...
        /* it takes at most 50s for whole flash memory to be erased */
        err = wait_for_ack(50000);
...
int protocol_cmd_write_otp(uint32_t address, const uint32_t *buf, uint32_t len)
{
        ...
        if (!strncmp(chip_rev, CHIP_REV_590XX, CHIP_REV_STRLEN)) {
                /*
                 * DA1459x handles OTP write in the same way as eFLASH write - the timeout should
                 * be the same.
                 */
                err = wait_for_ack(EXECUTION_TIMEOUT);
        } else {
...
int protocol_cmd_chip_erase_eflash(void)
{
        ...
        err = send_cmd_header(CMD_CHIP_ERASE_EFLASH, 0);
        if (err < 0) {
                return err;
        }
...
int protocol_cmd_read_eflash(uint32_t address, uint8_t *buf, uint32_t len)
{
        ...
        err = send_cmd_header(CMD_READ_EFLASH, sizeof(header_buf));
        if (err < 0) {
                return err;
        }
...
int protocol_cmd_direct_write_to_eflash(const uint8_t *buf, size_t size, uint32_t addr, bool verify)
{
        ...
        err = send_cmd_header(CMD_DIRECT_WRITE_TO_EFLASH, sizeof(header_buf) + size);
        if (err < 0) {
                return err;
        }
...
int protocol_cmd_copy_to_eflash(uint32_t src_address, size_t size, uint32_t dst_address)
{
        ...
        err = send_cmd_header(CMD_COPY_EFLASH, sizeof(header_buf));
        if (err < 0) {
                return err;
        }
...
int protocol_cmd_erase_eflash(uint32_t address, size_t size)
{
        ...
        err = send_cmd_header(CMD_ERASE_EFLASH, sizeof(header_buf));
        if (err < 0) {
                return err;
        }
        ...
        /*
         * Consider eFLASH erase timings: erasing eFLASH takes more time -
         * about 263ms for each page. Add 200ms to be sure.
         */
        err = wait_for_ack(200 + 263 * (size / EFLASH_PAGE_SIZE + 1));
...
int protocol_cmd_is_empty_eflash(unsigned int size, unsigned int start_address, int *ret_number)
{
        ...
        err =  send_cmd_header(CMD_IS_EMPTY_EFLASH, sizeof(header_buf));
        if (err < 0) {
                return err;
        }
...
connection_status_t protocol_verify_connection(void)
{
        ...
        // hello message is send with 1s delay. Wait 2s to be sure that it will be received.
