...
from eflash.program_eflash_config import ProgramEFLASHConfig, program_eflash_config, ProductId


PRODUCT_HEADER_SIZE = 0x800
FLASH_BASE_ADDRESS = 0x0
MIN_FW_ADDRESS = 0x2000
PRODUCT_HEADER_PRIMARY_ADDRESS = 0x1000
PRODUCT_HEADER_SECONDARY_ADDRESS = PRODUCT_HEADER_PRIMARY_ADDRESS + PRODUCT_HEADER_SIZE #0x1800
...
def make_product_header(active_fw_image_address, update_fw_image_address, flash_burstcmda_reg_value,
    ...
    if flash_write_config_command is not None:
...
def program_product_header(qspi_config, cli_programmer, serial):
    try:
        ...
        cli_programmer.write_eflash(PRODUCT_HEADER_SECONDARY_ADDRESS, product_header_bin,
                                  PRODUCT_HEADER_SIZE, serial_port=serial)
        cli_programmer.write_eflash(PRODUCT_HEADER_PRIMARY_ADDRESS, product_header_bin,
                                  PRODUCT_HEADER_SIZE, serial_port=serial)
    finally:
        os.remove(product_header_bin)


def handle_eflash_config(header_config_path, flash_config_path):
    eflash_config = ProgramEFLASHConfig(config_path=header_config_path)

    if eflash_config.is_valid():
        if flash_config_path is not None:
            if not eflash_config.check_flash_config(flash_config_path):
                if ui.ask(text="Current flash config isn't listed on supported flash config list "
                    ...
                    program_eflash_config(header_config_path=header_config_path,
    ...
    if not eflash_config.is_valid():
        raise RuntimeError("eFlash config is invalid.\nExecution aborted.\n")

    return eflash_config


def program_eflash(image_file, cli_programmer, serial=None, header_config=None, flash_config=None):
    handle_eflash_config(header_config, flash_config)

    cli_programmer.write_eflash(FLASH_BASE_ADDRESS, image_file, serial_port=serial, silent=False)


def parse_args():
    parser = ScriptArgumentsParser()
    parser.add_interface_args()
    parser.add_argument('image_file', type=str, help='path to image')
    parser.add_config_args()
    args = parser.parse_args()

    cli, serial = parser.get_interface_from_args(args)
    return args.image_file, cli, serial


if __name__ == '__main__':
    ui.print_header('PROGRAM EFLASH')
    run_script(program_eflash, parse_args)
