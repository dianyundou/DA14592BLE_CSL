...
/**
 * \brief Define the maximum number of connections
 *
 * Define what will be the maximum number of connections. The minimum supported number by the BLE
 * stack is 1 connection and the maximum is 8 connections. The default value for the DA1459X device
 * family is 2 connections and for the rest is 8 connections.
 *
 * \bsp_default_note{\bsp_config_option_app, \bsp_config_option_expert_only}
 */
#ifndef dg_configBLE_CONNECTIONS_MAX
#define dg_configBLE_CONNECTIONS_MAX         (2)
#elif ( ( dg_configBLE_CONNECTIONS_MAX > CFG_CON ) || ( dg_configBLE_CONNECTIONS_MAX < 1 ) )
#error "dg_configBLE_CONNECTIONS_MAX must be between 1 and 8"
#endif
...
#else
        #if   ( (dg_configBLE_INITIAL_TX_POWER < GAP_TX_POWER_MIN) || \
                (dg_configBLE_INITIAL_TX_POWER > GAP_TX_POWER_MAX) )
        #error "The TX power level value must be one of @gap_tx_power_t values."
        #endif
#endif

/**
 * \brief Set Radio Power mode
 *
 *  - LOW_POWER_ALL_PHYS
 *  - HIGH_PERFORMANCE_ALL_PHYS
 *
 */
#ifndef dg_configDEFAULT_RADIO_OP_MODE
...
#define defaultBLE_ADVERTISE_DATA            { 0x0D, GAP_DATA_TYPE_LOCAL_NAME, \
                                               'R', 'e', 'n', 'e', 's', 'a', 's', ' ', 'B', 'L', 'E' }
#endif
...
#define defaultBLE_DIS_MODEL_NUMBER          "Renesas BLE"
#endif
...
#define defaultBLE_DIS_SYSTEM_ID_OUI         { 0x80, 0xEA, 0xCA }
#endif
