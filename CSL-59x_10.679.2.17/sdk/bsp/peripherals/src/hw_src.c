...
#define HW_SRC_CLEAR_FLOW_ERROR(id, io) \
        while (SRCBA(id)->SRC1_CTRL_REG & (REG_MSK(SRC1, SRC1_CTRL_REG, SRC_##io##_OVFLOW) | REG_MSK(SRC1, SRC1_CTRL_REG, SRC_##io##_UNFLOW))) { \
...
void hw_src_init(HW_SRC_ID id, hw_src_config_t *config)
{
        ...
        if (config->in_sample_rate > 0) {
                ...
                HW_SRC_REG_SETF(config->id, SRC1, CTRL_REG, SRC_IN_DS, iir_setting);
        }
        ...
        if (config->out_sample_rate > 0) {
                ...
                HW_SRC_REG_SETF(config->id, SRC1, CTRL_REG, SRC_OUT_US, iir_setting);
        }
...
HW_SRC_FLOW_STATUS hw_src_get_flow_status(HW_SRC_ID id, HW_SRC_DIRECTION direction)
{
        ...
        switch (direction) {
        case HW_SRC_IN:
                if (src1_ctrl_reg & REG_MSK(SRC1, SRC1_CTRL_REG, SRC_IN_OVFLOW)) {
                        status |= HW_SRC_FLOW_OVER;
                }
                if (src1_ctrl_reg & REG_MSK(SRC1, SRC1_CTRL_REG, SRC_IN_UNFLOW)) {
                        status |= HW_SRC_FLOW_UNDER;
                }
                HW_SRC_CLEAR_FLOW_ERROR(id, IN);
                break;
        case HW_SRC_OUT:
                if (src1_ctrl_reg & REG_MSK(SRC1, SRC1_CTRL_REG, SRC_OUT_OVFLOW)) {
                        status |= HW_SRC_FLOW_OVER;
                }
                if (src1_ctrl_reg & REG_MSK(SRC1, SRC1_CTRL_REG, SRC_OUT_UNFLOW)) {
                        status |= HW_SRC_FLOW_UNDER;
                }
