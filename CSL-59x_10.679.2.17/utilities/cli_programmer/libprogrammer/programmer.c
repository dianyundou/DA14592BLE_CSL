...
/*
 * 590 specific chip registers
 */
static const prog_chip_regs_t chip_590_regs = {
        /* .sys_ctrl_reg = */                   0x50000024,
        /* .chip_id1_reg = */                   0x50050200,
        /* .chip_id2_reg = */                   0x50050204,
        /* .chip_id3_reg = */                   0x50050208,
        /* .chip_revision_reg = */              0x50050214,
        /* .chip_test1_reg = */                 0x500502F8,
        /* .otp_start_address = */              0x0,
        /* .otp_size = */                       0x400 * 16, /* Total size of all write-protectable sectors of eFLASH */
        /* .otp_header_chip_id = */             0x0,
        /* .otp_header_pos_pack_info = */       0x0,
        /* .memory_sysram_base = */             0x20000000,
        /* .memory_sysram_end = */              0x20018000,
        /* .memory_qspif_base = */              0x16000000,
        /* .memory_qspif_end = */               0x18000000,
        /* .magic_value1_reg = */               0x20010000,
        /* .magic_value2_reg = */               0x20010004,
        /* .magic_value3_reg = */               0x20010008,
        /* .magic_value4_reg = */               0x2001000C,
        /* .swd_reset_reg = */                  0x1A0C0050,
        /* .virtual_buf_mask = */               0xFFFE0000,
        /* .sys_ctrl_reg_val = */               0x00C3,
};
...
/*
 * 590 specific memory sizes
 */
static const prog_memory_sizes_t chip_590_mem_sizes = {
        /* .ram_size = */                       0x400 * 96,     /* 96 kB */
        /* .otp_size = */                       0x400 * 258,    /* N/A */
        /* .qspi_size = */                      0x100000 * 32,  /* 258 kB */
        /* .eflash_size = */                    0x400 * 258,    /* 258 kB */
};
...
/* If this is set to true, then prog_write_otp() works in 'block' mode, meaning that
 * a set of different (nonadjacent) areas can be programmed with a single call. This
 * is done by using 'empty' words (i.e. 0xFFFFFFFF in case of write_otp_32() and 0 in
 * case of write_otp_64()) in order to designate any locations that are to be skipped
 * (i.e. not programmed). */
...
typedef struct {
         ...
         * \return 0 on success, error code on failure
         *
         * \note: In case of DA1459x, the OTP is emulated by allowing certain regions
         *        in eFLASH to be write/erase protected.
         *
         */
         ...
         *
         * This function force uploads second stage bootloader.
         *
         * \returns 0 on success, negative value with error code on failure
         *
         */
        ...
        /**
         * \brief Apply chip erase on eFLASH
         *
         ...
         *
         */
        int (*cmd_chip_erase_eflash)(void);

        /**
         * \brief Read data from eFLASH
         *
         * \param [in] address offset in eflash
         * \param [out] buf buffer for data
         * \param [in] len length of buffer
         *
         * \return 0 on success, error code on failure
         *
         */
        int (*cmd_read_eflash)(uint32_t address, uint8_t *buf, uint32_t len);
         ...
         *
         */
        int (*cmd_direct_write_to_eflash)(const uint8_t *buf, size_t size, uint32_t addr, bool verify);

        /**
         * \brief Copy memory to eFLASH
         *
         * This function programs eFLASH memory with data already present in the device RAM.
         *
         * \param [in] src_address address in the device RAM
         * \param [in] size size of memory to copy
         * \param [in] dst_address offset in flash to write to
         ...
         *
         */
        int (*cmd_copy_to_eflash)(uint32_t src_address, size_t size, uint32_t dst_address);

        /**
         * \brief Erase the eFLASH region
         *
         * This function erases the flash at a specified offset.
         *
         * \param [in] address address in flash to start erase
         ...
         *
         */
        int (*cmd_erase_eflash)(uint32_t address, size_t size);
         ...
         *
         */
        int (*cmd_is_empty_eflash)(unsigned int size, unsigned int start_address, int *ret_number);
...
static const target_interface_t target_serial = {
        ...
        /* .cmd_chip_erase_eflash = */          protocol_cmd_chip_erase_eflash,
        /* .cmd_read_eflash = */                protocol_cmd_read_eflash,
        /* .cmd_direct_write_to_eflash = */     protocol_cmd_direct_write_to_eflash,
        /* .cmd_copy_to_eflash = */             protocol_cmd_copy_to_eflash,
        /* .cmd_erase_eflash = */               protocol_cmd_erase_eflash,
        /* .cmd_is_empty_eflash = */            protocol_cmd_is_empty_eflash,
...
static const target_interface_t target_gdb_server = {
        ...
        /* .cmd_chip_erase_eflash = */          gdb_server_cmd_chip_erase_eflash,
        /* .cmd_read_eflash = */                gdb_server_cmd_read_eflash,
        /* .cmd_direct_write_to_eflash = */     gdb_server_cmd_direct_write_to_eflash,
        /* .cmd_copy_to_eflash = */             gdb_server_cmd_copy_to_eflash,
        /* .cmd_erase_eflash = */               gdb_server_cmd_erase_eflash,
        /* .cmd_is_empty_eflash = */            gdb_server_cmd_is_empty_eflash,
...
int prog_write_file_to_otp(uint32_t otp_address, const char *file_name, uint32_t size)
{
        ...
        if (strncmp(prog_chip_rev, CHIP_REV_590XX, CHIP_REV_STRLEN) == 0) {
                return ERR_CMD_UNSUPPORTED;
        } else if (strncmp(prog_chip_rev, CHIP_REV_690AB, CHIP_REV_STRLEN) == 0) {
...
int write_otp_32(uint32_t address, const uint32_t *buf, uint32_t len)
{
        ...
        uint32_t buf_copy[len];
        const uint32_t *buf_ptr = buf;

        if (strncmp(prog_chip_rev, CHIP_REV_590XX, CHIP_REV_STRLEN) == 0) {
                otp_start_address = chip_590_regs.otp_start_address;

                if (block_otp_write) {
                        /*
                         * In DA1459x, OTP is emulated by allowing certain regions in eFLASH to be write/erase protected.
                         * If the specified address points to a region that is currently not protected and non empty,
                         * Bootloader may need to erase it before writing, so any specified 0xFFFFFFFF (i.e. empty)
                         * words will overwrite the current content. In order to avoid this, we first need to
                         * read the existing data and update the write data buffer by replacing any empty words with
                         * the ones currently existing at the corresponding locations in the eflash. And in order to
                         * avoid modifying the (given) write buffer, we copy it to temporary a buffer, make the
                         * replacements in it and and actually attempt to write the contents of that one instead.
                         */
                        memcpy(buf_copy, buf, len * sizeof(buf[0]));
                        buf_ptr = buf_copy;
                }
        } else {
        ...
        /* Normally, if at least one cell is already written (with a word different than
         * the one specified), there is no point to proceed, since any attempt to overwrite
         * the cell (with the specified word) will fail.
         * In case of DA1459x though, if otp_addr_not_empty = true, it doesn't mean necessarily
         * that the data cannot be written. Overwriting will only be impossible if write/erase
         * protection for the respective area has been enabled. Thus, let's proceed with the
         * write attempt, and, if the area is protected, we will fail (and return error) later.
         */
        if (strncmp(prog_chip_rev, CHIP_REV_590XX, CHIP_REV_STRLEN) != 0) {
                if (otp_addr_not_empty) {
                        prog_print_err("Otp address 0x%x not empty...\n",
        ...
        /*Write to OTP address*/
        if ((err = target->cmd_write_otp(address, buf_ptr, len)) != 0 ) {
                free(read_buf);
                return  err;
        }
        ...
        if (block_otp_write) {
                /* Now, wherever buf contains 'empty' words (i.e. equal to 0xFFFFFFFF), replace also the
                 * corresponding words in read_buf with 'empty' ones. */
                for (i = 0; i < len; i++) {
                        if (buf[i] == 0xFFFFFFFF) {
...
int prog_write_otp(uint32_t address, const uint32_t *buf, uint32_t len)
{
        if ((strncmp(prog_chip_rev, CHIP_REV_690AB, CHIP_REV_STRLEN) == 0) ||
            (strncmp(prog_chip_rev, CHIP_REV_590XX, CHIP_REV_STRLEN) == 0)) {
...
int prog_chip_erase_eflash(void)
{
        if (strncmp(prog_chip_rev, CHIP_REV_590XX, CHIP_REV_STRLEN) != 0) {
                return ERR_CMD_UNSUPPORTED;
        }

        return target->cmd_chip_erase_eflash();
}

int prog_read_eflash(uint32_t address, uint8_t *buf, uint32_t len)
{
        uint32_t offset = 0;
        int err = 0;
        ...
        while (err == 0 && offset < len) {
                ...
                err = target->cmd_read_eflash(address + offset, buf + offset, chunk_size);
                offset += chunk_size;
        }
        return err;
}

int prog_read_eflash_to_file(uint32_t address, const char *fname, uint32_t len)
{
        int err = 0;
        uint8_t *buf = NULL;
        FILE *f = NULL;
        ...
        err = prog_read_eflash(address, buf, len);
        if (err < 0) {
                goto end;
        }
...
int prog_write_to_eflash(uint32_t flash_address, const uint8_t *buf, uint32_t size)
{
        int err = 0;
        uint32_t offset = 0;
        uint8_t retry_cnt = 0;
        ...
        while (offset < size) {
                ...
                if (retry_cnt > 10) {
                        err = ERR_PROG_EFLASH_WRITE;
                        prog_print_err("Write to eFLASH failed. Abort. \n");
                        goto done;
                }
                ...
                err = target->cmd_direct_write_to_eflash(buf + offset, chunk_size,
                                                                (flash_address + offset), true);
                if (err != 0) {
                        prog_print_log("Verify writing to eFlASH address 0x%x failed. Retrying ...\n",
                                                                        flash_address + offset);
                        retry_cnt++;
                        continue;
                }
...
int prog_write_file_to_eflash(uint32_t flash_address, const char *file_name, uint32_t size)
{
        int err = 0;
        uint8_t *buf = NULL;
        FILE *f = NULL;
        ...
        err = prog_write_to_eflash(flash_address, buf, size);
end:
        if (f) {
                fclose(f);
        }
        ...
        if (!err && flash_binary) {
                 ...
                 * the rest of bytes in this sector will not be erased.
                 */
                err = prog_write_to_eflash(0, (const uint8_t *) "qQ", 2);
        }

        return err;
}

int prog_copy_to_eflash(uint32_t mem_address, uint32_t flash_address, uint32_t size)
{
        return target->cmd_copy_to_eflash(mem_address, size, flash_address);
}

int prog_erase_eflash(uint32_t flashAddress, uint32_t size)
{
        return target->cmd_erase_eflash(flashAddress, size);
}

int prog_is_empty_eflash(unsigned int size, unsigned int start_address, int *ret_number)
{
        return target->cmd_is_empty_eflash(size, start_address, ret_number);
}
...
int prog_map_product_id_to_chip_rev(const char *product_id, char *chip_rev)
{
        ...
        if ((strcmp(product_id, "DA1459x-00") == 0)) {
                if (chip_rev) {
                        strncpy(chip_rev, CHIP_REV_590XX, CHIP_REV_STRLEN);
                }
                return 0;
        }
...
int prog_chip_rev_valid(const char *chip_rev)
{
        ...
        if ((strcmp(chip_rev, CHIP_REV_680AH) == 0) || (strcmp(chip_rev, CHIP_REV_680BB) == 0) ||
                (strcmp(chip_rev, CHIP_REV_690AB) == 0) || (strcmp(chip_rev, CHIP_REV_590XX) == 0)) {
                return 0;
        }
...
int prog_get_chip_regs(const char *chip_rev, const prog_chip_regs_t **regs)
{
        ...
        if (strcmp(chip_rev, CHIP_REV_590XX) == 0) {
                *regs = &chip_590_regs;
