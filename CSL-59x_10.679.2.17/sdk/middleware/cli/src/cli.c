...
/* cli task handle */
__RETAINED static OS_TASK task;

/* Registered client data */
__RETAINED static client_t reg_client;
...
static OS_TASK_FUNCTION(cli_task_func, param)
{
...
void cli_init(void)
{
        ...
        OS_TASK_CREATE("cli", cli_task_func, NULL, OS_MINIMAL_TASK_STACK_SIZE,
...
void cli_handle_notified(cli_t cli)
{
        ...
        if (OS_QUEUE_MESSAGES_WAITING(reg_client.queue)) {
                OS_TASK_NOTIFY(reg_client.task, reg_client.notif_mask, OS_NOTIFY_SET_BITS);
        }
}
