...
void hw_spi_init_clk_reg(const HW_SPI_ID id, bool select_divn)
{
        ...
        if (select_divn) {
                ...
                CRG_COM->SET_CLK_COM_REG = CRG_COM_RESET_CLK_COM_REG_SPI_CLK_SEL_Msk;
        }
...
void hw_spi_init(HW_SPI_ID id, const hw_spi_config_t *cfg)
{
        ...
        if (cfg->smn_role == HW_SPI_MODE_MASTER) {
                hw_spi_set_clock_reg_clk_div(id, cfg->xtal_freq);
        }

        /*
         * Disable SPI / Reset FIFO before applying changes
         */
        uint32_t ctrl = hw_spi_get_ctrl_reg(id);
        HW_SPI_REG_SET_FIELD(SPI_CTRL, SPI_EN, ctrl, 0);
        HW_SPI_REG_SET_FIELD(SPI_CTRL, SPI_FIFO_RESET, ctrl, 1);
        ...
        // Set swap bytes
        HW_SPI_REG_SET_FIELD(SPI_CTRL, SPI_SWAP_BYTES, ctrl, cfg->swap_bytes ? 1 : 0);
        // Enable Tx and/or Rx paths according to FIFO mode
        ctrl = (ctrl & ~HW_SPI_FIFO_RX_TX) | cfg->fifo_mode;
        // Capture current for Slave / Capture next for Master
        HW_SPI_REG_SET_FIELD(SPI_CTRL, SPI_CAPTURE_AT_NEXT_EDGE, ctrl,
                (cfg->smn_role == HW_SPI_MODE_SLAVE) ? HW_SPI_MASTER_EDGE_CAPTURE_CURRENT: HW_SPI_MASTER_EDGE_CAPTURE_NEXT);
        // Enable SPI block (if requested)
        HW_SPI_REG_SET_FIELD(SPI_CTRL, SPI_EN, ctrl, cfg->disabled ? 0 : 1);
        // Apply
        hw_spi_set_ctrl_reg(id, ctrl);
         ...
         */
        uint32_t config = hw_spi_get_config_reg(id);
        // Set SPI Word length
        HW_SPI_REG_SET_FIELD(SPI_CONFIG, SPI_WORD_LENGTH, config, cfg->word_mode);
        // Set SPI Mode (CPOL, CPHA)
        HW_SPI_REG_SET_FIELD(SPI_CONFIG, SPI_MODE, config, cfg->cpol_cpha_mode);
        // Set SPI Master/Slave mode
        HW_SPI_REG_SET_FIELD(SPI_CONFIG, SPI_SLAVE_EN, config, cfg->smn_role);
        // Apply
        hw_spi_set_config_reg(id, config);
        ...
        /*
         * Setup CS - GPIO is controlled in application layer
         */
        hw_spi_set_cs_pad(id, &(cfg->cs_pad), cfg->spi_cs, false);
...
void hw_spi_set_cs_pad(HW_SPI_ID id, const SPI_Pad *pad, HW_SPI_CS_MODE cs_mode, bool validate)
{
        ...
        if (validate) {
                if (spid->smn_role == HW_SPI_MODE_SLAVE) {
                        HW_GPIO_MODE mode;
                        HW_GPIO_FUNC func;
                        hw_gpio_get_pin_function(pad->port, pad->pin, &mode, &func);
                        ASSERT_WARNING(mode == HW_GPIO_MODE_INPUT);
                        ASSERT_WARNING(func == HW_GPIO_FUNC_SPI_EN);
                        ASSERT_WARNING(cs_mode == HW_SPI_CS_0);
                } else {
                        HW_GPIO_MODE mode;
                        HW_GPIO_FUNC func, valid_func;
                        hw_gpio_get_pin_function(pad->port, pad->pin, &mode, &func);
                        switch (cs_mode) {
                        case HW_SPI_CS_0:
                                valid_func = HW_GPIO_FUNC_SPI_EN;
                                break;
                        case HW_SPI_CS_1:
                                valid_func = HW_GPIO_FUNC_SPI_EN2;
                                break;
                        case HW_SPI_CS_GPIO:
                                valid_func = HW_GPIO_FUNC_GPIO;
                                break;
                        default:
                                valid_func = HW_GPIO_FUNC_LAST; // invalid
                                ASSERT_WARNING(0);
                                break;
                        }
                        ASSERT_WARNING(mode == HW_GPIO_MODE_OUTPUT); // Rule out an open-drain pin
                        ASSERT_WARNING(func == valid_func);
                        ASSERT_WARNING(cs_mode != HW_SPI_CS_NONE);
                }
        }
        /* Update SPI_Data */
        spid->cs_pad.port = pad->port;
        spid->cs_pad.pin = pad->pin;
        spid->cs_mode = cs_mode;
}

void hw_spi_set_cs_low(const HW_SPI_ID id)
{
        SPI_Data *spid = SPIDATA(id);
...
void hw_spi_set_cs_high(const HW_SPI_ID id)
{
        SPI_Data *spid = SPIDATA(id);
...
{
        uint32_t ctrl = hw_spi_get_ctrl_reg(id);
        hw_spi_set_ctrl_reg(id, (ctrl & ~HW_SPI_FIFO_RX_TX) | mode);
}

HW_SPI_FIFO hw_spi_get_fifo_mode(HW_SPI_ID id)
{
        uint32_t ctrl = hw_spi_get_ctrl_reg(id);
        return (ctrl & HW_SPI_FIFO_RX_TX);
}
...
{
        SPI_Data *spid = SPIDATA(id);
        /* Assert a valid pair of channels */
        if (((rx_channel & 0x1) == 1) || (rx_channel + 1 >= HW_DMA_CHANNEL_INVALID)) {
                ASSERT_WARNING(0);
                /* Proceed with fallback scenario, without DMA */
                spid->use_dma = 0;
                spid->rx_dma.channel_number = HW_DMA_CHANNEL_INVALID;
                spid->tx_dma.channel_number = HW_DMA_CHANNEL_INVALID;
                return;
        }

        uint16_t wordsize = hw_spi_get_memory_word_size(id);
        if (wordsize > 4 || wordsize == 3) {
                /* Proceed with fallback scenario, without DMA */
                spid->use_dma = 0;
                spid->rx_dma.channel_number = HW_DMA_CHANNEL_INVALID;
                spid->tx_dma.channel_number = HW_DMA_CHANNEL_INVALID;
        ...
        HW_DMA_PRIO rx_priority;
        HW_DMA_PRIO tx_priority;
        if (prio && prio->use_prio) {
                rx_priority = prio->rx_prio;
                tx_priority = prio->tx_prio;
        } else {
                rx_priority = HW_SPI_DEFAULT_DMA_RX_PRIO;
                tx_priority = HW_SPI_DEFAULT_DMA_TX_PRIO;
        }

        spid->use_dma = 1;

        spid->rx_dma.channel_number = rx_channel;
        ...
        spid->rx_dma.dma_req_mux = HW_DMA_TRIG_SPI_RXTX;
        ...
        spid->tx_dma.channel_number = rx_channel + 1;
        ...
        spid->tx_dma.dma_req_mux = HW_DMA_TRIG_SPI_RXTX;
...
static uint16_t hw_spi_transfer_read(HW_SPI_ID id)
{
        ...
        bool is_slave = hw_spi_is_slave(id);
...
static uint16_t hw_spi_transfer(HW_SPI_ID id)
{
        ...
        uint32_t wordsize = hw_spi_get_memory_word_size(id);
        uint32_t txbuf_inc = spid->tx_buffer != NULL ? wordsize : 0;
        uint32_t rxbuf_inc = spid->rx_buffer != NULL ? wordsize : 0;
                                                             ...
                                                             hw_spi_tx_callback cb, void *user_data)
{
        ...
        ASSERT_WARNING(spid->tx_tl <= hw_spi_get_fifo_depth_in_bytes(id));
        ASSERT_WARNING(spid->rx_tl <= hw_spi_get_fifo_depth_in_bytes(id));

        // Clear Tx, Rx and DMA enable paths in Control Register
        hw_spi_set_ctrl_reg_clear_enable(id);

        uint32_t fifo_config = hw_spi_get_fifo_config_reg(id);
        HW_SPI_REG_SET_FIELD(SPI_FIFO_CONFIG, SPI_TX_TL, fifo_config, spid->tx_tl);
        HW_SPI_REG_SET_FIELD(SPI_FIFO_CONFIG, SPI_RX_TL, fifo_config, spid->rx_tl);
        hw_spi_set_fifo_config_reg(id, fifo_config);

        uint32_t ctrl = hw_spi_get_ctrl_reg(id);
        HW_SPI_REG_SET_FIELD(SPI_CTRL, SPI_TX_EN, ctrl, 1);
        HW_SPI_REG_SET_FIELD(SPI_CTRL, SPI_RX_EN, ctrl, 1);
        hw_spi_set_ctrl_reg(id, ctrl);
...
void hw_spi_deinit(HW_SPI_ID id)
{
        ...
        // Disable RX & TX paths / Reset FIFO in SPI Control Register
        uint32_t ctrl = hw_spi_get_ctrl_reg(id);
