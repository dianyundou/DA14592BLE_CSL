                               ...
                               AD_IO_CONF_STATE state, bool is_ext_api)
{
        ...
        if (ad_io_configure(&io->rx, size, state) != AD_IO_ERROR_NONE) {
...
static int ad_uart_gpio_configure(const ad_uart_controller_conf_t *ad_uart_ctrl_conf)
{
        ...
        if (id == HW_UART1) {
                if (auto_flow_control) {
                        return  AD_UART_ERROR_AUTOFLOW_CONF_INVALID;
                }
        } else if ((id == HW_UART2)) {
        ...
        return ad_uart_gpio_config(id, io, AD_IO_CONF_ON, false);
}
...
{
        return ((id == HW_UART1) ? &ad_uart_events1 : &ad_uart_events2);
}
#endif /* CONFIG_UART_USE_SYNC_TRANSACTIONS */

__STATIC_INLINE resource_mask_t uart_resource_mask(const HW_UART_ID id)
{
        return RES_MASK(id == HW_UART1 ? RES_ID_UART1 : RES_ID_UART2);
}
...
static int ad_uart_res_acquire(ad_uart_handle_t handle, AD_UART_RES_TYPE res_type, uint32_t timeout)
{
        ...
        case AD_UART_RES_TYPE_CONFIG:
                resource_mask = RES_MASK(id == HW_UART1 ? RES_ID_UART1_CONFIG : RES_ID_UART2_CONFIG);
                break;
        case AD_UART_RES_TYPE_WRITE:
                resource_mask = RES_MASK(id == HW_UART1 ? RES_ID_UART1_WRITE : RES_ID_UART2_WRITE);
                break;
        case AD_UART_RES_TYPE_READ:
                resource_mask = RES_MASK(id == HW_UART1 ? RES_ID_UART1_READ : RES_ID_UART2_READ);
                break;
        default:
                /* Invalid argument. */
                OS_ASSERT(0);
...
static int ad_uart_apply_controller_config(ad_uart_handle_t handle)
{
        ...
        ad_uart_data_t *ad_uart_data = (ad_uart_data_t *)handle;
        HW_UART_ID id = ad_uart_data->ctrl->id;
        const uart_config *drv = &ad_uart_data->ctrl->drv->hw_conf;
#if (dg_configUART_RX_CIRCULAR_DMA == 1)
        ...
        if (hw_uart_init(id, drv) != HW_UART_CONFIG_ERR_NOERR) {
                UART_RES_TYPE_RELEASE(handle, AD_UART_RES_TYPE_CONFIG);
                return AD_UART_ERROR_CONTROLLER_CONF_INVALID;
        }
...
ad_uart_handle_t ad_uart_open(const ad_uart_controller_conf_t *ad_uart_ctrl_conf)
{
        ...
        HW_UART_ID id = ad_uart_ctrl_conf->id;
        uint32_t resource_mask = uart_resource_mask(id);
#if (HW_UART_DMA_SUPPORT == 1)
        ...
        int check_cfg = ad_uart_gpio_configure(ad_uart_ctrl_conf);
        if (check_cfg != AD_UART_ERROR_NONE) {
                ASSERT_WARNING(0);
                /*Before returning NULL, revert all configuration changes and restore the system to its original state*/
        ...
        if (ad_uart_apply_controller_config(ret)) {
                ...
                ret->ctrl = (ad_uart_handle_t) NULL;
...
int ad_uart_reconfig(ad_uart_handle_t handle, const ad_uart_driver_conf_t *ad_drv)
{
        ...
        ad_uart_data_t *ad_uart_data = (ad_uart_data_t *)handle;
        HW_UART_ID id = ad_uart_data->ctrl->id;
        const uart_config *drv = &ad_drv->hw_conf;
#if (HW_UART_DMA_SUPPORT == 1)
...
int ad_uart_write(ad_uart_handle_t handle, const char *wbuf, size_t wlen)
{
        ...
        HW_UART_CONFIG_ERR check_uart_send = hw_uart_send(id, (const uint8_t *) wbuf, wlen, ad_uart_signal_event_write, &cb_data);
        if (check_uart_send != HW_UART_CONFIG_ERR_NOERR) {
                UART_RES_TYPE_RELEASE(handle, AD_UART_RES_TYPE_WRITE);
                return AD_UART_ERROR_CONFIG_TX_SIZE;
        }
...
int ad_uart_read(ad_uart_handle_t handle, char *rbuf, size_t rlen, OS_TICK_TIME timeout)
{
        ...
        HW_UART_CONFIG_ERR check_uart_receive = hw_uart_receive(id, (uint8_t *) rbuf, rlen, ad_uart_signal_event_read, &cb_data);
        if (check_uart_receive != HW_UART_CONFIG_ERR_NOERR) {
                UART_RES_TYPE_RELEASE(handle, AD_UART_RES_TYPE_READ);
                return AD_UART_ERROR_CONFIG_RX_SIZE;
        }
        ...
        if (ad_uart_data->use_rx_circular_dma) {
                hw_uart_copy_dma_rx_to_user_buffer(id);
        }
#endif
