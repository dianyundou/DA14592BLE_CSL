...
enum l2cc_security_code
{
    ...
    /// Pairing Public Key
    L2C_CODE_PAIRING_PUBLIC_KEY           = 0x0C,
    /// Pairing DHKey Check
...
enum l2cc_cb_resp_value
{
    ...
    L2C_CB_CON_INVALID_SCID,
    /// Connection Refused - Source CID already allocated
    L2C_CB_CON_SCID_ALLOCATED,
    /// Connection Refused - unacceptable parameters
    L2C_CB_CON_UNACCEPTABLE_PARAM
#endif /* (RWBLE_SW_VERSION >= VERSION_8_1) */
};

/// Values for LE Credit Based Connection Status
enum l2cc_cb_con_status
{
    /// Not connected
    L2C_CB_NOT_CONNECTED          = 0x00,
    /// Connected
    L2C_CB_CONNECTED,
    /// Local device initiates disconnection (no error)
    L2C_CB_DISC_BY_LOCAL_DEV,
    /// Remote device initiates disconnection
    L2C_CB_DISC_BY_REMOTE_DEV,
    /// No More Credit Available
    L2C_CB_DISC_CREDIT_ERROR,
    /// Invalid MTU (MTU Exceed)
    L2C_CB_DISC_INVALID_MTU_SIZE,
    /// Invalid Packet Size (MPS exceed)
    L2C_CB_DISC_INVALID_MPS_SIZE,
    /// Invalid received SDU (SDU size invalid for instance)
    L2C_CB_DISC_INVALID_SDU,
};
...
#if (RWBLE_SW_VERSION_MAJOR >= 8)
/// Security Request
struct l2cc_public_key_exchange
{
    /// security code - 0x0C
    uint8_t     code;
    /// public key X
    uint8_t     public_keyx[ECDH_KEY_LEN];
    /// public key Y
    uint8_t     public_keyy[ECDH_KEY_LEN];
};
/// DHKey check
struct l2cc_dhkey_check
{
    ...
    uint8_t     code;
    /// dhkey check (E)
    uint8_t     dhkey_check[KEY_LEN];
};

enum l2cc_keypress_notification_type
{
    KEY_NOTIFICATION_ENTRY_START,
    KEY_NOTIFICATION_DIGIT_ENTERED,
    KEY_NOTIFICATION_DIGIT_ERASED,
    KEY_NOTIFICATION_CLEAR,
    KEY_NOTIFICATION_ENTRY_COMPLETE,
    KEY_NOTIFICATION_MAX,
};

/// Keypress notification
struct l2cc_keypress_notification
{
    /// security code - 0x0E
    uint8_t     code;
    /// Notification Type. Allowed values are from the enum KEY_NOTIFICATION_*
    uint8_t     type;
};
#endif /* (RWBLE_SW_VERSION_MAJOR >= 8) */
...
#if (RWBLE_SW_VERSION_MAJOR >= 8)
        /// Public key exchange
        struct l2cc_public_key_exchange     public_key_exchange;
        /// DHKey check 
        struct l2cc_dhkey_check             dhkey_check;
        /// Keypress notification
        struct l2cc_keypress_notification   keypress_notification;
#endif /* (RWBLE_SW_VERSION_MAJOR >= 8) */
 ...
 ****************************************************************************************
 */
uint8_t l2cc_pdu_pack(struct l2cc_pdu *p_pdu, uint16_t *p_offset, uint16_t *p_length, uint8_t *p_buffer,
        uint8_t conidx, uint8_t *llid);

/**
 ****************************************************************************************
 * @brief Handles the reception of an invalid PDU coming from Link Layer.
 *
 * @param[in] conidx    Connection index
 *
 ****************************************************************************************
 */
void l2cc_handle_invalid_pdu(uint8_t conidx);
 ...
 * @param[in]      packed_pkt           Pointer to the data of the received (packed) packet
 * @param[in]      packed_pkt_length    Length of the received packet
 * @param[in]      conidx               Connection Index
...
uint8_t l2cc_pdu_unpack(struct l2cc_pdu *p_pdu, uint16_t *p_offset, uint16_t *p_rem_len,
                        const uint8_t *packed_pkt, uint16_t packed_pkt_length, uint8_t conidx, uint8_t llid);

/**
 ****************************************************************************************
 * @brief Parse L2CAP header to detect the receiver of the data
 * If the packet has erroneous channel id.
 *
...
/**
 ****************************************************************************************
 * @brief Check if segmented SDU is fully received
 *
 * @param[in]      conidx   Connection Index
 ...
 ****************************************************************************************
 */
uint16_t l2c_process_sdu(uint8_t conidx, struct l2cc_pdu *pdu);
 ...
 * @param[in]      conidx   Connection Index
 * @param[in]      cid      Channel ID
 * @param[in]      size     Length of stored segments
 *
 * @return Destination task
 *
 ****************************************************************************************
 */
void l2c_send_lecb_message(uint8_t conidx, uint16_t cid, uint16_t size);
