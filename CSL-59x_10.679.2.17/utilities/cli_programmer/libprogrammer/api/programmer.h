...
#define LIB_PROG_VERSION_MINOR       1
...
#define ERR_PROG_EFLASH_WRITE           -302    /**< eFLASH write error */
#define ERR_PROG_EFLASH_VERIFY          -303    /**< eFLASH verify error */
 ...
 * \return 0 on success, error code on failure
 *
 * \warning Not applicable in DA1459x.
 *
 */
 ...
 * \return 0 on success, error code on failure
 *
 * \note: In case of DA1459x, the OTP is emulated by allowing certain regions in eFLASH to be
 * write/erase protected. Thus, this functions is basically similar to prog_write_eflash()
 * with the main difference that the address and data are specified in words instead of bytes.
 *
 */
 ...
 *
 * \note: In case of DA1459x, the OTP is emulated by allowing certain regions in eFLASH to be
 * write/erase protected. Thus, this functions is basically similar to prog_read_eflash()
 * with the main difference that the address and data are specified in words instead of bytes.
 *
 */
 ...
 *
 */
int DLLEXPORT prog_chip_erase_eflash(void);
 ...
 * \return 0 on success, error code on failure
 */
int DLLEXPORT prog_read_eflash(uint32_t address, uint8_t *buf, uint32_t len);
 ...
 *
 */
int DLLEXPORT prog_read_eflash_to_file(uint32_t address, const char *fname, uint32_t len);

/**
 * \brief Write buffer eFLASH memory
 *
 * \param [in] flash_address eFLASH address, where file will be written
 * \param [in] buf Buffer to written to device flash
 * \param [in] size Number of bytes to write
 *
 * \return 0 on success, error code on failure
 *
 */
int DLLEXPORT prog_write_to_eflash(uint32_t flash_address, const uint8_t *buf, uint32_t size);

/**
 * \brief Write file to eFLASH memory
 *
 *
 * \param [in] flash_address eFLASH address, where file will be written
 * \param [in] file_name Name of the file to written
 ...
 *
 */
int DLLEXPORT prog_write_file_to_eflash(uint32_t flash_address, const char *file_name, uint32_t size);

/**
 * \brief Copy from RAM to eFLASH memory
 *
 *
 * \param [in] mem_address RAM address memory (source)
 * \param [in] flash_address eFLASH address memory (destination)
 * \param [in] size Number of bytes to copy
 *
 * \return 0 on success, error code on failure
 *
 */
int DLLEXPORT prog_copy_to_eflash(uint32_t mem_address, uint32_t flash_address, uint32_t size);

/**
 * \brief Erase part of eFLASH memory
 *
 *
 * \param [in] flash_address eFLASH address
 * \param [in] size Number of bytes of the flash memory to erase
 *
 * \return 0 on success, error code on failure
 *
 */
int DLLEXPORT prog_erase_eflash(uint32_t flashAddress, uint32_t size);

/**
 * \brief Check emptiness eFLASH
 *
 * If specified flash region is empty, parameter \p ret_number is a number of checked bytes
 * (positive value). Otherwise if specified flash region contains values different than 0xFF,
 * parameter \p ret_number is a nonpositive value. This value is a number of first non 0xFF byte in
 ...
 *
 */
int DLLEXPORT prog_is_empty_eflash(unsigned int size, unsigned int start_address, int *ret_number);
