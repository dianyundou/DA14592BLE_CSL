...
#define ED25519_PRIV_KEY_LENGTH 32

/* Ed25519 signature length */
#define ED25519_SIG_LENGTH      64

/* Size of version string field in header */
#define VERSION_FIELD_LENGTH    16

/* Size of FW version length */
...
static char *find_cstring(const char *text)
{
...
#if defined(_WIN32)
        /* strndup is not available in MINGW32 toolchain */
        s = malloc(length + 1);
        if (!s) {
                return NULL;
        }
        ...
        return s;
#else
        return strndup(from, length);
#endif /* _WIN32 */
}

static bool get_version(size_t size, const char *buffer, uint8_t *hdr_version)
{
        const char *from;
        char *version;
        char *version_start;

        from = strstr(buffer, "SW_VERSION ");
        ...
        if (strlen(version_start) >= VERSION_FIELD_LENGTH) {
                /* Force '\0' sign at the end of the string */
                strncpy((char *) hdr_version, version_start, VERSION_FIELD_LENGTH - 1);
                hdr_version[VERSION_FIELD_LENGTH - 1] = '\0';
        } else {
                /* Copy whole string (with '\0' sign). Unused bytes should keep 0xFF value */
                strcpy((char *) hdr_version, version_start);
        }
...
static bool get_date(size_t size, const char *buffer, uint8_t *hdr_timestamp)
{
        const char *from;
        char *timestamp;
        struct tm tm;
        ...
        from = strstr(buffer, "SW_VERSION_DATE ");
        ...
        sscanf(timestamp, "%d-%d-%d %d:%d", &tm.tm_year, &tm.tm_mon, &tm.tm_mday, &tm.tm_hour,
                                                                                        &tm.tm_min);
        tm.tm_year -= 1900;
        tm.tm_mon--;
        ...
        store32(hdr_timestamp, (uint32_t) secs);
        free(timestamp);
...
{
        uint8_t version_string[VERSION_FIELD_LENGTH] = { 0 };

        if (!buffer || size < 1) {
                return false;
        }

        if (!get_version(size, buffer, version_string)) {
                return false;
        }

        return version_str_to_version_number((char *) version_string, version);
}
                                                                ...
                                                                size_t out_size, uint8_t *out)
{
        ...
        if (!get_version(ver_size, (char *) ver, header.version)) {
                return MKIMAGE_STATUS_INVALID_DATA;
        }

        if (!get_date(ver_size, (char *) ver, (uint8_t *) &header.timestamp)) {
                return MKIMAGE_STATUS_INVALID_DATA;
        }
...
static size_t fill_device_administration_section(uint8_t *buffer,
                                                        ...
                                                        const mkimage_secure_image_opt_data_t *data)
{
        ...
        payload_ptr = put_tlv(payload_ptr, SECURITY_HDR_TYPE_FW_VERSION_NUMBER,
...
/*
 * Function writes device administration section (for DA1469x devices) content to the buffer. It
 * returns number of the written bytes if no error occurs, 0 otherwise.
 */
static size_t fill_device_adm_section_da1469x(uint8_t *buffer,
                                                const mkimage_device_adm_data_da1469x_t *data)
{
        int i;
        uint8_t key_rev_record[256] = { 0 };
        uint8_t *tmp_ptr = key_rev_record;
        ...
        if (!data || !data->key_rev_array || data->key_rev_number < 1) {
                /* This section is empty - write only the type and the size */
                goto write_section;
        }

        /* Write key revocation record */
        for (i = 0; i < data->key_rev_number; i++) {
                if (data->key_rev_array[i].id > 0xFF) {
                        /* Only 1-byte index could be used for DA1469x */
                        continue;
                }

                switch (data->key_rev_array[i].type) {
                case MKIMAGE_KEY_TYPE_PUBLIC:
                        *tmp_ptr = SECURITY_HDR_KEY_TYPE_SIGNATURE;
                        break;
                case MKIMAGE_KEY_TYPE_SYMMETRIC:
                        *tmp_ptr = SECURITY_HDR_KEY_TYPE_USER_DATA;
                        break;
                case MKIMAGE_KEY_TYPE_DECRYPTION:
                        *tmp_ptr = SECURITY_HDR_KEY_TYPE_DECRYPTION;
                        break;
                default:
                        /* Unrecognized key ID */
                        continue;
                ...
                ++tmp_ptr;
                *tmp_ptr = (uint8_t) data->key_rev_array[i].id;
                ++tmp_ptr;
        }
...
/*
 * Function writes device administration section (for DA1459X devices) content to the buffer. It
 * returns number of the written bytes if no error occurs, 0 otherwise.
 */
static size_t fill_device_adm_section_da1459x(uint8_t *buffer,
                                                        const mkimage_device_adm_data_da1459x_t *data)
{
        int i;
        uint8_t key_rev_record[256] = { 0 };
        uint8_t *tmp_ptr = key_rev_record;
        ...
        if (!data->key_rev_array || data->key_rev_number < 1) {
                goto minimum_fw_version;
        }

        /* Write key revocation record */
        for (i = 0; i < data->key_rev_number; i++) {
                switch (data->key_rev_array[i].type) {
                case MKIMAGE_DA1459X_KEY_TYPE_SIGNATURE:
                        *tmp_ptr = 0xA1;
                        break;
                case MKIMAGE_DA1459X_KEY_TYPE_USER_DATA:
                        *tmp_ptr = 0xA2;
                        break;
                default:
                        /* Unrecognized key ID */
...
minimum_fw_version:
        if (!data->set_minimum_fw_version) {
                goto write_section;
        }
        /* notice that there is no length field */
        store16(payload_ptr, SECURITY_HDR_TYPE_FW_VERSION_NUMBER);
        payload_ptr += 2;
        store32(payload_ptr, data->minimum_fw_version);
        payload_ptr += 4;
...
static size_t fill_security_section(uint8_t *buffer, const void *hdr, size_t hdr_size,
                                                const uint8_t *signature, uint32_t signature_len)
{
        ...
        if (hdr_size > 0) {
                memcpy(payload_ptr, hdr, hdr_size);
                payload_ptr += hdr_size;
        }

        if (signature && signature_len > 0) {
                payload_ptr = put_tlv(payload_ptr, SECURITY_HDR_TYPE_SIGNATURE_SECTION,
 ...
 */
static size_t calculate_pattern_size(size_t hdr_size, size_t dev_adm_section_size,
                                                                size_t security_section_size)
{
        size_t padding_size = 0;
        size_t header_size;

        header_size = hdr_size + security_section_size + dev_adm_section_size;
...
static void rng_init(void)
{
        ...
        if (init_done) {
                /* Random number generator has been initialized already - don't do this again */
                return;
        }
...
static void rng_bytes(uint8_t *buffer, size_t len)
{
        int i;
...
/*
 * Encrypt input buffer using given nonce and store result in output buffer. Return false if error
 * occurs, true otherwise. The 'nonce' should have 8 bytes and the key should have 32 bytes.
 */
static bool aes_ctr_encrypt(const uint8_t *nonce, const uint8_t *key, size_t input_size,
                                                        const uint8_t *input, uint8_t *output)
{
        uint8_t counter_block[AES_BLOCKSIZE];
        crypto_buffer_t key_buffer, input_buffer;

        /* Counter block = XXXXXXXXXXXXXXXX0000000000000000, where 'XX' are bytes of NONCE */
        memset(counter_block, 0, sizeof(counter_block));
        memcpy(counter_block, nonce, 8);

        /* These buffers will be used in read-only mode */
        crypto_buffer_init(&key_buffer, 32, (uint8_t *) key);
        crypto_buffer_init(&input_buffer, input_size, (uint8_t *) input);

        return crypto_aes_ctr_encrypt(&key_buffer, counter_block, &input_buffer, output);
}
                                                                ...
                                                                uint8_t **out, size_t *out_size)
{
        ...
        security_section_size = fill_security_section(security_section, &hdr, sizeof(hdr),
        ...
        pattern_size = calculate_pattern_size(sizeof(suota_1_1_image_header_t),
                                                dev_adm_section_size, security_section_size);
...
mkimage_status_t mkimage_create_da1469x_image(size_t in_size, const uint8_t *in,
                                                size_t ver_size, const uint8_t *ver,
                                                const mkimage_security_data_da1469x_t *data,
                                                const mkimage_device_adm_data_da1469x_t *opt_data,
                                                                uint8_t **out, size_t *out_size)
{
        suota_1_1_image_header_da1469x_t header;
        suota_security_header_da1469x_t hdr;
        mkimage_status_t status = MKIMAGE_STATUS_OK;
        uint32_t crc32 = 0xFFFFFFFF;
        uint8_t dev_adm_section[2048] = { 0 };
        uint8_t security_section[2048] = { 0 };
        uint8_t signature[ED25519_SIG_LENGTH] = { 0 };
        uint8_t *data_buffer = NULL;
        uint8_t *in_aligned = NULL;
        uint8_t *out_ptr;
        size_t signature_len = sizeof(signature);
        size_t in_aligned_size = in_size;
        unsigned int tlv_length;
        unsigned int dev_adm_section_size;
        unsigned int security_section_size;
        unsigned int pattern_size;
        bool secure_image = data ? true : false;

        /* Check pointers */
        if (!in || !ver || (secure_image && !(data->priv_key && data->sym_key))) {
                return MKIMAGE_STATUS_INVALID_PARAMETER;
        }
        ...
        if (secure_image) {
                /* Copy security section content */
                hdr.ecc_key_idx = data->ecc_key_idx;
                hdr.sym_key_idx = data->sym_key_idx;

                if (data->nonce) {
                        /* Copy NONCE */
                        memcpy(hdr.nonce, data->nonce, sizeof(hdr.nonce));
                } else {
                        /* Generate NONCE using random number generator */
                        rng_init();
                        rng_bytes(hdr.nonce, sizeof(hdr.nonce));
                }

                /*
                 * In secure mode executable must be encrypted using AES CTR mode. In this case
                 * input must be aligned to the multiply of 16 bytes.
                 */
                if (in_aligned_size % AES_BLOCKSIZE) {
                        in_aligned_size += AES_BLOCKSIZE - in_aligned_size % AES_BLOCKSIZE;
                }

                /* Allocate new buffer for aligned input. Last bytes should have 0 value */
                in_aligned = malloc(in_aligned_size);

                if (!in_aligned) {
                        return MKIMAGE_STATUS_ALLOCATION_ERROR;
                }

                memset(in_aligned, 0, in_aligned_size);
                memcpy(in_aligned, in, in_size);
        }
        else {
                memset(&hdr, 0, sizeof(hdr));
        }

        /* Create security section with empty signature - it will be overwritten later */
        security_section_size = fill_security_section(security_section, &hdr, secure_image ?
                                sizeof(hdr) : 0, signature, secure_image ? ED25519_SIG_LENGTH : 0);

        /* Create device administration section */
        dev_adm_section_size = fill_device_adm_section_da1469x(dev_adm_section, opt_data);
        ...
        /* Calculate pattern size */
        pattern_size = calculate_pattern_size(sizeof(header), dev_adm_section_size,
                                                                        security_section_size);

        if (!secure_image) {
                /* Non-secure images are not encrypted and not signed */
                goto common_part;
        }

        /* Store encrypted executable in the same buffer */
        aes_ctr_encrypt(hdr.nonce, data->sym_key, in_aligned_size, in_aligned, in_aligned);

        /* Glue aligned device administration section and the input (application binary) */
        data_buffer = malloc(dev_adm_section_size + pattern_size + in_aligned_size);
        ...
        memcpy(data_buffer, dev_adm_section, dev_adm_section_size);
        memset(data_buffer + dev_adm_section_size, 0xFF, pattern_size);
        memcpy(data_buffer + dev_adm_section_size + pattern_size, in_aligned, in_aligned_size);

        /* Generate signature (it covers aligned device administration section and the input */
        status = create_signature(dev_adm_section_size + pattern_size + in_aligned_size, data_buffer,
                                        MKIMAGE_ELLIPTIC_CURVE_EDWARDS25519,
                                        MKIMAGE_HASH_METHOD_SHA512, ED25519_PRIV_KEY_LENGTH,
                                                        data->priv_key, &signature_len, signature);
...
common_part:
        ...
        /* Calculate CRC-32 */
        crc32 = compute_crc32(in_aligned_size, in_aligned ? in_aligned : in);
        ...
        /*
         * Output = DA1469x header + security section + device administration section + pattern +
         *          app binary
         */
        *out_size =  sizeof(header) + tlv_length + in_aligned_size;
        *out = malloc(*out_size);
        out_ptr = *out;
        ...
        header.image_identifier[0] = SUOTA_1_1_IMAGE_DA1469x_HEADER_SIGNATURE_B1;
        header.image_identifier[1] = SUOTA_1_1_IMAGE_DA1469x_HEADER_SIGNATURE_B2;
        store32((uint8_t *) &header.size, in_aligned_size);
        store32((uint8_t *) &header.crc, crc32);
        store32((uint8_t *) &header.pointer_to_ivt, sizeof(header) + tlv_length);

        if (!get_version(ver_size, (char *) ver, header.version_string)) {
                status = MKIMAGE_STATUS_INVALID_DATA;
                goto done;
        }
        ...
        out_ptr += sizeof(header);
        memcpy(out_ptr, data_buffer, tlv_length);
        out_ptr += tlv_length;
        memcpy(out_ptr, in_aligned ? in_aligned : in, in_aligned_size);

        status = MKIMAGE_STATUS_OK;
...
mkimage_status_t mkimage_create_d2522_image(size_t in_size, const uint8_t *in,
                                                size_t ver_size, const uint8_t *ver,
                                                const mkimage_security_data_d2522_t *data,
                                                const mkimage_device_adm_data_d2522_t *opt_data,
                                                                uint8_t **out, size_t *out_size)
{
        return mkimage_create_da1469x_image(in_size, in, ver_size, ver, data, opt_data, out, out_size);
}

mkimage_status_t mkimage_create_da1459x_image(size_t in_size, const uint8_t *in, size_t ver_size,
                                                const uint8_t *ver, uint32_t fw_version,
                                                const mkimage_security_data_da1459x_t *data,
                                                const mkimage_device_adm_data_da1459x_t *opt_data,
                                                                uint8_t **out, size_t *out_size)
{
        mkimage_status_t status = MKIMAGE_STATUS_OK;
        suota_1_1_image_header_da1469x_t *header;
        uint8_t dev_adm_section[2048] = { 0 };
        size_t dev_adm_section_size;
        uint8_t hdr;
        uint8_t security_section[2048] = { 0 };
        size_t security_section_size;
        uint8_t signature[ED25519_SIG_LENGTH] = { 0 };
        ...
        /* Create device administration section */
        dev_adm_section_size = fill_device_adm_section_da1459x(dev_adm_section, opt_data);

        if (data) {
                /* There is only public key index in header */
                hdr = data->pub_key_idx;

                /* Create security section with empty signature - it will be overwritten later */
                security_section_size = fill_security_section(security_section, &hdr, sizeof(hdr),
                                                                signature, ED25519_SIG_LENGTH);
        } else {
                /* Create empty security section */
                security_section_size = fill_security_section(security_section, NULL, 0, NULL, 0);
        }

        header_size = sizeof(suota_1_1_image_header_da1469x_t) + security_section_size +
                                                        FW_VERSION_SIZE + dev_adm_section_size;
        alignment_size = ((header_size / 0x400) + 1) * 0x400 - header_size;
        *out_size = header_size + alignment_size + in_size;
        ...
        *out = malloc(*out_size);
        ...
        /* Put the content to sign */
        write_ptr = *out + *out_size - in_size;
        memcpy(write_ptr, in, in_size);
        write_ptr -= alignment_size;
        memset(write_ptr, 0xFF, alignment_size);
        write_ptr -= dev_adm_section_size;
        memcpy(write_ptr, dev_adm_section, dev_adm_section_size);
        write_ptr -= FW_VERSION_SIZE;
        store32(write_ptr, fw_version);

        if (data) {
                status = create_signature(in_size + alignment_size + dev_adm_section_size + FW_VERSION_SIZE,
                                                write_ptr, MKIMAGE_ELLIPTIC_CURVE_EDWARDS25519,
                                                MKIMAGE_HASH_METHOD_SHA512, ED25519_PRIV_KEY_LENGTH,
                                                data->priv_key, &signature_len, signature);

                /* Replace signature at the end of security section */
                memcpy(security_section + security_section_size - signature_len, signature,
                                                                                signature_len);
        }

        write_ptr -= security_section_size;
        memcpy(write_ptr, security_section, security_section_size);

        /* Prepare header */
        write_ptr -= sizeof(suota_1_1_image_header_da1469x_t);
        header = (suota_1_1_image_header_da1469x_t*) write_ptr;
        header->image_identifier[0] = SUOTA_1_1_IMAGE_DA1469x_HEADER_SIGNATURE_B1;
        header->image_identifier[1] = SUOTA_1_1_IMAGE_DA1469x_HEADER_SIGNATURE_B2;
        store32((uint8_t *) &header->size, in_size);
        store32((uint8_t *) &header->crc, compute_crc32(in_size, in));
        ...
        store32((uint8_t *) &header->pointer_to_ivt, header_size + alignment_size);
