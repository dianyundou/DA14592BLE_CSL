...
{
        uint32_t pdm_div_reg = CRG_AUD->PDM_DIV_REG;

        if (pdm_div_reg & REG_MSK(CRG_AUD, PDM_DIV_REG, PDM_MASTER_MODE)) {
                return true;
        }

        return(!!(pdm_div_reg & REG_MSK(CRG_AUD, PDM_DIV_REG, CLK_PDM_EN)));
}
