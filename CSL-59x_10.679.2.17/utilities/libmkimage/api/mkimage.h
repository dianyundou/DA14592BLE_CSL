...
typedef enum {
        ...
        /** Symmetric key used in executable decryption */
        MKIMAGE_KEY_TYPE_DECRYPTION,
} mkimage_key_type_t;
...
typedef enum {
        /** Asymmetric, public key used in signature verification */
        MKIMAGE_DA1459X_KEY_TYPE_SIGNATURE,
        /** Symmetric key used in user data encryption */
        MKIMAGE_DA1459X_KEY_TYPE_USER_DATA,
} mkimage_da1459x_key_type_t;

/** DA1459X Key identifier */
typedef struct {
        /** Key type */
        mkimage_da1459x_key_type_t type;
        /** Key ID - index in range (0-7) */
        uint8_t id;
} mkimage_da1459x_key_id_t;
...
/** DA1469x security configuration */
typedef struct {
        /** Private key - used in signature generation (32 bytes in length) */
        const uint8_t *priv_key;
        /** Symmetric key - used in executable encryption (32 bytes in length) */
        const uint8_t *sym_key;
        /**
         * Public key index - used in signature validation (on platform). It must match to the used
         * private key.
         */
        uint8_t ecc_key_idx;
        /**
         * Symmetric key index - used in executable decryption (on platform). It must match to the
         * used symmetric key.
         */
        uint8_t sym_key_idx;
        /**
         * Nonce used in AES CTR encryption/decryption (8 bytes in length). Could be NULL - in this
         * case it will be generated using random number generator.
         */
        const uint8_t *nonce;
} mkimage_security_data_da1469x_t;

/** This struct is about to be deprecated - Kept for backward compatibility **/
typedef mkimage_security_data_da1469x_t mkimage_security_data_d2522_t;

/** DA1469x device administration configuration */
typedef struct {
        /** Number of keys which should be revoked */
        unsigned int key_rev_number;
        /** Array with keys which should be revoked */
        const mkimage_key_id_t *key_rev_array;
} mkimage_device_adm_data_da1469x_t;

/** This struct is about to be deprecated - Kept for backward compatibility **/
typedef mkimage_device_adm_data_da1469x_t mkimage_device_adm_data_d2522_t;

/** DA1459X security configuration */
typedef struct {
        /** Private key - used in signature generation (32 bytes in length) */
        const uint8_t *priv_key;
        /** Public key index - used in signature validation (on platform).
         * ECC key is synonym to that.
         */
        uint8_t pub_key_idx;
} mkimage_security_data_da1459x_t;

/** DA1459X device administration configuration */
typedef struct {
        /** Number of keys which should be revoked */
        unsigned int key_rev_number;
        /** Array with keys which should be revoked */
        const mkimage_da1459x_key_id_t *key_rev_array;
        /** Indicate new minimal FW version in image header */
        bool set_minimum_fw_version;
        /** New minimum FW version. Used only if set_minimum_fw_version = true */
        uint32_t minimum_fw_version;
} mkimage_device_adm_data_da1459x_t;
...
/**
 * \brief Create DA1469x device image
 *
 * Function creates image which could be used as SUOTA image for DA1469x devices. The image is created
 * from binary and version file data. If \p data is passed then image is created in secure mode -
 * executable (application binary) data is encrypted (using AES CTR) and signed (together with
 * device administration section). Ed25519 is used as algorithm of digital signature generation.
 * \p opt_data is an optional parameter and contains device administration configuration e.g.
 ...
 * \param [in]  ver_size                version data size
 * \param [in]  ver                     version data (text file content)
 * \param [in]  data                    security configuration, could be NULL
 * \param [in]  opt_data                device administration configuration, could be NULL
 * \param [out] out                     allocated buffer with image data
 * \param [out] out_size                output buffer size
 *
 * \return command execution status
 *
 */
mkimage_status_t DLLEXPORT mkimage_create_da1469x_image(size_t in_size, const uint8_t *in,
                                                        size_t ver_size, const uint8_t *ver,
                                                        const mkimage_security_data_da1469x_t *data,
                                                        const mkimage_device_adm_data_da1469x_t *opt_data,
                                                                uint8_t **out, size_t *out_size);

/** Function to be deprecated - Kept for backwards compatibility **/
mkimage_status_t DLLEXPORT mkimage_create_d2522_image(size_t in_size, const uint8_t *in,
                                                      size_t ver_size, const uint8_t *ver,
...
/**
 * \brief Create DA1459X device image
 *
 * Function creates image which could be used as SUOTA image for DA1459X devices. The image is created
 * from binary and version file data. If \p data is passed then image is signed using Ed25519
 * digital signature generation algorithm.
 * \p opt_data is an optional parameter and contains device administration configuration i.e.
 * minimal FW version and key revocation commands.
 *
 * \param [in]  in_size                 input data size
 * \param [in]  in                      input data (binary file content)
 * \param [in]  ver_size                version data size
 * \param [in]  ver                     version data (text file content)
 * \param [in]  fw_version              FW version number
 * \param [in]  data                    security configuration, could be NULL
 * \param [in]  opt_data                device administration configuration, could be NULL
 * \param [out] out                     allocated buffer with image data
 * \param [out] out_size                output buffer size
