...
#define mainBIT_BLE_STACK_READ_CB       (1 << 7)
#define mainBIT_TEMP_MONITOR_ENABLE     (1 << 9)
#define mainBIT_TEMP_MONITOR_DISABLE    (1 << 10)
#define mainBIT_BLE_RF_CALIBRATION      (1 << 11)
...
/**
 * \brief Notify BLE adapter to perform RF calibration
 *
 * Should be called by sys_gpadc when there has been a temperature drift for which an RF calibration
 * should be performed.
 */
void ad_ble_notify_rf_calibration(void);
...
/**
 * \brief Set RF calibration info
 *
 * This function sets the value of the rf_calibration_info, which is used by the BLE adapter in
 * order to update the corresponding field of the dynamic CMAC configuration table.
 *
 * \param[in] value New value of RF calibration info
 */
