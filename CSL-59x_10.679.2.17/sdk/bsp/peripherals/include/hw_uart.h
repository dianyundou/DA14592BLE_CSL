 ...
 * \brief Baud rates
 *
 * \note Baud rate values greater than 2000000 can only be selected when Doubler is used as system clock!
 *       Otherwise, a warning-assertion will be triggered!
 */
typedef enum {
        HW_UART_BAUDRATE_1200           = 0,    /**< baud rate 1200 */
        HW_UART_BAUDRATE_2400           = 1,    /**< baud rate 2400 */
        HW_UART_BAUDRATE_4800           = 2,    /**< baud rate 4800 */
        HW_UART_BAUDRATE_9600           = 3,    /**< baud rate 9600 */
        HW_UART_BAUDRATE_14400          = 4,    /**< baud rate 14400 */
        HW_UART_BAUDRATE_19200          = 5,    /**< baud rate 19200 */
        HW_UART_BAUDRATE_28800          = 6,    /**< baud rate 28800 */
        HW_UART_BAUDRATE_38400          = 7,    /**< baud rate 38400 */
        HW_UART_BAUDRATE_57600          = 8,    /**< baud rate 57600 */
        HW_UART_BAUDRATE_115200         = 9,    /**< baud rate 115200 */
        HW_UART_BAUDRATE_230400         = 10,   /**< baud rate 230400 */
        HW_UART_BAUDRATE_256000         = 11,   /**< baud rate 256000 */
        HW_UART_BAUDRATE_460800         = 12,   /**< baud rate 460800 */
        HW_UART_BAUDRATE_500000         = 13,   /**< baud rate 500000 */
        HW_UART_BAUDRATE_921600         = 14,   /**< baud rate 921600 */
        HW_UART_BAUDRATE_1000000        = 15,   /**< baud rate 1000000 */
        HW_UART_BAUDRATE_2000000        = 16,   /**< baud rate 2000000 */
        HW_UART_BAUDRATE_3000000        = 17,   /**< baud rate 3000000 */
 ...
 * +--------+--------+--------+--------+
 *
 * \note BAUDRATE_3000000 -> Requires Doubler@64MHz to be used as sys clk! An actual baud rate of 3047619.05 will be generated!
 */
extern const uint32_t hw_uart_divisor[HW_UART_BAUDRATE_MAX];
 ...
 *
 */
void hw_uart_configure_dma_channels(HW_UART_ID uart, const uart_config *uart_init);
...
{
        if (uart == HW_UART2) {
                REG_SETF(UART2, UART2_CONFIG_REG, ISO7816_SCRATCH_PAD, value);
        } else {
                UART->UART_SCR_REG = value;
        }
}
...
__STATIC_INLINE uint8_t hw_uart_read_scr(const HW_UART_ID uart)
{
        uint8_t ret_val = 0;
        if (uart == HW_UART2) {
                ret_val =  REG_GETF(UART2, UART2_CONFIG_REG, ISO7816_SCRATCH_PAD);
        } else {
                ret_val = UART->UART_SCR_REG;
        }
        return ret_val;
}
 ...
 *
 * This function allows the user to use custom high baud rates. The baud rates that can be used
 * are not only the ones that are part of the HW_UART_BAUDRATE enum.
 *
 * \param [in] uart identifies UART to use
 * \param [in] sclk Serial clock, if true then DIV1 is used otherwise DIVN.
 *
 */
 ...
 *
 * \note If baud_rate < 0x100, an ASSERT_WARNING() is triggered unless all the conditions
 *       below are satisfied:
