...
{
        gatt_client_t *gatt_client = (gatt_client_t *) client;
        gatt_client_serialized_t *s_data = data;
        *length = sizeof(gatt_client_serialized_t);

        if (!data) {
                return;
        }

        s_data->service_changed_h = gatt_client->service_changed_h;
        s_data->service_changed_ccc_h = gatt_client->service_changed_ccc_h;
        s_data->start_h = gatt_client->client.start_h;
        s_data->end_h = gatt_client->client.end_h;
}

static gatt_client_t *init(uint16_t conn_idx, const gatt_client_callbacks_t *cb)
{
        ...
        gatt_client->client.serialize           = serialize;
        gatt_client->client.conn_idx            = conn_idx;
                                                        ...
                                                        const ble_evt_gattc_browse_svc_t *evt)
{
        ...
        gatt_client = init(evt->conn_idx, cb);
        gatt_client->client.start_h = evt->start_h;
        gatt_client->client.end_h = evt->end_h;
...
{
        const gatt_client_serialized_t *s_data = (const gatt_client_serialized_t *) data;
        gatt_client_t *gatt_client;

        if (!data || !cb || (length < sizeof(gatt_client_serialized_t))) {
                return NULL;
        }

        gatt_client = init(conn_idx, cb);

        gatt_client->service_changed_h = s_data->service_changed_h;
        gatt_client->service_changed_ccc_h = s_data->service_changed_ccc_h;
        gatt_client->client.start_h = s_data->start_h;
        gatt_client->client.end_h = s_data->end_h;

        ble_client_attach(&gatt_client->client, conn_idx);
