...
-# Select the project you wish to import from the checklist

## Building for RAM

Build your project on the host (user's PC) using a *RAM build configuration (e.g DA1459x-00-Release_RAM)
...
-# Select the chip revision and flash options by running `program_eflash_config` or `program_qspi_config` script from `External tools` drop down list, depending on your XiP device (Embedded Flash or QSPI). This step needs to be done only once.

-# Build the project on the host (user's PC) using one of the flash build configurations named *eFlash or *QSPI.
  \note Make sure that the build configuration matches your chip revision e.g DA1459x-00-Release_eFlash
...
-# Flash the image to the XiP device over either the jtag interface or the serial port, using the appropriate script from the `External tools`:
 - Flash over jtag: `program_eflash_jtag` or `program_qspi_jtag`
 - Flash over serial port: `program_eflash_serial` or `program_qspi_serial`. The scripts prompt for the PC serial port where the board is attached.
...
-# Click <b> `'Debug As...'` </b> drop-down list and select:
 ...
 - RAM - to launch and debug the project executed from RAM
 - eFLASH - to launch and debug the project executed from eFlash
 - QSPI - to launch and debug the project executed from QSPI
