...
extern void accessory_enable_serial_number_lookup(void);
extern void accessory_start_pair_mode(void);

static void cmd_help(uint8_t argc, char *argv[]);
static void cmd_reset(uint8_t argc, char *argv[]);
static void cmd_pair_mode(uint8_t argc, char *argv[]);
...
/*      command,     min max args, handler,             help text,             command args */
        ...
        {"pairmode",    0, 0,      cmd_pair_mode,       "Enable pair mode",    ""},
...
static void read_serial_number(uint8_t *serial)
{
        afmn_conn_params_t params[] = {
                { .param = AFMN_CONN_PARAMS_SERIAL_NUMBER, .data = serial,
                  .len = SERIAL_NUMBER_LENGTH }
        };

        afmn_conn_params_get_params(params, ARRAY_LENGTH(params));
}
