...
__RETAINED static uint8_t state;

__RETAINED static queue_t device_list;
...
static bool pending_events_match_elem(const void *elem, const void *handle)
{
        return ((pending_event_t *) elem)->handle == *((uint16_t *) handle);
}

void pending_events_put_handle(device_t *dev, uint16_t handle)
{
        pending_event_t *elem;

        elem = OS_MALLOC(sizeof(pending_event_t));
        elem->handle = handle;

        queue_push_back(&dev->pending_events, elem);
}

void pending_events_remove_handle(device_t *dev, uint16_t handle)
{
        pending_event_t *elem;

        elem = queue_remove(&dev->pending_events, pending_events_match_elem, &handle);
        if (elem != NULL) {
                OS_FREE(elem);
        }
}

bool pending_events_has_handle(const device_t *dev, uint16_t handle)
{
        return queue_find(&dev->pending_events, pending_events_match_elem, &handle) != NULL;
}

void pending_events_clear_handles(device_t *dev)
{
        queue_remove_all(&dev->pending_events, OS_FREE_FUNC);
}
