...
static hw_src_config_t src_config[MAX_NUM_OF_PATHS];
static  dma_user_data_t dma_user_data[CHANNEL_NUM_MAX * MAX_NUM_OF_PATHS * 2];
static bool single_dev_type_out[AUDIO_SIZE];
static bool single_dev_type_in[AUDIO_SIZE];
static bool audio_path_idx_status[MAX_NUM_OF_PATHS];
...
static void initialize_dma_reg(uint8_t path_num, uint8_t max_path_num, sys_audio_memory_specific_t *param, SYS_AUDIO_MGR_DMA_DIR dir)
{
        ...
        if (src_config[path_num].id != 0) {
                if (src_config[path_num].id == HW_SRC1) {
                         ...
                         * For 2 DATA PATHS
                         */
                        if (max_path_num == MAX_NUM_OF_PATHS) {

                                /* For 2 audio channels (stereo) */
                                if (param->stereo) {
                                        /*
                                         * For Peripheral(SRCx)-to-Memory transfer
                                         */
                                        if (paths[path_num].dev_in->device_type != AUDIO_MEMORY) {
                                                channel_setup.dma_req_mux = HW_DMA_TRIG_SRC_RXRX;
                                        }

                                        /*
                                         * For Memory-to-Peripheral(SRCx) transfer
                                         */
                                        if (paths[path_num].dev_out->device_type != AUDIO_MEMORY) {
                                                channel_setup.dma_req_mux = HW_DMA_TRIG_SRC_TXTX;
                                        }
                                } else {
                                        /* For 1 audio channel (mono) */
                                        if (paths[path_num].dev_out->device_type ==
                                                paths[path_num^1].dev_out->device_type &&
                                                paths[path_num].dev_out->device_type == AUDIO_MEMORY) {

                                                /*
                                                 * For Peripheral(SRCx)-to-Memory transfer for the one SRC and
                                                 * Memory-to-Peripheral(SRCx) transfer for the other
                                                 */
                                                channel_setup.dma_req_mux = HW_DMA_TRIG_SRC2_SRC_RXTX;
                                        }
                                }
                        } else {
                                /*
                        ...
                        if (max_path_num == MAX_NUM_OF_PATHS) {
                                ...
                                if (param->stereo) {
                                         ...
                                         */
                                        if ( paths[path_num].dev_in->device_type != AUDIO_MEMORY ) {
                                                channel_setup.dma_req_mux = HW_DMA_TRIG_SRC2_RXRX;
                                        }
                                         ...
                                         */
                                        if ( paths[path_num].dev_out->device_type != AUDIO_MEMORY ) {
                                                channel_setup.dma_req_mux = HW_DMA_TRIG_SRC2_TXTX;
                                        }
                                } else {
                                        /* For 1 audio channel (mono) */
                                        if (paths[path_num].dev_out->device_type ==
                                                 ...
                                                 * Memory-to-Peripheral(SRCx) transfer for the other
                                                 */
                                                channel_setup.dma_req_mux = HW_DMA_TRIG_SRC_SRC2_RXTX;
                                        }
                                }
                        } else {
                                /*
...
# if dg_configUSE_HW_SDADC
                if (paths[path_num].dev_in->device_type == AUDIO_SDADC) {
                        channel_setup.dma_req_mux = HW_DMA_TRIG_SD_ADC_FCU;
                }
                else
# endif /* dg_configUSE_HW_SDADC */
                {
        ...
        for (uint8_t ch = 0; ch < CHANNEL_NUM_MAX; ch++) {
                ...
                if (param->dma_channel[ch] != HW_DMA_CHANNEL_INVALID) {
                        ...
                        if (dir == DMA_DIR_MEM_TO_PERIPH) {
                                ...
                                if (src_config[path_num].id != 0) {
                                        if (channel_setup.dma_req_mux == HW_DMA_TRIG_SRC_RXTX ||
                                                channel_setup.dma_req_mux == HW_DMA_TRIG_SRC2_RXTX  ) {
                                                //Odd channels are only applicable for SRC input
                                ...
                                if (src_config[path_num].id != 0) {
                                        ...
                                        if (channel_setup.dma_req_mux == HW_DMA_TRIG_SRC_RXTX ||
                                                channel_setup.dma_req_mux == HW_DMA_TRIG_SRC2_RXTX  ) {
                                                //Even channels are only applicable for SRC output
...
#if dg_configUSE_HW_SDADC
static void initialize_sdadc_reg(sys_audio_sdadc_specific_t *param)
{
        hw_sdadc_pga_config_t adc_config = {
                .pga_en = HW_SDADC_PGA_ENABLE_BOTH,
                .pga_bias = param->pga_bias,
                .pga_gain = param->pga_gain,
                .pga_mode = param->pga_mode,
        };

        hw_sdadc_audio_init(param->cb, &adc_config);
}
#endif /* dg_configUSE_HW_SDADC */
...
# if dg_configUSE_HW_SDADC
        case AUDIO_SDADC:
                initialize_sdadc_reg(&(dev_id->sdadc_param));
                break;
# endif /* dg_configUSE_HW_SDADC */
        default :
                ASSERT_ERROR(0);
...
static void assert_same_cfg(sys_audio_device_t *dev, uint8_t path_idx)
{
        ...
        for (uint8_t i = 0; i < MAX_NUM_OF_PATHS; i++) {
...
static uint8_t path_index_acquire(void)
{
        ...
        for (path_idx = 0; path_idx < MAX_NUM_OF_PATHS; path_idx++) {
        ...
        ASSERT_ERROR(path_idx < MAX_NUM_OF_PATHS);
...
# if dg_configUSE_HW_SDADC
        if (dev_in->device_type == AUDIO_SDADC) {
                if (!resource_acquire(RES_MASK(RES_ID_SDADC), RES_WAIT_FOREVER)) {
                        ASSERT_ERROR(0);
...
# endif /* dg_configUSE_HW_SDADC */
        ...
        /* Input/Output interface configuration for each data path */
...
static bool is_any_path_active(void)
{
        ...
        for (uint8_t path_idx = 0; path_idx < MAX_NUM_OF_PATHS; path_idx++) {
                if (src_config[path_idx].id) {
                        return true;
                }
        }
        /* Check if there is any device active except from memory */
        for (SYS_AUDIO_MGR_DEVICE dev = AUDIO_PCM; dev < AUDIO_SIZE; dev++) {
...
void sys_audio_mgr_close_path(uint8_t index)
{
        ...
        if (is_any_path_active() == false) {
                ...
                memset(single_dev_type_out, 0, AUDIO_SIZE);
                memset(single_dev_type_in, 0, AUDIO_SIZE);
        }
...
#ifdef OS_PRESENT
# if dg_configUSE_HW_SDADC
        if (path->dev_in->device_type == AUDIO_SDADC) {
