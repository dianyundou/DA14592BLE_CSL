...
/**
 * \brief Micro Trace Buffer mask
 *
 * This value determines the maximum size of the trace buffer in SRAM.
 * A circular buffer of size 2 ^ (dg_configMTB_MASK + 4) in bytes is reserved.
 * The buffer is located at the end of RAM.
 *
 * \note The larger the MTB buffer the less RAM for the application is available.
 *
 * \note Since the total RAM size is limited, dg_configMTB_MASK should certainly
 *       not be set to a value higher than 12 (which corresponds to an MTB buffer
 *       of 64 KB), and depending on the code size of the application, the actual
 *       maximum allowed value may be even lower!
 *
 * \sa dg_configENABLE_MTB
 *
 ...
 *
 * Threads' and IRQ handlers' stack size will be increased by
