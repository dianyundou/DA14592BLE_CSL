...
__RETAINED static const ad_ble_interface_t *adapter_if;    // adapter interface
__RETAINED static ble_mgr_interface_t mgr_if;              // manager interface
__RETAINED static OS_TASK app_task;
__RETAINED static OS_MUTEX ble_interface_lock;

#if (dg_configBLE_SKIP_LATENCY_API == 1)
#endif // (dg_configBLE_SKIP_LATENCY_API == 1)

#ifndef BLE_STACK_PASSTHROUGH_MODE
__RETAINED static OS_MUTEX ble_dev_params_lock;
#if (BLE_MGR_DIRECT_ACCESS == 1)
__RETAINED static OS_MUTEX ble_waitqueue_lock;
 ...
 */
static OS_TASK_FUNCTION(ble_mgr_task, pvParameters)
{
        ad_ble_hdr_t *msg_rx;
                ...
                if (ulNotifiedValue & mainBIT_ADAPTER_EVENT_QUEUE) {
...
#endif /* (BLE_MGR_USE_EVT_LIST == 0) */
                        {
                                ...
                                if (msg_rx->op_code == AD_BLE_OP_CODE_STACK_MSG) {
                                         ...
                                         */
                                        if ((ble_mgr_get_status() == BLE_IS_RESET)) {
...
{
        /* Update task registered to receive BLE events */
        app_task = task;
}
...
OS_BASE_TYPE ble_mgr_event_queue_send(const void *item, OS_TICK_TIME wait_ticks)
{
        ...
        if (in_interrupt()) {
                ...
                #ifdef OS_GET_TASK_SCHEDULER_STATE
                {
                        OS_ASSERT( !( ( OS_GET_TASK_SCHEDULER_STATE() == OS_SCHEDULER_SUSPENDED ) && ( wait_ticks != 0 ) ) );
                }
                #endif
...
void ble_mgr_event_queue_flush(void)
{
        ...
        while (OS_QUEUE_GET(mgr_if.evt_q, &item, OS_QUEUE_NO_WAIT) == OS_QUEUE_OK) {
                ble_msg_free(item);
        }
#else
        ...
        while (mgr_if.evt_q != NULL) {
                /* Pop element from the list */
                struct ble_evt_q_elem *q_elem = list_pop_back(&mgr_if.evt_q);
        ...
        /* Discard pending event notification if called from app_task */
        if (OS_GET_CURRENT_TASK() == app_task) {
                uint32_t notif;
                OS_TASK_NOTIFY_WAIT(OS_TASK_NOTIFY_NONE, mainBIT_MANAGER_EVENT_QUEUE, &notif, OS_TASK_NOTIFY_NO_WAIT);
        }
#endif /* (BLE_MGR_USE_EVT_LIST == 0) */
}
...
OS_BASE_TYPE ble_mgr_event_queue_get(void *item, OS_TICK_TIME wait_ticks)
{
        ...
        if (mgr_if.evt_q == NULL) {
                ...
                OS_TASK_NOTIFY_WAIT(OS_TASK_NOTIFY_NONE, mainBIT_MANAGER_EVENT_QUEUE, &notif, wait_ticks);

                /* Restore app_task to its previous value */
                app_task = app_task_in;
                if (!(notif & mainBIT_MANAGER_EVENT_QUEUE)) {
                        /* Unblocked by another notification bit */
        ...
        /* Clear notification if queue was/is now empty. Covers case of OS_TASK_NOTIFY_WAIT() above being skipped */
        if (mgr_if.evt_q == NULL) {
                OS_TASK_NOTIFY_WAIT(OS_TASK_NOTIFY_NONE, mainBIT_MANAGER_EVENT_QUEUE, &notif, OS_TASK_NOTIFY_NO_WAIT);
        }
...
OS_BASE_TYPE ble_mgr_event_queue_peek(void *item, OS_TICK_TIME wait_ticks)
{
        ...
        if (mgr_if.evt_q == NULL) {
                ...
                app_task = OS_GET_CURRENT_TASK();
                OS_LEAVE_CRITICAL_SECTION();
                if (OS_TASK_NOTIFY_WAIT(OS_TASK_NOTIFY_NONE, mainBIT_MANAGER_EVENT_QUEUE, &notif, wait_ticks) == OS_FAIL) {
                        /* Restore app_task to its previous value */
                        app_task = app_task_in;
                        return OS_QUEUE_EMPTY;
                }
...
{
        ble_status_t status;
        ble_dev_params_t *_ble_dev_params;

        _ble_dev_params = ble_mgr_dev_params_acquire();
        status = _ble_dev_params->status;
