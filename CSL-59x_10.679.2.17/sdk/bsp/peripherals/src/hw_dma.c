...
#define DMA_CHN_REG(reg, chan) ((volatile dma_size_t *)(&(reg)) + ((chan) * ((volatile dma_size_t *)&DMA->DMA1_A_START_REG - (volatile dma_size_t *)&DMA->DMA0_A_START_REG)))
...
{
        if (dma_trigger != HW_DMA_TRIG_PCM_RXTX && dma_trigger != HW_DMA_TRIG_SRC_SRC2_RXTX &&
                dma_trigger != HW_DMA_TRIG_SRC2_SRC_RXTX && dma_trigger != HW_DMA_TRIG_SRC_RXTX &&
...
void hw_dma_channel_initialization(DMA_setup *channel_setup)
{
        ...
        volatile dma_size_t *dma_x_a_start_reg;
        volatile dma_size_t *dma_x_b_start_reg;
        uint32_t src_address;
        uint32_t dest_address;
        dma_size_t req_sense = 0;
        ...
        /* Set REQ_SENSE bit of Uart, I2C TX path */
        if (((channel_setup->dma_req_mux == HW_DMA_TRIG_UART_RXTX) ||
                ...
                (channel_setup->channel_number & 1)) {
                /* odd channels used for TX */
                req_sense = DMA_DMA0_CTRL_REG_REQ_SENSE_Msk;
        }
        ...
        *dma_x_ctrl_reg =
                 ...
                 * That is when DREQ_MODE = 0. For the rest of the cases this setting is overruled by the HW. */
                REG_MSK(DMA, DMA0_CTRL_REG, DMA_EXCLUSIVE_ACCESS);
        ...
        src_address = hw_sys_get_physical_addr(channel_setup->src_address);
        dest_address = hw_sys_get_physical_addr(channel_setup->dest_address);
...
{
        uint32_t phy_addr = hw_sys_get_physical_addr((uint32_t) addr);

        dma_callbacks_user_data[channel].callback = cb;

        // Look up DMAx_A_START_REG address
        volatile dma_size_t *dma_x_a_start_reg = DMA_CHN_REG(DMA->DMA0_A_START_REG, channel);
                                                                        ...
                                                                        hw_dma_transfer_cb cb)
{
        ...
        // Look up DMAx_B_START_REG address
        volatile dma_size_t *dma_x_b_start_reg = DMA_CHN_REG(DMA->DMA0_B_START_REG, channel);
...
{
        volatile const dma_size_t *dma_x_idx_reg = DMA_CHN_REG(DMA->DMA0_IDX_REG, channel_number);
        return *dma_x_idx_reg;
}
