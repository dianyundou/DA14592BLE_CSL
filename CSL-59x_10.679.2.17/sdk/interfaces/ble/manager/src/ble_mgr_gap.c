...
static void send_gapm_cancel_cmd(uint8_t operation)
{
        ble_mgr_common_stack_msg_t *gmsg;
        struct gapm_cancel_cmd *gcmd;

        /* Cancel the requested air operation. GAPM_CANCEL operation cancels *any* ongoing air operation. */
...
#if BLE_SSP_DEBUG
static void send_ltk_evt(uint16_t conn_idx)
{
        ble_evt_gap_ltk_t *evt = ble_evt_init(BLE_EVT_GAP_LTK, sizeof(*evt));
        device_t *dev;
        ...
        if (dev && dev->remote_ltk) {
                memcpy(&evt->ltk.key, dev->remote_ltk->key, sizeof(evt->ltk.key));
        }
        storage_release();
...
__STATIC_INLINE gap_sec_level_t auth_2_sec_level(uint8_t auth, uint8_t key_size)
{
        return (auth & GAP_AUTH_MITM) ?
                (((auth & GAP_AUTH_SEC) && (key_size == BLE_ENC_KEY_SIZE_MAX)) ?
                        GAP_SEC_LEVEL_4 : GAP_SEC_LEVEL_3) :
                GAP_SEC_LEVEL_2;
}
...
static void ble_mgr_gap_conn_cleanup(uint16_t conn_idx, uint8_t reason)
{
        ...
        if (dev) {
                pending_events_clear_handles(dev);
                ...
                if (dev->bonded) {
                        ...
                        dev->sec_level_req = GAP_SEC_LEVEL_1;
                        dev->discon_reason = 0x00;
                        app_value_remove_np(dev);
                } else {
                        device_remove(dev);
                }

                /* Flush any wait queue elements corresponding to this connection */
                ble_gtl_waitqueue_flush(conn_idx);
                ...
                /* Send disconnection event to event queue */
                ble_mgr_event_queue_send(&evt, OS_QUEUE_FOREVER);
        } else {
                /* Device not found in storage, nothing to cleanup */
        }
...
{
        ble_evt_gap_air_op_bdaddr_t *evt;
        ...
        /* Create new event and fill it */
        evt = ble_evt_init(BLE_EVT_GAP_AIR_OP_BDADDR, sizeof(*evt));

        memcpy(&evt->address.addr, ble_dev_params->own_addr.addr, BD_ADDR_LEN);
        evt->address.addr_type = ble_dev_params->own_addr.addr_type;
...
void ble_mgr_gap_adv_report_evt_handler(ble_gtl_msg_t *gtl)
{
        ...
        evt->rssi = (int8_t) gevt->report.rssi;
#if (dg_configBLE_PRIVACY_1_2 == 1)
...
static void gapm_address_resolve_complete(ble_gtl_msg_t *gtl, void *param)
{
        ...
        if (!dev) {
                ...
                /* Send the BLE_EVT_GAP_CONNECTED event to the application */
                ble_mgr_event_queue_send(&evt, OS_QUEUE_FOREVER);

                /* Clear the resolving flag */
                dev->resolving = false;

                if (dev->discon_reason) {
                        /* Device has disconnected while address resolution was ongoing,
                         * cleanup the connection */
                        ble_mgr_gap_conn_cleanup(conn_idx, dev->discon_reason);
                } else {
                        /* Send GAPC_CONNECTION_CFM to the BLE Host */
                        gmsg = ble_gtl_alloc_with_conn(GAPC_CONNECTION_CFM, TASK_ID_GAPC, conn_idx,
                                sizeof(*gcmd));
...
void ble_mgr_gap_peer_features_ind_evt_handler(ble_gtl_msg_t *gtl)
{
        ...
        ble_evt_gap_peer_features_t *evt;

        /* Create new event, fill it and send to application */
        evt = ble_evt_init(BLE_EVT_GAP_PEER_FEATURES, sizeof(*evt));
        evt->conn_idx = TASK_2_CONNIDX(gtl->src_id);
        memcpy(evt->le_features, gevt->features, LE_FEATS_LEN);
        ble_mgr_event_queue_send(&evt, OS_QUEUE_FOREVER);
...
{
        device_t *dev;
        ble_evt_gap_peer_version_t *evt;
        struct gapc_peer_version_ind *gevt = (void *) gtl->param;

        /* Create new event, fill it and send to application */
        evt = ble_evt_init(BLE_EVT_GAP_PEER_VERSION, sizeof(*evt));
        evt->conn_idx = TASK_2_CONNIDX(gtl->src_id);
        evt->lmp_version = gevt->lmp_vers;
        evt->company_id = gevt->compid;
        evt->lmp_subversion = gevt->lmp_subvers;
        ble_mgr_event_queue_send(&evt, OS_QUEUE_FOREVER);
...
void ble_mgr_gap_connected_evt_handler(ble_gtl_msg_t *gtl)
{
        ...
        dev->tx_phy = BLE_GAP_PHY_1M;
        dev->rx_phy = BLE_GAP_PHY_1M;
...
void ble_mgr_gap_adv_stop_cmd_handler(void *param)
{
        ...
        send_gapm_cancel_cmd(GAPM_CANCEL_ADVERTISE);
...
void ble_mgr_gap_adv_data_set_cmd_handler(void *param)
{
        ...
        if (ble_dev_params->advertising == true) {
                ...
                if (((ble_dev_params->adv_type == GAP_CONN_MODE_NON_CONN) && (cmd->adv_data_len > BLE_NON_CONN_ADV_DATA_LEN_MAX)) ||
                        ...
                        /* Setup GTL message */
                        ble_mgr_common_stack_msg_t *gmsg = ble_gtl_alloc(GAPM_UPDATE_ADVERTISE_DATA_CMD,
                                                                         TASK_ID_GAPM, sizeof(*gcmd));
...
void ble_mgr_gap_adv_set_perm_id_cmd_handler(void *param)
{
        const ble_mgr_gap_adv_set_perm_id_cmd_t *cmd = param;
        ble_mgr_gap_adv_set_perm_id_rsp_t *rsp;
        ble_mgr_common_stack_msg_t *gmsg;
        struct gapm_set_adv_perm_cmd *gcmd;
        ble_error_t ret = BLE_ERROR_FAILED;

        if (cmd->permutation_index > 5) {
            ret = BLE_ERROR_INVALID_PARAM;
            goto done;
        }
        /* Setup GTL message */
        gmsg = ble_gtl_alloc(GAPM_SET_ADV_PERMUTATION_CMD, TASK_ID_GAPM, sizeof(*gcmd));
        gcmd = (struct gapm_set_adv_perm_cmd *) gmsg->msg.gtl.param;
        gcmd->operation = GAPM_SET_ADV_PERMUTATION;
        gcmd->permutation_index = cmd->permutation_index;
        ...
        ble_msg_free(param);
        rsp = ble_msg_init(BLE_MGR_GAP_ADV_SET_PERMUTATION_CMD, sizeof(*rsp));
        rsp->status = ret;
        ble_mgr_response_queue_send(&rsp, OS_QUEUE_FOREVER);
}
...
void ble_mgr_gap_scan_stop_cmd_handler(void *param)
{
        ...
        send_gapm_cancel_cmd(GAPM_CANCEL_SCAN);
...
void ble_mgr_gapm_connect_cmp_evt_handler(ble_gtl_msg_t *gtl)
{
        ...
        if (gevt->status != GAP_ERR_NO_ERROR) {
                ...
                device_t *dev = find_device(match_connecting_dev, NULL);
                if (dev) {
                        /* Reset device connecting flag */
                        dev->connecting = false;

                        /* Remove device from storage if not bonded */
                        if (!dev->bonded) {
                                device_remove(dev);
                        }
                }
...
void ble_mgr_gap_disconnected_evt_handler(ble_gtl_msg_t *gtl)
{
        ...
        } else if (dev->resolving) {
                /* Address resolution has not been completed, do the cleanup when it completes */
                dev->discon_reason = gevt->reason;
        } else {
                /* Cleanup the connection and send the disconnection event to the application */
                ble_mgr_gap_conn_cleanup(conn_idx, gevt->reason);
        }
...
void ble_mgr_gap_peer_version_get_cmd_handler(void *param)
{
        const ble_mgr_gap_peer_version_get_cmd_t *cmd = param;
        ble_mgr_gap_peer_version_get_rsp_t *rsp;
        ble_error_t ret = BLE_ERROR_FAILED;
        device_t *dev;
        ...
        if (!dev) {
                ...
                /* Send get peer version command to the stack */
                get_peer_version(cmd->conn_idx);
        ...
        ble_msg_free(param);
        rsp = ble_msg_init(BLE_MGR_GAP_PEER_VERSION_GET_CMD, sizeof(*rsp));
        rsp->status = ret;
        ble_mgr_response_queue_send(&rsp, OS_QUEUE_FOREVER);
}

void ble_mgr_gap_peer_features_get_cmd_handler(void *param)
{
        const ble_mgr_gap_peer_features_get_cmd_t *cmd = param;
        ble_mgr_gap_peer_features_get_rsp_t *rsp;
        ble_error_t ret = BLE_ERROR_FAILED;
        device_t *dev;
        ...
        if (!dev) {
                ...
                /* Send get peer features command to the stack */
                get_peer_features(cmd->conn_idx);
        ...
        ble_msg_free(param);
        rsp = ble_msg_init(BLE_MGR_GAP_PEER_FEATURES_GET_CMD, sizeof(*rsp));
        rsp->status = ret;
        ble_mgr_response_queue_send(&rsp, OS_QUEUE_FOREVER);
}

static void gap_get_con_rssi_complete(ble_gtl_msg_t *gtl, void *param)
{
        ble_mgr_gap_conn_rssi_get_rsp_t *rsp = ble_msg_init(BLE_MGR_GAP_CONN_RSSI_GET_CMD, sizeof(*rsp));

        /* Initiate the RSSI value with an invalid value */
        rsp->conn_rssi = BLE_RSSI_NOT_AVAILABLE;
        ...
        if (gtl != NULL) {
                ...
                case GAP_ERR_NO_ERROR:
                        rsp->status = BLE_STATUS_OK;
                        /* Retrieve the RSSI reading reported through the GAPC_CON_RSSI_IND event */
                        ble_dev_params_t *ble_dev_params = ble_mgr_dev_params_acquire();
                        rsp->conn_rssi = ble_dev_params->conn_rssi;
                        ble_mgr_dev_params_release();
                        break;
                case GAP_ERR_COMMAND_DISALLOWED:
                ...
                /* Waitqueue has been flushed due to disconnection */
...
{
        struct gapc_con_rssi_ind *gevt = (void *) gtl->param;

        /* Store the reported RSSI to ble_dev_params */
        ble_dev_params_t *ble_dev_params = ble_mgr_dev_params_acquire();
        ble_dev_params->conn_rssi = gevt->rssi;
        ble_mgr_dev_params_release();
}
...
void ble_mgr_gap_conn_rssi_get_cmd_handler(void *param)
{
        ...
        /* Wait for the GAPC_CMP_EVT event. If the relevant status is succesfull, a
         * GAPC_CON_RSSI_IND event should have been generated before GAPC_CMP_EVT
         * has been received. The RSSI value which is reported by GAPC_CON_RSSI_IND
         * will be temporarily stored in ble_dev_params and will be retrieved when the
         * GAPC_CMP_EVT is received.  */
        ble_gtl_waitqueue_add(conn_idx, GAPC_CMP_EVT, GAPC_GET_CON_RSSI,
                                     gap_get_con_rssi_complete, (void *) NULL);
...
void ble_mgr_gap_conn_param_update_cmd_handler(void *param)
{
        ...
        /* Set the device updating flag to prevent starting another update while one is ongoing */
        dev->updating = true;
...
static void send_bond_cmd(uint16_t conn_idx, gap_io_cap_t io_cap, bool bond, bool mitm, bool secure)
{
        ...
        device_t *dev;
        gap_sec_level_t sec_level_req = GAP_SEC_LEVEL_1;
        ...
        gcmd->pairing.key_size = BLE_ENC_KEY_SIZE_MAX;
        gcmd->pairing.ikey_dist = dg_configBLE_PAIR_INIT_KEY_DIST;
        gcmd->pairing.rkey_dist = dg_configBLE_PAIR_RESP_KEY_DIST;
        gcmd->pairing.sec_req = (sec_level_req == GAP_SEC_LEVEL_4) ? GAP_SEC1_SEC_PAIR_ENC :
                                (sec_level_req == GAP_SEC_LEVEL_3) ? GAP_SEC1_AUTH_PAIR_ENC :
                                (sec_level_req == GAP_SEC_LEVEL_2) ? GAP_SEC1_NOAUTH_PAIR_ENC :
                                GAP_NO_SEC;
        ...
        if (master) {
                ...
                if (dev) {
                        if (dev->security_req_pending) {
                                status = BLE_ERROR_IN_PROGRESS;
                        } else {
                                dev->security_req_pending = true;
                                send_security_req(conn_idx, bond, io_cap == GAP_IO_CAP_NO_INPUT_OUTPUT ? false : true, secure);
                                status = BLE_STATUS_OK;
                        }
                } else {
                        status = BLE_ERROR_NOT_CONNECTED;
...
void ble_mgr_gap_pair_reply_cmd_handler(void *param)
{
        ...
        gcmd->data.pairing_feat.key_size = BLE_ENC_KEY_SIZE_MAX;
        ...
        gcmd->data.pairing_feat.sec_req = (sec_level_req == GAP_SEC_LEVEL_4) ? GAP_SEC1_SEC_PAIR_ENC :
                                          (sec_level_req == GAP_SEC_LEVEL_3) ? GAP_SEC1_AUTH_PAIR_ENC :
                                          (sec_level_req == GAP_SEC_LEVEL_2) ? GAP_SEC1_NOAUTH_PAIR_ENC :
                                          GAP_NO_SEC;
...
void ble_mgr_gap_bond_ind_evt_handler(ble_gtl_msg_t *gtl)
{
        ...
        case GAPC_PAIRING_SUCCEED:
        {
                ...
                dev = find_device_by_conn_idx(TASK_2_CONNIDX(gtl->src_id));
                if (dev) {
                        gap_sec_level_t sec_level = auth_2_sec_level(ind->data.auth, dev->remote_ltk->key_size);
                        dev->paired = true;
                        dev->bonded = evt->bond;
                        dev->encrypted = true;
                        dev->mitm = evt->mitm;
        ...
        case GAPC_PAIRING_FAILED:
        {
                ...
                case SMP_ERROR_REM_PAIRING_NOT_SUPP:
                        ...
                        break;
                case SMP_ERROR_REM_UNSPECIFIED_REASON:
                case SMP_ERROR_LOC_UNSPECIFIED_REASON:
                        evt->status = BLE_ERROR_FAILED;
                        break;
                case SMP_ERROR_TIMEOUT:
                        evt->status = BLE_ERROR_TIMEOUT;
                        break;
                default:
...
static bool encrypt_conn_using_ltk(uint16_t conn_idx, uint8_t auth, gap_sec_level_t sec_level_req,
                                   void *param)
{
        ble_mgr_common_stack_msg_t *gmsg;
...
#if (dg_configBLE_SECURE_CONNECTIONS == 1)
        if ((sec_level_req == GAP_SEC_LEVEL_4) &&
                ((dev->secure == false) ||
                (dev->mitm == false) ||
                (dev->remote_ltk->key_size != BLE_ENC_KEY_SIZE_MAX))) {
                storage_release();
                return false;
        }
#endif /* (dg_configBLE_SECURE_CONNECTIONS == 1) */
...
void ble_mgr_gap_security_ind_evt_handler(ble_gtl_msg_t *gtl)
{
        ...
        if (dev) {
                ...
                if (!encrypt_conn_using_ltk(conn_idx, ind->auth, dev->sec_level_req, evt)) {
                        /* Send to event queue */
                        ble_mgr_event_queue_send(&evt, OS_QUEUE_FOREVER);
                }
        }
...
void ble_mgr_gap_update_remote_ltk(uint8_t *key, uint8_t key_size);
...
void ble_mgr_gap_encrypt_req_ind_evt_handler(ble_gtl_msg_t *gtl)
{
        ...
        if ((!dev->bonded) || (!ind->ediv)) {
                ...
                if (!dev->remote_ltk) {
                        goto done;
                } else if ((ind->ediv != 0) || memcmp(&ind->rand_nb.nb, zeros, RAND_NB_LEN)) {
                        // We need to disconnect after rejecting the encryption request
                        disconnect = true;
                        goto done;
                }

                ble_mgr_gap_update_remote_ltk(dev->remote_ltk->key, sizeof(gcmd->ltk.key));
        ...
        if (disconnect == true) {
                send_gapc_disconnect_cmd(TASK_2_CONNIDX(gtl->src_id), BLE_HCI_ERROR_REMOTE_USER_TERM_CON);
        }
}

__WEAK void ble_mgr_gap_update_remote_ltk(uint8_t *key, uint8_t key_size)
{
        (void) key;
        (void) key_size;
}
...
void ble_mgr_gap_encrypt_ind_evt_handler(ble_gtl_msg_t *gtl)
{
        ...
        if (dev) {
                ...
                if (dev->paired
                        && (dev->sec_level != auth_2_sec_level(ind->auth, dev->remote_ltk->key_size))
                        && (ind->auth != 0x00)) {
                        /* Update device security level */
                        dev->sec_level = auth_2_sec_level(ind->auth, dev->remote_ltk->key_size);
                        ...
                        send_sec_level_changed_evt(TASK_2_CONNIDX(gtl->src_id),
                                auth_2_sec_level(ind->auth, dev->remote_ltk->key_size));
                }
        }
...
void ble_mgr_gapc_cmp__update_params_evt_handler(ble_gtl_msg_t *gtl)
{
        ...
        dev = find_device_by_conn_idx(conn_idx);
        if (dev) {
                /* Reset the device updating flag, the update is completed */
                dev->updating = false;
        }
        storage_release();
        ...
        case LL_ERR_LMP_COLLISION:
                evt->status = BLE_ERROR_LMP_COLLISION;
                break;
        case LL_ERR_DIFF_TRANSACTION_COLLISION:
                evt->status = BLE_ERROR_DIFF_TRANS_COLLISION;
                break;
        default:
                evt->status = gevt->status;
                break;
...
void ble_mgr_gap_set_sec_level_cmd_handler(void *param)
{
...
#if (dg_configBLE_SECURE_CONNECTIONS == 0)
        if (level == GAP_SEC_LEVEL_4) {
                status = BLE_ERROR_NOT_SUPPORTED;
                goto done;
        }
#endif /* (dg_configBLE_SECURE_CONNECTIONS == 0) */
        ...
        if (master) {
                ...
                evt->conn_idx = conn_idx;
                uint8_t auth = (mitm ? GAP_AUTH_MITM : 0) | (secure ? GAP_AUTH_SEC : 0);

                if (!encrypt_conn_using_ltk(conn_idx, auth, level, evt)) {
                        /* Free not needed event buffer */
                        ble_msg_free(evt);

                        dev->sec_level_req = level;
...
void ble_mgr_gap_cmp__data_length_set_evt_handler(ble_gtl_msg_t *gtl)
{
        ...
        if (gevt->operation == GAPM_SET_SUGGESTED_DFLT_LE_DATA_LEN) {
                evt->conn_idx = BLE_CONN_IDX_INVALID;
        } else {
                evt->conn_idx = TASK_2_CONNIDX(gtl->src_id);
        }
...
static void ble_mgr_gap_ral_sync_complete(ble_gtl_msg_t *gtl, void *param)
{
        ...
        ble_gtl_waitqueue_add(BLE_CONN_IDX_INVALID, GAPM_CMP_EVT, GAPM_ADD_DEV_IN_RAL,
                              ble_mgr_gap_ral_sync_complete, param);
        ble_gtl_send(gmsg);
}
...
static void ble_mgr_gap_ral_sync(ble_mgr_cmd_handler_t handler, void *param)
{
        ...
        ble_gtl_waitqueue_add(BLE_CONN_IDX_INVALID, GAPM_CMP_EVT, GAPM_CLEAR_RAL,
                              ble_mgr_gap_ral_sync_complete, ral_sync_param);
        ble_gtl_send(gmsg);
}
#endif /* (dg_configBLE_PRIVACY_1_2 == 1) */

#if (dg_configBLE_2MBIT_PHY == 1)
static void ble_mgr_gap_set_default_phy_complete(ble_gtl_msg_t *gtl, void *param)
{
        struct gapm_cmp_evt *gevt = (void *) gtl->param;
        ble_dev_params_t *ble_dev_params = ble_mgr_dev_params_acquire();

        if (gevt->status == GAP_ERR_NO_ERROR) {
                ble_mgr_gap_phy_set_cmd_t *cmd = param;

                /* Update default PHY preferences for new connections */
                ble_dev_params->tx_phy_pref_default = cmd->tx_phy;
                ble_dev_params->rx_phy_pref_default = cmd->rx_phy;
        }
        ...
        /* Create new event and fill it */
        ble_evt_gap_phy_set_completed_t *evt = ble_evt_init(BLE_EVT_GAP_PHY_SET_COMPLETED, sizeof(*evt));
        evt->conn_idx = TASK_2_CONNIDX(gtl->src_id);
        ...
        case LL_ERR_COMMAND_DISALLOWED:
                evt->status = BLE_ERROR_NOT_ALLOWED;
                break;
        case LL_ERR_UNKNOWN_CONNECTION_ID:
                evt->status = BLE_ERROR_NOT_CONNECTED;
                break;
        default:
                evt->status = gevt->status;
        ...
        /* Reset phy_set_pending flag */
        ble_dev_params->phy_set_pending = false;

        ble_mgr_dev_params_release();
}

static void ble_mgr_gap_set_phy_complete(ble_gtl_msg_t *gtl, void *param)
{
        if (gtl == NULL) {
                /* Called by ble_gtl_waitqueue_flush(), just free param buffer */
                goto flush;
        }
...
void ble_mgr_gap_phy_set_cmd_handler(void *param)
{
        const ble_mgr_gap_phy_set_cmd_t *cmd = param;
        ble_mgr_gap_phy_set_rsp_t *rsp;
        ble_mgr_common_stack_msg_t *gmsg;
        ble_error_t status = BLE_ERROR_FAILED;
        ble_dev_params_t *ble_dev_params;

        /* Check if PHY values provided are within the supported range */
        if (cmd->tx_phy & BLE_GAP_PHY_PREF_CODED) {
                /*
                 * Only BLE_GAP_PHY_PREF_AUTO, BLE_GAP_PHY_PREF_1M and BLE_GAP_PHY_PREF_2M options
                 * are supported.
                 */
                status = BLE_ERROR_NOT_SUPPORTED;
                ble_msg_free(param);
        ...
        /* Set phy_set_pending flag */
        ble_dev_params = ble_mgr_dev_params_acquire();
        ble_dev_params->phy_set_pending = true;
        ble_mgr_dev_params_release();

        if (cmd->conn_idx == BLE_CONN_IDX_INVALID) {
                /* Set preferred PHY for future connections */
                struct gapm_le_set_default_phy_cmd *gcmd;

                gmsg = ble_gtl_alloc(GAPM_SET_DEFAULT_PHY_CMD, TASK_ID_GAPM,
                                                        sizeof(struct gapm_le_set_default_phy_cmd));
                gcmd = (struct gapm_le_set_default_phy_cmd *) gmsg->msg.gtl.param;
                gcmd->operation = GAPM_SET_DEFAULT_PHY;

                if (cmd->tx_phy == BLE_GAP_PHY_PREF_AUTO) {
                        gcmd->all_phys = GAPC_NO_PREFERRED_TX;
                } else {
                        gcmd->tx_phys = cmd->tx_phy;
                }

                if (cmd->rx_phy == BLE_GAP_PHY_PREF_AUTO) {
                        gcmd->all_phys |= GAPC_NO_PREFERRED_RX;
                } else {
                        gcmd->rx_phys = cmd->rx_phy;
                }

                ble_gtl_waitqueue_add(BLE_CONN_IDX_INVALID, GAPM_CMP_EVT, GAPM_SET_DEFAULT_PHY,
                        ble_mgr_gap_set_default_phy_complete, param);
                /* param will be freed in ble_mgr_gap_set_default_phy_complete */
        } else {
                /* Set PHY for specified connection */
                struct gapc_le_set_phy_cmd *gcmd;

                storage_acquire();
                device_t *dev = find_device_by_conn_idx(cmd->conn_idx);
                ...
                /* Check if a change in PHY is requested */
                if ( !( ( (cmd->tx_phy == BLE_GAP_PHY_PREF_AUTO)
                                || (cmd->tx_phy & ((uint8_t) dev->tx_phy) ) )
                        && ( (cmd->rx_phy == BLE_GAP_PHY_PREF_AUTO)
                                || (cmd->rx_phy & ((uint8_t) dev->rx_phy) ) ) ) ) {
                        ble_dev_params = ble_mgr_dev_params_acquire();
                        ble_dev_params->phy_change_req = true;
                        ble_mgr_dev_params_release();
                }
                storage_release();

                gmsg = ble_gtl_alloc_with_conn(GAPC_LE_SET_PHY_CMD, TASK_ID_GAPC, cmd->conn_idx,
                                                                                     sizeof(*gcmd));
                gcmd = (struct gapc_le_set_phy_cmd *) gmsg->msg.gtl.param;
                gcmd->operation = GAPC_LE_SET_PHY;
                ...
                ble_gtl_waitqueue_add(cmd->conn_idx, GAPC_CMP_EVT, GAPC_LE_SET_PHY,
                        ble_mgr_gap_set_phy_complete, param);
                /* param will be freed in ble_mgr_gap_set_phy_complete */
        }
        ...
        rsp = ble_msg_init(BLE_MGR_GAP_PHY_SET_CMD, sizeof(*rsp));
        rsp->status = status;
        ble_mgr_response_queue_send(&rsp, OS_QUEUE_FOREVER);
}
...
{
        device_t *dev;
        struct gapc_le_phy_ind *gevt = (void *) gtl->param;
        ble_dev_params_t *ble_dev_params = ble_mgr_dev_params_acquire();
        ...
        phy_changed = ( (dev->tx_phy != (ble_gap_phy_t) gevt->tx_phy)
                || (dev->rx_phy != (ble_gap_phy_t) gevt->rx_phy) );

        /* Check if a BLE_EVT_GAP_SET_COMPLETED event should be sent to the application */
        if (ble_dev_params->phy_set_pending) {
                ble_evt_gap_phy_set_completed_t *evt;
                ble_dev_params->phy_set_pending = false;

                /* Create new event and fill it */
                evt = ble_evt_init(BLE_EVT_GAP_PHY_SET_COMPLETED, sizeof(*evt));
                evt->conn_idx = TASK_2_CONNIDX(gtl->src_id);

                if (gevt->status == GAP_ERR_NO_ERROR) {
                        if (ble_dev_params->phy_change_req && !phy_changed) {
                ...
                ble_dev_params->phy_change_req = false;
        ...
        if (phy_changed) {
                ble_evt_gap_phy_changed_t *evt;

                /* Update connected device attributes */
                dev->tx_phy = gevt->tx_phy;
                dev->rx_phy = gevt->rx_phy;

                /* Create new event and fill it */
                evt = ble_evt_init(BLE_EVT_GAP_PHY_CHANGED, sizeof(*evt));
                evt->conn_idx = TASK_2_CONNIDX(gtl->src_id);
                evt->tx_phy   = (ble_gap_phy_t) gevt->tx_phy;
                evt->rx_phy   = (ble_gap_phy_t) gevt->rx_phy;
...
void ble_mgr_gap_tx_power_set_cmd_handler(void *param)
{
        const ble_mgr_gap_tx_power_set_cmd_t *cmd = param;
        ble_mgr_gap_tx_power_set_rsp_t *rsp;
        ble_mgr_common_stack_msg_t *gmsg;
        struct gapm_set_tx_pw_cmd *gcmd;
        ble_error_t ret = BLE_ERROR_FAILED;

        if (!(cmd->air_operation & (GAP_AIR_OP_ADV | GAP_AIR_OP_SCAN | GAP_AIR_OP_INITIATE))) {
                /* No valid air operation is defined */
                ret = BLE_ERROR_INVALID_PARAM;
                goto done;
        }

        if (cmd->tx_power > GAP_TX_POWER_MAX) {
                /* TX power level does not correspond to any of @gap_tx_power_t values */
                ret = BLE_ERROR_INVALID_PARAM;
                goto done;
        }

        /* Setup GTL message */
        gmsg = ble_gtl_alloc(GAPM_SET_TX_PW_CMD, TASK_ID_GAPM, sizeof(*gcmd));
        gcmd = (struct gapm_set_tx_pw_cmd *) gmsg->msg.gtl.param;
        gcmd->operation = GAPM_SET_TX_PW;
        gcmd->air_operation_type = cmd->air_operation;
        gcmd->pw_lvl = (uint8_t) cmd->tx_power;
        ...
        ble_msg_free(param);
        rsp = ble_msg_init(BLE_MGR_GAP_TX_POWER_SET_CMD, sizeof(*rsp));
        rsp->status = ret;
        ble_mgr_response_queue_send(&rsp, OS_QUEUE_FOREVER);
}

void ble_mgr_gap_conn_tx_power_set_cmd_handler(void *param)
{
        const ble_mgr_gap_conn_tx_power_set_cmd_t *cmd = param;
        ble_mgr_gap_conn_tx_power_set_rsp_t *rsp;
        ble_mgr_common_stack_msg_t *gmsg;
        struct gapc_set_tx_pwr_cmd *gcmd;
        ble_error_t ret = BLE_ERROR_FAILED;
        device_t *dev;
        uint16_t conn_idx = cmd->conn_idx;
        ...
        /* Setup GTL message */
        gmsg = ble_gtl_alloc_with_conn(GAPC_SET_TX_PWR_CMD, TASK_ID_GAPC, conn_idx, sizeof(*gcmd));
        gcmd = (struct gapc_set_tx_pwr_cmd *) gmsg->msg.gtl.param;
        gcmd->operation = GAPC_SET_TX_PWR;
        gcmd->pwr_lvl = (uint8_t) cmd->tx_power;
        ...
        ble_msg_free(param);
        rsp = ble_msg_init(BLE_MGR_GAP_CONN_TX_POWER_SET_CMD, sizeof(*rsp));
        rsp->status = ret;
        ble_mgr_response_queue_send(&rsp, OS_QUEUE_FOREVER);
}

#ifdef F_PCLE
static void gapc_rd_tx_pwr_lvl_enh_rsp(ble_gtl_msg_t *gtl, void *param)
{
        ble_mgr_gap_local_tx_power_get_rsp_t *rsp =
                ble_msg_init(BLE_MGR_GAP_LOCAL_TX_POWER_GET_CMD, sizeof(*rsp));
...
void ble_mgr_gap_local_tx_power_get_cmd_handler(void *param)
{
        const ble_mgr_gap_local_tx_power_get_cmd_t *cmd = param;
        ble_mgr_gap_local_tx_power_get_rsp_t *rsp;
        ble_mgr_common_stack_msg_t *gmsg;
        struct gapc_le_rd_tx_pwr_lvl_enh_cmd *gcmd;
        /* cppcheck-suppress unreadVariable */
        ble_error_t ret = BLE_ERROR_FAILED;
        device_t *dev;
        uint16_t conn_idx = cmd->conn_idx;

        if ((cmd->phy != BLE_GAP_PHY_1M) && (cmd->phy != BLE_GAP_PHY_2M)) {
                /* Requested PHY not supported */
                ret = BLE_ERROR_NOT_SUPPORTED;
                goto done;
        }
        ...
        /* Setup GTL message */
        gmsg = ble_gtl_alloc_with_conn(GAPC_LE_RD_TX_PWR_LVL_ENH_CMD, TASK_ID_GAPC, conn_idx, sizeof(*gcmd));
        gcmd = (struct gapc_le_rd_tx_pwr_lvl_enh_cmd *) gmsg->msg.gtl.param;
        gcmd->operation = GAPC_LE_RD_TX_PWR_LVL_ENH;
        gcmd->phy = cmd->phy;

        ble_gtl_waitqueue_add(conn_idx, GAPC_CMP_EVT, GAPC_LE_RD_TX_PWR_LVL_ENH,
                              gapc_rd_tx_pwr_lvl_enh_rsp, NULL);

        ble_gtl_send(gmsg);

        /* Free command buffer, it is no longer needed */
        ble_msg_free(param);

        /* Response to the application will be sent after GAPC_CMP_EVT is received, return */
        return;
done:
        /* Send response in case of command completion (no message sent to the stack) */
        ble_msg_free(param);
        rsp = ble_msg_init(BLE_MGR_GAP_LOCAL_TX_POWER_GET_CMD, sizeof(*rsp));
        rsp->status = ret;
        ble_mgr_response_queue_send(&rsp, OS_QUEUE_FOREVER);
}

static void gapc_rd_rem_tx_pwr_lvl_rsp(ble_gtl_msg_t *gtl, void *param)
{
        ble_mgr_gap_rd_rem_tx_pwr_lvl_rsp_t *rsp =
                ble_msg_init(BLE_MGR_GAP_REMOTE_TX_POWER_GET_CMD, sizeof(*rsp));
...
void ble_mgr_gap_rd_rem_tx_pwr_lvl_cmd_handler(void *param)
{
        const ble_mgr_gap_rd_rem_tx_pwr_lvl_cmd_t *cmd = param;
        ble_mgr_gap_rd_rem_tx_pwr_lvl_rsp_t *rsp;
        ble_mgr_common_stack_msg_t *gmsg;
        struct gapc_le_rd_rem_tx_pwr_lvl_cmd *gcmd;
        /* cppcheck-suppress unreadVariable */
        ble_error_t ret = BLE_ERROR_FAILED;
        device_t *dev;
        uint16_t conn_idx = cmd->conn_idx;
        ...
        /* Setup GTL message */
        gmsg = ble_gtl_alloc_with_conn(GAPC_LE_RD_REM_TX_PWR_LVL_CMD, TASK_ID_GAPC, conn_idx, sizeof(*gcmd));
        gcmd = (struct gapc_le_rd_rem_tx_pwr_lvl_cmd *) gmsg->msg.gtl.param;
        gcmd->operation = GAPC_LE_RD_REM_TX_PWR_LVL;
        gcmd->phy = cmd->phy;

        ble_gtl_waitqueue_add(conn_idx, GAPC_CMP_EVT, GAPC_LE_RD_REM_TX_PWR_LVL,
                              gapc_rd_rem_tx_pwr_lvl_rsp, NULL);
        ...
        /* Send response in case of command completion (no message sent to the stack) */
        ble_msg_free(param);
        rsp = ble_msg_init(BLE_MGR_GAP_REMOTE_TX_POWER_GET_CMD, sizeof(*rsp));
        rsp->status = ret;
        ble_mgr_response_queue_send(&rsp, OS_QUEUE_FOREVER);
}

static void gapc_set_path_loss_report_params_rsp(ble_gtl_msg_t *gtl, void *param)
{
        ble_mgr_gap_set_path_loss_report_params_rsp_t *rsp =
                ble_msg_init(BLE_MGR_GAP_PATH_LOSS_REPORT_PARAMS_SET_CMD, sizeof(*rsp));
...
void ble_mgr_gap_set_path_loss_report_params_cmd_handler(void *param)
{
        const ble_mgr_gap_set_path_loss_report_params_cmd_t *cmd = param;
        ble_mgr_gap_set_path_loss_report_params_rsp_t *rsp;
        ble_mgr_common_stack_msg_t *gmsg;
        struct gapc_le_set_path_loss_report_params_cmd *gcmd;
        /* cppcheck-suppress unreadVariable */
        ble_error_t ret = BLE_ERROR_FAILED;
        device_t *dev;
        uint16_t conn_idx = cmd->conn_idx;

        if ((cmd->high_thres != 0xFF) &&
           (((cmd->high_thres + cmd->high_hyst) > 0xFF) ||
           (cmd->low_thres < cmd->low_hyst) ||
           (cmd->low_thres > cmd->high_thres) ||
           ((cmd->low_thres + cmd->low_hyst) > (cmd->high_thres - cmd->high_hyst)))) {
                /* Given path loss parameters are violating the spec */
                ret = BLE_ERROR_INVALID_PARAM;
                goto done;
        }
        else if ((cmd->high_thres == 0xFF) &&
               (cmd->low_thres < cmd->low_hyst)) {
                /* Given path loss parameters are violating the spec */
                ret = BLE_ERROR_INVALID_PARAM;
                goto done;
        ...
        /* Setup GTL message */
        gmsg = ble_gtl_alloc_with_conn(GAPC_LE_SET_PATH_LOSS_REPORT_PARAMS_CMD, TASK_ID_GAPC, conn_idx, sizeof(*gcmd));
        gcmd = (struct gapc_le_set_path_loss_report_params_cmd *) gmsg->msg.gtl.param;
        gcmd->operation = GAPC_LE_SET_PATH_LOSS_REPORT_PARAMS;
        gcmd->high_thres = cmd->high_thres;
        gcmd->high_hyst = cmd->high_hyst ;
        gcmd->low_thres = cmd->low_thres ;
        gcmd->low_hyst = cmd->low_hyst ;
        gcmd->min_time_spent = cmd->min_time_spent ;

        ble_gtl_waitqueue_add(conn_idx, GAPC_CMP_EVT, GAPC_LE_SET_PATH_LOSS_REPORT_PARAMS,
                              gapc_set_path_loss_report_params_rsp, NULL);
        ...
        /* Send response in case of command completion (no message sent to the stack) */
        ble_msg_free(param);
        rsp = ble_msg_init(BLE_MGR_GAP_PATH_LOSS_REPORT_PARAMS_SET_CMD, sizeof(*rsp));
        rsp->status = ret;
        ble_mgr_response_queue_send(&rsp, OS_QUEUE_FOREVER);
}

static void gapc_set_path_loss_report_en_rsp(ble_gtl_msg_t *gtl, void *param)
{
        ble_mgr_gap_set_path_loss_report_en_rsp_t *rsp =
                ble_msg_init(BLE_MGR_GAP_PATH_LOSS_REPORT_EN_CMD, sizeof(*rsp));
...
void ble_mgr_gap_set_path_loss_report_en_cmd_handler(void *param)
{
        const ble_mgr_gap_set_path_loss_report_en_cmd_t *cmd = param;
        ble_mgr_gap_set_path_loss_report_en_rsp_t *rsp;
        ble_mgr_common_stack_msg_t *gmsg;
        struct gapc_le_set_path_loss_report_en_cmd *gcmd;
        /* cppcheck-suppress unreadVariable */
        ble_error_t ret = BLE_ERROR_FAILED;
        device_t *dev;
        uint16_t conn_idx = cmd->conn_idx;
        ...
        if (cmd->enable > 1) {
                /* Path loss reporting enable parameter is invalid */
                ret = BLE_ERROR_INVALID_PARAM;
                goto done;
        }
        ...
        /* Setup GTL message */
        gmsg = ble_gtl_alloc_with_conn(GAPC_LE_SET_PATH_LOSS_REPORT_EN_CMD, TASK_ID_GAPC, conn_idx, sizeof(*gcmd));
        gcmd = (struct gapc_le_set_path_loss_report_en_cmd *) gmsg->msg.gtl.param;
        gcmd->operation = GAPC_LE_SET_PATH_LOSS_REPORT_EN;
        gcmd->enable = cmd->enable;

        ble_gtl_waitqueue_add(conn_idx, GAPC_CMP_EVT, GAPC_LE_SET_PATH_LOSS_REPORT_EN,
                              gapc_set_path_loss_report_en_rsp, NULL);
        ...
        /* Send response in case of command completion (no message sent to the stack) */
        ble_msg_free(param);
        rsp = ble_msg_init(BLE_MGR_GAP_PATH_LOSS_REPORT_EN_CMD, sizeof(*rsp));
        rsp->status = ret;
        ble_mgr_response_queue_send(&rsp, OS_QUEUE_FOREVER);
}

static void gapc_set_tx_pwr_report_en_rsp(ble_gtl_msg_t *gtl, void *param)
{
        ble_mgr_gap_set_tx_pwr_report_en_rsp_t *rsp =
                ble_msg_init(BLE_MGR_GAP_TX_PWR_REPORT_EN_CMD, sizeof(*rsp));
...
void ble_mgr_gap_set_tx_pwr_report_en_cmd_handler(void *param)
{
        const ble_mgr_gap_set_tx_pwr_report_en_cmd_t *cmd = param;
        ble_mgr_gap_set_tx_pwr_report_en_rsp_t *rsp;
        ble_mgr_common_stack_msg_t *gmsg;
        struct gapc_le_set_tx_pwr_report_en_cmd *gcmd;
        /* cppcheck-suppress unreadVariable */
        ble_error_t ret = BLE_ERROR_FAILED;
        device_t *dev;
        uint16_t conn_idx = cmd->conn_idx;
        ...
        if (cmd->loc_en > 1) {
                /* Local Tx power reporting enable parameter is invalid */
                ret = BLE_ERROR_INVALID_PARAM;
                goto done;
        }
        else if (cmd->rem_en > 1) {
                /* Remote Tx power reporting enable parameter is invalid */
                ret = BLE_ERROR_INVALID_PARAM;
                goto done;
        }
        ...
        /* Setup GTL message */
        gmsg = ble_gtl_alloc_with_conn(GAPC_LE_SET_TX_PWR_REPORT_EN_CMD, TASK_ID_GAPC, conn_idx, sizeof(*gcmd));
        gcmd = (struct gapc_le_set_tx_pwr_report_en_cmd *) gmsg->msg.gtl.param;
        gcmd->operation = GAPC_LE_SET_TX_PWR_REPORT_EN;
        gcmd->loc_en = cmd->loc_en;
        gcmd->rem_en = cmd->rem_en;

       ble_gtl_waitqueue_add(conn_idx, GAPC_CMP_EVT, GAPC_LE_SET_TX_PWR_REPORT_EN,
                             gapc_set_tx_pwr_report_en_rsp, NULL);
        ...
        /* Send response in case of command completion (no message sent to the stack) */
        ble_msg_free(param);
        rsp = ble_msg_init(BLE_MGR_GAP_TX_PWR_REPORT_EN_CMD, sizeof(*rsp));
        rsp->status = ret;
        ble_mgr_response_queue_send(&rsp, OS_QUEUE_FOREVER);
}
...
{
    device_t *dev;
    struct gapc_le_rd_tx_pwr_lvl_enh_ind *gevt = (void *) gtl->param;
    ble_evt_gap_local_tx_pwr_t *evt;
    ...
    /* Create new event and fill it */
    evt = ble_evt_init(BLE_EVT_GAP_LOCAL_TX_PWR, sizeof(*evt));
    evt->conn_idx        = TASK_2_CONNIDX(gtl->src_id);
    evt->status          = gevt->status;
    evt->phy             = gevt->phy;
    evt->curr_tx_pwr_lvl = gevt->curr_tx_pwr_lvl;
    evt->max_tx_pwr_lvl  = gevt->max_tx_pwr_lvl;
...
{
    device_t *dev;
    ble_evt_gap_tx_pwr_report_t *evt;
    struct gapc_le_tx_pwr_lvl_report_ind *gevt = (void *) gtl->param;
    ...
    /* Create new event and fill it */
    evt = ble_evt_init(BLE_EVT_GAP_TX_PWR_REPORT, sizeof(*evt));
    evt->conn_idx = TASK_2_CONNIDX(gtl->src_id);
    evt->status          = gevt->status;
    evt->reason          = gevt->reason;
    evt->phy             = gevt->phy;
    evt->tx_pwr_lvl      = gevt->tx_pwr_lvl;
    evt->tx_pwr_lvl_flag = gevt->tx_pwr_lvl_flag;
    evt->delta           = gevt->delta;
...
{
    device_t *dev;
    ble_evt_gap_path_loss_thres_t *evt;
    struct gapc_le_path_loss_thres_ind *gevt = (void *) gtl->param;
    ...
    /* Create new event and fill it */
    evt = ble_evt_init(BLE_EVT_GAP_PATH_LOSS_THRES, sizeof(*evt));
    evt->conn_idx       = TASK_2_CONNIDX(gtl->src_id);
    evt->curr_path_loss = gevt->curr_path_loss;
    evt->zone_enter     = gevt->zone_enter;
...
void ble_mgr_gap_rf_path_compensation_set_cmd_handler(void *param)
{
        const ble_mgr_gap_rf_path_compensation_set_cmd_t *cmd = param;
        ble_mgr_gap_rf_path_compensation_set_rsp_t *rsp;
        ble_mgr_common_stack_msg_t *gmsg;
        struct gapm_le_wr_rf_path_compens_cmd *gcmd;
        ble_error_t ret = BLE_ERROR_FAILED;

        if (((cmd->rf_tx_path_compens < -1280) || (cmd->rf_tx_path_compens > 1280)) ||
            ((cmd->rf_rx_path_compens < -1280) || (cmd->rf_rx_path_compens > 1280))) {
        ...
        /* Setup GTL message */
        gmsg = ble_gtl_alloc(GAPM_LE_WR_RF_PATH_COMPENS_CMD, TASK_ID_GAPM, sizeof(*gcmd));
        gcmd = (struct gapm_le_wr_rf_path_compens_cmd *) gmsg->msg.gtl.param;
        gcmd->operation = GAPM_LE_WR_RF_PATH_COMPENS;
        gcmd->rf_path_tx = cmd->rf_tx_path_compens;
        gcmd->rf_path_rx = (uint8_t) cmd->rf_rx_path_compens;
        ...
        ble_msg_free(param);
        rsp = ble_msg_init(BLE_MGR_GAP_RF_PATH_COMPENSATION_SET_CMD, sizeof(*rsp));
        rsp->status = ret;
        ble_mgr_response_queue_send(&rsp, OS_QUEUE_FOREVER);
}
#endif /* F_PCLE */
