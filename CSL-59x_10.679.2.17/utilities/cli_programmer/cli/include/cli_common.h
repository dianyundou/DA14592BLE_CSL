...
/**
 * \brief Pretty-print contents of OTP 32bit cell size data buffer
 *
 * This is just special case of dump_hex().
 *
 * \param [in] cell_offset OTP cell
 * \param [in] buf OTP data buffer
 * \param [in] len length of data buffer (words)
 *
 * \note Should be used only with 32-bit cell OTP
 *
 */
void dump_otp_32(uint32_t cell_offset, uint32_t *buf, size_t len);

/**
 * \brief Pretty-print contents of OTP 64bit cell size data buffer
 *
 * This is just special case of dump_hex().
 *
 * \param [in] cell_offset OTP cell
 * \param [in] buf OTP data buffer
 * \param [in] len length of data buffer (64 bit words)
 *
 * \note Should be used only with 64-bit cell OTP
 *
 */
...
/**
 * \brief Display the product information
 *
 * \param [in] buf buffer where the product information is stored in
 * \param [in] total_len the size of buffer in bytes
 *
 * \return 0 product info found - success, otherwise not found - failed
 *
 */
...
/**
 * \brief Get selected partition size
 *
 * \param [in] id partition id
 *
 * \return partition size on success, otherwise 0
 *
 */
