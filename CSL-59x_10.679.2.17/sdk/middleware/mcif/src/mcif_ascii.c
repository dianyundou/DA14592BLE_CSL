...
__RETAINED uint8_t framebuffer[MCIF_ASCII_MAX_LINE + 1];
__RETAINED int pos;
        ...
        struct mcif_message_s *msg, const char *simple_msg)
{
        ...
        tx_msg->len = len;
        if (mcif_queue_send(0, &tx_msg, 0) != OS_OK) {
                OS_FREE(tx_msg);
        }
}
...
__STATIC_INLINE void mcif_ascii_print_done(
        const struct mcif_ascii_parse_element_s *parse_table,
        xQueueHandle txq,
...
__STATIC_INLINE char * skip_whitespace(char *s)
{
...
{
        if (mcif_queue_send(0, &msg, 0) != OS_OK) {
                OS_FREE(msg);
        }
