...
typedef enum {
        ...
        HW_PDC_PERIPH_TRIG_ID_TIMER3_QDEC       = 0x2, /**< Timer3 OR Quadrature Decoder (QDec) */
        ...
        HW_PDC_PERIPH_TRIG_ID_COMBO             = 0xA, /**< CMAC2SYS_IRQ OR JTAG present OR Debounced IO */
        HW_PDC_PERIPH_TRIG_ID_FCU_IRQ           = 0xC, /**< FCU_IRQ */
        HW_PDC_PERIPH_TRIG_ID_FAST_WKUP         = 0xD, /**< Fast wakeup trigger */
 ...
 */
typedef enum {
        HW_PDC_LUT_ENTRY_EN_COM  = PDC_PDC_CTRL0_REG_EN_COM_Msk, /**< If set, enables PD_COM for GPIO access */
...
/**
 * \brief blocked PDC triggers
 */
typedef enum {
        HW_PDC_RESET_BLOCKED_TRIGGERS = 0,                                                      /**< None PDC trigger is blocked */
        HW_PDC_PD_SYS_BLOCKED = REG_MSK(PDC, PDC_CONFIG_REG, PD_SYS_WKUP_CONFIG),               /**< PD_SYS is woken up only when the FCU is ready or powered off */
        HW_PDC_PD_RAD_BLOCKED = REG_MSK(PDC, PDC_CONFIG_REG, PD_RAD_WKUP_CONFIG),               /**< PD_RAD is woken up only when the FCU is ready or powered off */
        HW_PDC_ALL_BLOCKED_EXCEPT_FCU = REG_MSK(PDC, PDC_CONFIG_REG, TRIG_SELECT_CONFIG),       /**< All triggers apart from FCU (0xC) are blocked until the FCU is ready or powered off */
} HW_PDC_BLOCKED_TRIG;
 ...
 *
 * This function returns the first matching entry after starting entry with the below criteria.
 * In case we are not interesting in one or more criteria we could use the value
 * HW_PDC_FILTER_DONT_CARE, thus this function will return the index of the entry which
 * matches to the remaining criteria.
 *
...
/**
 * \brief Set PDC configuration register
 *
 * \param [in] blocked          type of PDC configured entries
 */
__STATIC_FORCEINLINE void hw_pdc_set_blocked_triggers(HW_PDC_BLOCKED_TRIG blocked)
{
        PDC->PDC_CONFIG_REG = blocked;
}

/**
 * \brief get PDC configuration register
 *
 * \return                      type of blocked PDC configured entries.
 */
__STATIC_FORCEINLINE HW_PDC_BLOCKED_TRIG hw_pdc_get_blocked_triggers()
{
        return PDC->PDC_CONFIG_REG;
}
