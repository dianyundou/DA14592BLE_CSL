...
{
        ble_mgr_gattc_browse_range_cmd_t *cmd;
        ble_mgr_gattc_browse_range_rsp_t *rsp;
        ble_error_t ret = BLE_ERROR_FAILED;

        /* Create new command and fill it */
        cmd = alloc_ble_msg(BLE_MGR_GATTC_BROWSE_RANGE_CMD, sizeof(*cmd));
        cmd->conn_idx = conn_idx;
        cmd->uuid = uuid;
        cmd->start_h = start_h;
        cmd->end_h = end_h;

        if (!ble_cmd_execute(cmd, (void **) &rsp, ble_mgr_gattc_browse_range_cmd_handler)) {
                return BLE_ERROR_BUSY;
        }
