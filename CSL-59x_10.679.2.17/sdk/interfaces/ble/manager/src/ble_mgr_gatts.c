...
__RETAINED static ble_mgr_common_stack_msg_t *gattm_add_svc_msg;
__RETAINED static uint16_t attr_idx;
__RETAINED static uint16_t extended_prop;
...
void ble_mgr_gatts_service_characteristic_get_prop_cmd_handler(void *param)
{
        ...
        /* API takes characteristic value handle to read permissions of the attribute.*/
...
void ble_mgr_gatts_send_event_cmd_handler(void *param)
{
        ...
        if (!dev) {
                ...
                /* Check if already sending events on this handle */
                if (pending_events_has_handle(dev, cmd->handle)) {
                        ret = BLE_ERROR_BUSY;
                        storage_release();
                        goto done;
                } else {
                        pending_events_put_handle(dev, cmd->handle);
                }
        }
...
void ble_mgr_gatts_event_sent_evt_handler(ble_gtl_msg_t *gtl)
{
        ...
        if (dev) {
                pending_events_remove_handle(dev, evt->handle);
        }
        storage_release();
