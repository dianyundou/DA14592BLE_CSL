...
enum hl_err
{
    ...
    // No more credit (symbol compatible with RWBLE_SW_VERSION_MAJOR < 8 code)
    L2C_ERR_NO_MORE_CREDIT            = 0x3B,
    ...
    L2C_ERR_INVALID_SCID = 0x3D,
    //  Connection refused - Source CID already allocated
    L2C_ERR_SCID_ALLOCATED = 0x3E,
    //  Connection refused - unacceptable parameters
    L2C_ERR_UNACCEPTABLE_PARAM = 0x3F,
#endif /* (RWBLE_SW_VERSION >= VERSION_8_1) */
#else
    ///Invalid Channel ID
    L2C_ERR_INVALID_CID               = 0x32,
    ///Invalid PDU
    L2C_ERR_INVALID_PDU               = 0x33,
    /// Connection refused - no resources available
    L2C_ERR_NO_RES_AVAIL              = 0x34,
    /// Connection refused - insufficient authentication
    L2C_ERR_INSUFF_AUTHEN             = 0x35,
    /// Connection refused - insufficient authorization
    L2C_ERR_INSUFF_AUTHOR             = 0x36,
    /// Connection refused - insufficient encryption key size
    L2C_ERR_INSUFF_ENC_KEY_SIZE       = 0x37,
    /// Connection Refused - insufficient encryption
    L2C_ERR_INSUFF_ENC                = 0x38,
    /// Connection refused - LE_PSM not supported
    L2C_ERR_LEPSM_NOT_SUPP            = 0x39,
    /// No more credit
    L2C_ERR_NO_MORE_CREDIT            = 0x3A,
    /// Command not understood by peer device
    L2C_ERR_NOT_UNDERSTOOD            = 0x3B,
    ...
    ///The request sent by the HL cannot be handled for some reasons (unauthorized source
    ///task, role, ...)
    ...
    ///The command cannot be executed because a SMP timeout has been raised during the
    ///connection.
    ...
    ///The encryption procedure failed because the slave device didn't find the LTK needed
    ///to start an encryption session.
