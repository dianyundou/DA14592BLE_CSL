...
static void handle_event_sent_evt(ble_service_t *svc, const ble_evt_gatts_event_sent_t *evt)
{
        ...
        if (evt->handle == bls->blood_measurement_h && bls->cb->meas_indication_sent) {
                bls->cb->meas_indication_sent(svc, evt->conn_idx, evt->status);
        } else if (evt->handle == bls->interm_cuff_pressure_h &&
                                                        bls->cb->interm_cuff_pressure_notif_sent) {
                bls->cb->interm_cuff_pressure_notif_sent(svc, evt->conn_idx, evt->status);
        }
}
...
static uint8_t pack_send_value(const bls_measurement_t *measurement, uint8_t *value)
{
        uint8_t *ptr = &value[1];
        uint8_t *flags = value;
        ...
        *flags = 0;
        ...
        if (measurement->measurement_status_present) {
                *flags |= BLS_FLAG_MEASUREMENT_STATUS;
                put_u16_inc(&ptr, measurement->measurement_status);
        }
                                                              ...
                                                              const bls_measurement_t *measurement)
{
        ...
        value_len = pack_send_value(measurement, value);
