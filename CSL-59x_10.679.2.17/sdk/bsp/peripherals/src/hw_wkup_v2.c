...
void hw_wkup_configure_hibernation(HW_WKUP_HIBERN_PIN pin, HW_WKUP_HIBERN_POLARITY pol)
{
        ...
        uint32_t tmp = CRG_TOP->HIBERN_CTRL_REG;
        REG_SET_FIELD(CRG_TOP, HIBERN_CTRL_REG, HIBERN_WKUP_MASK, tmp, pin);
        REG_SET_FIELD(CRG_TOP, HIBERN_CTRL_REG, HIBERN_WKUP_POLARITY, tmp, pol);
        CRG_TOP->HIBERN_CTRL_REG = tmp;
