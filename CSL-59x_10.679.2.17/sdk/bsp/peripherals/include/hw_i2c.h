...
/**
 * \brief Sets a field value of an I2C register. Aimed to be used with local variables.
 *
 * \param [in] reg the I2C register
 * \param [in] field the I2C register field
 ...
 * \note Even with \p cfg set to NULL, I2C clock (SCL) will be configured using default values.
 *
 * \note For I2C MASTER mode it is recommended to disable the interface before calling this function.
 *
 * \param [in] id I2C controller instance
 * \param [in] cfg configuration
 *
...
__STATIC_INLINE void hw_i2c_set_mode(HW_I2C_ID id, HW_I2C_MODE mode)
{
        ...
        bool set_master = (mode != HW_I2C_MODE_SLAVE) ? 1 : 0;
#if (HW_I2C_SLAVE_SUPPORT == 0)
        ASSERT_WARNING(set_master);
#endif
        uint32_t tmp = IBA(id)->I2C_CON_REG;
        HW_I2C_REG_SET_FIELD(I2C_CON, I2C_MASTER_MODE, tmp, set_master ? 1 : 0);
        HW_I2C_REG_SET_FIELD(I2C_CON, I2C_SLAVE_DISABLE, tmp, set_master ? 1 : 0);
...
{
        uint32_t tmp = IBA(id)->I2C_TAR_REG;
        HW_I2C_REG_SET_FIELD(I2C_TAR, SPECIAL, tmp, !!enabled);
        HW_I2C_REG_SET_FIELD(I2C_TAR, GC_OR_START, tmp, !enabled);
        IBA(id)->I2C_TAR_REG = tmp;
}
 ...
 *
 * \note It is recommended to disable the interface before calling this function.
 *
 * \param [in] id I2C controller instance
 * \param [in] addr_mode mode of addressing
 ...
 * \endparblock
 *
 * \return number of requested bytes in case of success, -1 otherwise.
 *
 * \note This function should be used only when operating in master mode.
 *
 * \warning This function registers an internal interrupt handler, which overrides any previously
