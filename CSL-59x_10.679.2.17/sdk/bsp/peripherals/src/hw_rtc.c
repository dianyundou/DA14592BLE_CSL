...
static __RETAINED void (*correction_callback)(const hw_rtc_time_t *time);

void hw_rtc_register_correction_cb(void (*cb)(const hw_rtc_time_t *time))
{
        correction_callback = cb;
}
#endif
...
HW_RTC_SET_REG_STATUS hw_rtc_set_time_clndr(const hw_rtc_time_t *time, const hw_rtc_calendar_t *clndr)
{
        ...
        if ((time != NULL) && (clndr != NULL)) {
                ...
                hw_rtc_set_hour_clk_mode(time->hour_mode);
                ...
                if ((status & (HW_RTC_VALID_TIME | HW_RTC_VALID_CLNDR)) == 0x0) {
                        ...
                        hw_rtc_set_hour_clk_mode(0x1 & (time_cur_val >> 30));
                ...
                if (correction_callback) {
                        ...
                        ((ret == HW_RTC_INVALID_TIME_CLNDR) || (ret == HW_RTC_INVALID_TIME)) ? correction_callback(&time_cur) : correction_callback(time);
                }
#endif
                hw_rtc_start();
                ...
                if (correction_callback) {
                        hw_rtc_time_t time_cur;
                        bdc_to_time(time_cur_val, &time_cur);
                        (ret == HW_RTC_INVALID_TIME) ? correction_callback(&time_cur) : correction_callback(time);
                }
#endif
                hw_rtc_set_hour_clk_mode(time->hour_mode);
                hw_rtc_time_start();
...
HW_RTC_SET_REG_STATUS hw_rtc_set_alarm(const hw_rtc_time_t *time, const hw_rtc_alarm_calendar_t *clndr, const uint8_t mask)
{
        ...
        if (time && time->hour_mode != hw_rtc_get_hour_clk_mode()) {
                // Do not allow alarm with different hour clock mode from time
                return HW_RTC_INVALID_TIME_HOUR_MODE_ALM;
        }
        ...
        if (time != NULL) {
                ...
                hw_rtc_set_alarm_time_bcd(time_to_bcd(time));
                status = hw_rtc_get_status();
