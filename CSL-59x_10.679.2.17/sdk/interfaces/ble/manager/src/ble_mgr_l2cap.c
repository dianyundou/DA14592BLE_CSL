...
/* Local COC list */
__RETAINED queue_t l2cap_chan;

/* Mask of allocated channels, by source CID */
__RETAINED uint64_t scid_mask;

/** \brief Allocate free Source CID */
__STATIC_INLINE uint16_t alloc_scid(uint16_t conn_idx)
{
        int i;
...
__STATIC_INLINE void dealloc_scid(uint16_t scid)
{
        scid_mask &= ~(1ULL << (scid - SCID_BASE));
}
...
__STATIC_INLINE l2cap_chan_t *find_chan_by_psm(uint16_t conn_idx, uint16_t psm)
{
...
__STATIC_INLINE l2cap_chan_t *find_chan_by_scid(uint16_t conn_idx, uint16_t scid)
{
        struct chan_match_data md = {
                .conn_idx = conn_idx,
                .value  = scid,
...
__STATIC_INLINE l2cap_chan_t *find_chan_by_dcid(uint16_t conn_idx, uint16_t scid)
{
        struct chan_match_data md = {
                .conn_idx = conn_idx,
                .value  = scid,
...
__STATIC_INLINE l2cap_chan_t *find_chan_by_connecting(uint16_t conn_idx)
{
        struct chan_match_data md = {
                .conn_idx = conn_idx,
        };
...
__STATIC_INLINE l2cap_chan_t *add_chan(uint16_t conn_idx)
{
        l2cap_chan_t *chan;
        uint16_t scid;
...
__STATIC_INLINE void remove_chan(l2cap_chan_t *chan)
{
        OS_ASSERT(chan);
