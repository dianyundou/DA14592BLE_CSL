 ...
 * \sa ble_gattc_browse_range()
 *
 */
ble_error_t ble_gattc_browse(uint16_t conn_idx, const att_uuid_t *uuid);

/**
 * \brief Browse services on remote GATT server in a given range
 *
 ...
 * ::BLE_EVT_GATTC_BROWSE_COMPLETED will be sent.
 *
 * If \p uuid is NULL, all services in range are returned.
 *
 * \param [in] conn_idx  connection index
 * \param [in] start_h   beginning of browse range
 * \param [in] end_h     end of browse range
 * \param [in] uuid      optional service UUID
 *
 * \return result code
 *
 ...
 *
 */
ble_error_t ble_gattc_browse_range(uint16_t conn_idx, uint16_t start_h, uint16_t end_h,
