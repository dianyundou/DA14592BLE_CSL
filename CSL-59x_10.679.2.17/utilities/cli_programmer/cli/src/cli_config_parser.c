...
bool PathCombine(char *dst, const char *path, const char *more)
{
        ...
        if (len + strlen(more) >= MAX_CLI_CONFIG_FILE_PATHNAME_LEN) {
                return false;
        }
        strcpy(dst, path);
        if (len > 0 && dst[len - 1] != '/') {
...
bool PathCanonicalize(char *resolved, const char *path) {
        bool ret;
        ...
        /* there is no directory, only file in path */
        if (strncmp(dname, ".", 1) == 0) {
                ret = true;
                goto done;
        }

        /* dir maps to root folder - not allowed */
        if (strcmp(dname, "/") == 0) {
                ret = false;
                goto done;
        }
        ...
        /* canonicalize dir */
        if (!realpath(dname, resolved)) {
                ret = false;
                goto done;
        }

        /* copy dir to resolved */
        strcpy(resolved, dname);
        strcat(resolved, "/");
        ret = true;
done:
        /* append file */
        strcat(resolved, bname);
        free(basec);
        free(dirc);
        return ret;
}
...
bool cli_config_canonicalize_file_name(char *out_name, const char *in_name)
{
        ...
        return PathCanonicalize(out_name, in_name);
}
...
char *get_default_config_file_path(char *path_buf, const char *argv0)
{
        ...
        if (stat(DEFAULT_CLI_CONFIG_FILE_NAME, &st) == 0) {
                if (!(PathCombine(path_buf, "", DEFAULT_CLI_CONFIG_FILE_NAME))) {
                        return NULL;
                }
                return path_buf;
        }
        ...
        if ((dir = getenv(HOMEPATHENV)) != NULL) {
                if (!(PathCombine(path_buf, dir, DEFAULT_CLI_CONFIG_FILE_NAME))) {
                        return NULL;
                }
        ...
        if (!PathCanonicalize(path_buf, argv0)) {
                return NULL;
        }
