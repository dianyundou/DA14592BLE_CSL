 ...
 */
typedef enum {
        RTC_24H_CLK = 0,        /**< 24 hour clock format */
...
{
        ASSERT_WARNING(mode == RTC_24H_CLK || mode == RTC_12H_CLK);
...
{
        uint32_t tmp = CRG_TOP->CLK_RTCDIV_REG;

        REG_SET_FIELD(CRG_TOP, CLK_RTCDIV_REG, RTC_DIV_DENOM, tmp, div_denom);
        REG_SET_FIELD(CRG_TOP, CLK_RTCDIV_REG, RTC_DIV_INT, tmp, div_int);
        REG_SET_FIELD(CRG_TOP, CLK_RTCDIV_REG, RTC_DIV_FRAC, tmp, div_frac);

        CRG_TOP->CLK_RTCDIV_REG = tmp;
}
