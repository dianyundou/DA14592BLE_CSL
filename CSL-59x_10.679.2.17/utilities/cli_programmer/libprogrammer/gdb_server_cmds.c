...
/* GDB Server socket */
#ifdef _WIN32
static SOCKET gdb_server_sock = INVALID_SOCKET;
#else
static int gdb_server_sock = INVALID_SOCKET;
#endif
...
#else
/* GDB Server PID */
static pid_t gdb_server_pid = PID_INVALID;
#endif
...
static bool check_gdb_server_port(int pid, int port)
{
        ...
        int cnt = 0;
        int netstat_port = PORT_INVALID;
        int netstat_pid = PID_INVALID;

        fp = _popen("netstat -nao -p TCP", "r");
...
static int gdb_server_init_socket(uint16_t port, const char *host_name)
{
        ...
        if ((gdb_server_sock = socket(PF_INET, SOCK_STREAM, 0)) == INVALID_SOCKET) {
                return ERR_GDB_SERVER_SOCKET;
        }
...
/* check that received frame contains '$', '#' characters, correct checksum  and isn't error code */
static bool gdb_server_check_frame(void)
{
        ...
        for (i = 0; i < gdb_server_recv_buf.len; i++) {
                ...
                if (gdb_server_recv_buf.buf[dollar_pos + 1] == 'E' && dollar_pos + 3 == hash_pos) {
                        return false;
                }
...
/* Load application binary via GDB Server */
static int gdb_server_loadbin(uint8_t *bin, size_t bin_size, uint32_t addr)
{
        return gdb_server_cmd_write(bin, bin_size, addr);
}
...
static int gdb_server_set_run_swd(void)
{
        ...
        return gdb_server_send_write_cmd(swd_addr.run_swd_addr, sizeof(arr), arr);
}
...
static int gdb_server_inc_cmd_num(void)
{
        ...
        return gdb_server_send_write_cmd(swd_addr.cmd_num_addr, sizeof(cmd_num_cp),
                                                                        (uint8_t *) &cmd_num_cp);
}

/* set NAK before command execution - uartboot should update this value */
static int gdb_server_set_nak(void)
{
        uint32_t nak = NAK;

        return gdb_server_send_write_cmd(swd_addr.ack_nak_addr, sizeof(NAK), (uint8_t *) &nak);
}
...
static int gdb_server_reset_device()
{
        ...
        if (target_reset_cmd && is_localhost(configuration.host_name)) {
                ...
                if (CreateProcessA(NULL, target_reset_cmd, NULL, NULL, FALSE, 0, NULL, NULL, &si,
                                                                                        &pi)) {
                        DWORD ret_code;
               ...
               if (status  != 0) {
                ...
                tmp_32 = 0x00000001;
                if ((status = gdb_server_send_write_cmd(regs->swd_reset_reg, sizeof(tmp_32),
                                                                                (uint8_t *)&tmp_32)) != 0) {
                        return status;
                }
        ...
        return gdb_server_send_write_cmd(regs->sys_ctrl_reg, sizeof(regs->sys_ctrl_reg_val),
                                                        (uint8_t *) &regs->sys_ctrl_reg_val);
}
...
static int gdb_server_upload_executable(uint8_t *executable_code, size_t executable_code_size)
{
        int status;

        prog_print_log("Uploading boot loader/application executable...\n");

        /* reboot device before uploading image */
        if ((status = gdb_server_reset_device()) != 0) {
                return status;
        }

        /* write boot loader code from array to platform */
        if ((status = gdb_server_loadbin(executable_code, executable_code_size, 0)) != 0) {
                return status;
        }
...
static int gdb_server_send_swd_cmd_header_without_continue(uint32_t data_len, const uint8_t *data)
{
        int status;
...
static int gdb_server_read_dynamic_length(uint8_t **buf, uint32_t *len)
{
        ...
        *len = size;
        ...
        /* receive payload */
        if ((ret = gdb_server_send_read_cmd(swd_addr.buf_addr, *len)) != 0) {
                goto done;
        }
        ...
        if (!gdb_server_frame_to_uint8_buf(*buf, *len)) {
                free(*buf);
                ret = ERR_FAILED;
                goto done;
        }
...
int gdb_server_initialization(const prog_gdb_server_config_t *gdb_server_conf)
{
        ...
        if (gdb_server_sock != INVALID_SOCKET) {
                closesocket(gdb_server_sock);
                gdb_server_sock = INVALID_SOCKET;
        }
        ...
        if (!configuration.host_name) {
                return ERR_GDB_SERVER_OUT_OF_MEMORY;
        }
...
int gdb_server_cmd_write(const uint8_t *buf, size_t size, uint32_t addr)
{
        ...
        prog_get_chip_rev(&chip_rev);
        status = prog_get_chip_regs(chip_rev, &regs);
        virtual_buf_mask = regs->virtual_buf_mask;
         ...
         */
        if (addr == ADDRESS_TMP) {
                addr = swd_addr.buf_addr;
        } else if ((addr & virtual_buf_mask) == VIRTUAL_BUF_ADDRESS) {
                addr = (addr & ~virtual_buf_mask) + swd_addr.buf_addr;
        }
...
int gdb_server_cmd_direct_write_to_qspi(const uint8_t *buf, size_t size, uint32_t addr, bool verify)
{
        ...
        header_buf[0] = SOH;
        header_buf[1] = CMD_DIRECT_WRITE_TO_QSPI;

        header_buf[2] = (uint8_t) 5 + size;
        header_buf[3] = (uint8_t) ((5 + size) >> 8);

        header_buf[4] = (uint8_t) (verify);

        header_buf[5] = (uint8_t) (addr);
        header_buf[6] = (uint8_t) (addr >> 8);
        header_buf[7] = (uint8_t) (addr >> 16);
        header_buf[8] = (uint8_t) (addr >> 24);

        /* send data */
        if ((status = gdb_server_send_write_cmd(swd_addr.buf_addr, size, buf)) != 0) {
                return status;
        }
...
int gdb_server_cmd_run(uint32_t address)
{
        ...
        if ((status = gdb_server_send_swd_cmd_header_without_continue(sizeof(header_buf),
                                                                                header_buf)) != 0) {
                return status;
        }

        /*
         * Send step and continue commands without acknowledge - there could be no uartboot
         * anymore. New application code could not reach any breakpoint.
         */
        if ((status = gdb_server_send_recv_ack(GDB_STEP, strlen(GDB_STEP))) != 0) {
                return status;
        }

        return gdb_server_send(GDB_CONTINUE, strlen(GDB_CONTINUE));
}
...
int gdb_server_cmd_write_otp(uint32_t address, const uint32_t *buf, uint32_t len)
{
        ...
        if ((!strncmp(chip_rev, CHIP_REV_690AB, CHIP_REV_STRLEN)) || (!strncmp(chip_rev, CHIP_REV_590XX, CHIP_REV_STRLEN))) {
                cell_size_32bit = true;
        }
        ...
        for (part_size = 0; part_size < size;) {
                ...
                part_size += write_size;
                /* It is a cell address - should be converted to byte address */
                if (cell_size_32bit) {
                        /* It is a cell address contains 32bit information should be divided by 4 bytes */
                        address += (write_size >> 2);
                }
                else {
                        /* It is a cell address contains 64bit information - should be divided by 8 bytes */
...
{
        return gdb_server_upload_executable(executable_code, executable_code_size);
}
...
int gdb_server_cmd_upload_bootloader(void)
{
        ...
        if ((status = gdb_server_upload_executable(boot_loader_code, boot_loader_size)) != 0) {
                return status;
        }
...
int gdb_server_cmd_chip_erase_eflash(void)
{
        uint8_t header_buf[2];

        /* create header */
        header_buf[0] = SOH;
        header_buf[1] = CMD_CHIP_ERASE_EFLASH;

        /* send header */
        return gdb_server_send_swd_cmd_header(sizeof(header_buf), header_buf);
}

int gdb_server_cmd_read_eflash(uint32_t address, uint8_t *buf, uint32_t len)
{
        uint8_t header_buf[10];
        uint32_t size = len * sizeof(*buf);
        int status = 0;
...
int gdb_server_cmd_direct_write_to_eflash(const uint8_t *buf, size_t size, uint32_t addr, bool verify)
{
        uint8_t header_buf[9];
        int status = 0;
        ...
        /* create constant part of header */
        header_buf[0] = SOH;
        header_buf[1] = CMD_DIRECT_WRITE_TO_EFLASH;
...
int gdb_server_cmd_copy_to_eflash(uint32_t src_address, size_t size, uint32_t dst_address)
{
        uint8_t header_buf[14];

        /* create header */
        header_buf[0] = SOH;
        header_buf[1] = CMD_COPY_EFLASH;
        header_buf[2] = (uint8_t) 10;
        header_buf[3] = (uint8_t) (10 >> 8);
...
int gdb_server_cmd_erase_eflash(uint32_t address, size_t size)
{
        uint8_t header_buf[12];

        /* create header */
        header_buf[0] = SOH;
        header_buf[1] = CMD_ERASE_EFLASH;
        header_buf[2] = (uint8_t) 8;
        header_buf[3] = (uint8_t) (8 >> 8);
...
int gdb_server_cmd_is_empty_eflash(unsigned int size, unsigned int start_address, int *ret_number)
{
        uint8_t header_buf[12];
        int status = 0;
        ...
        /* create header */
        header_buf[0] = SOH;
        header_buf[1] = CMD_IS_EMPTY_EFLASH;
        header_buf[2] = (uint8_t) 8;
        header_buf[3] = (uint8_t) (8 >> 8);
        header_buf[4] = (uint8_t) (size);
        header_buf[5] = (uint8_t) (size >> 8);
...
int gdb_server_cmd_get_product_info(uint8_t **buf, uint32_t *len)
{
        ...
        /* create header */
        header_buf[0] = SOH;
        header_buf[1] = CMD_GET_PRODUCT_INFO;
...
int gdb_server_cmd_get_qspi_state(uint8_t id, flash_dev_info_t *qspi_flash_info)
{
        ...
        /* create header */
        header_buf[0] = SOH;
        header_buf[1] = CMD_GET_QSPI_STATE;
        header_buf[2] = (uint8_t) 1;
        header_buf[3] = (uint8_t) (1 >> 8);
        header_buf[4] = id;
        ...
        /* receive data */
        if ((status = gdb_server_send_read_cmd(swd_addr.buf_addr, sizeof(flash_dev_info_t))) != 0) {
                return status;
        }

        return (gdb_server_frame_to_uint8_buf((uint8_t *)qspi_flash_info, sizeof(flash_dev_info_t)) ? 0 :
                                                                ERR_GDB_SERVER_INVALID_RESPONSE);
}
...
static prog_gdb_server_info_t parse_ps_line(char *line)
{
        ...
        for (; token; token = strtok(NULL, " ")) {
                ...
                if (strstr(arg, "usb")) {
                        char *tmp = strchr(arg, '=');
                        instance.sn = (tmp == NULL) ? SERIAL_NUMBER_NA : strtoll(tmp + 1, NULL, 10);
        ...
        instance.port = (instance.port > PORT_INVALID) ? instance.port : GDB_SERVER_DEFAULT_PORT;
...
prog_gdb_server_info_t *gdb_server_get_instances(const char *gdb_server_cmd)
{
        ...
        while (fgets(line, sizeof(line), fp)) {
                ...
                if (gdb_server_instances[index].pid > PID_INVALID) {
                        ++index;
                }
        ...
        gdb_server_instances[index].pid = PID_INVALID;
        ...
        if ((snap_shot !=  INVALID_HANDLE_VALUE) && Process32First(snap_shot, &proc_entry)) {
                do {
                        if (_stricmp(proc_name, proc_entry.szExeFile) == 0) {
                                ...
                                instances[index].port = PORT_INVALID;
                                ++index;
...
#ifdef _WIN64
                src_handle = (HANDLE) (ULONG64) handles_info->Handle[i].HandleValue;
#else
                src_handle = (HANDLE) (UINT) handles_info->Handle[i].HandleValue;
#endif
                if (!DuplicateHandle(proc_handle, src_handle,
...
prog_gdb_server_info_t *gdb_server_get_instances(const char *gdb_server_cmd)
{
        ...
        gdb_server_instances[proc_num].pid = PID_INVALID;
