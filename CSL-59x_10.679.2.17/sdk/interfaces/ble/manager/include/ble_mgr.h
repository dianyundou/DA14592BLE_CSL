 ...
 * \{
 * \addtogroup BLE_MANAGER_API API
 *
...
typedef struct ble_dev_params_tag {
        ...
        uint8_t            tx_phy_pref_default;                         /**< GAP default TX PHY preference */
        uint8_t            rx_phy_pref_default;                         /**< GAP default RX PHY preference */
        bool               phy_set_pending;                             /**< GAP PHY set operation pending */
        bool               phy_change_req;                              /**< GAP PHY change requested */
#endif /* (dg_configBLE_2MBIT_PHY == 1) */
        int8_t             conn_rssi;                                   /**< The RSSI reading reported by GAPC_CON_RSSI_IND */
} ble_dev_params_t;
...
#else
        void *evt_q;       /**< BLE Manager event queue */
#endif
...
/**
 * \brief Flush BLE Manager's event queue
 *
 ...
 *
 * \return BLE_IS_DISABLED if BLE has not been enabled yet,
 *         BLE_IS_ENABLED if BLE is up and running,
 *         BLE_IS_BUSY if BLE is currently busy and cannot accept requests, or
 *         BLE_IS_RESET if a BLE reset is currently in progress.
 * \sa ble_status_t
 */
ble_status_t ble_mgr_get_status(void);

/**
 * \brief Set BLE status
 *
