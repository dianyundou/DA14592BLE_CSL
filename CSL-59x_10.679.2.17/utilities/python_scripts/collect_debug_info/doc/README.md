...
This section starts from
__NVIC_REGISTERS_START__ marker until
__NVIC_REGISTERS_STOP__.

It contains the content of certain registers of (or related to) the Nested Vectored Interrupt 
Controller of SYSCPU (Cortex M33 processor). These are the following:
...
This provides an overview of which interrupts are enabled, what priorities have been set for them, 
and whether some of them are active or pending.
...
The register names are read from the corresponding xml file, for example DA1459x-00.xml.
...
This section starts from
__CMAC_CACHE_HEX_DUMP_START__ marker until
