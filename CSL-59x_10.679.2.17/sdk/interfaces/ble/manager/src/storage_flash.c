...
#error "dg_configNVMS_ADAPTER must be defined for BLE Flash Storage!"
#endif

#if !dg_configNVMS_VES
#error "dg_configNVMS_VES must be defined for BLE Flash Storage!"
#endif
...
__RETAINED static nvms_t part;     // partition handle

/* Calculates partition offset for device at index */
__STATIC_INLINE uint32_t get_addr(uint32_t index)
{
...
__STATIC_INLINE uint32_t nvms_read_inc(uint32_t addr, void *ptr, size_t length)
{
...
__STATIC_INLINE uint32_t nvms_write_inc(uint32_t addr, const void *ptr, size_t length)
{
