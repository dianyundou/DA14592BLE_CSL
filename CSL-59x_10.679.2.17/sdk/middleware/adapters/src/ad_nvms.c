...
#define PARTITION_TABLE_ENTRY(start, size, id) \
        {((start) / AD_FLASH_GET_SECTOR_SIZE(start)) * (AD_FLASH_GET_SECTOR_SIZE(start)), \
        ((size) / AD_FLASH_GET_SECTOR_SIZE(start)) * (AD_FLASH_GET_SECTOR_SIZE(start)), (id), 0},
#define PARTITION(start, id, flags) \
        {((start) / AD_FLASH_GET_SECTOR_SIZE(start)) * (AD_FLASH_GET_SECTOR_SIZE(start)), \
        0, (id), (flags)},
#define PARTITION2(id, flags) \
        {((id##_START) / AD_FLASH_GET_SECTOR_SIZE(id##_START)) * (AD_FLASH_GET_SECTOR_SIZE(id##_START)), \
        ((id##_SIZE) / AD_FLASH_GET_SECTOR_SIZE(id##_START)) * (AD_FLASH_GET_SECTOR_SIZE(id##_START)), \
        (id), (flags)},
...
static __RETAINED partition_t *partitions;
...
static void add_partition(uint8_t type, uint32_t start_addr, uint32_t size, uint8_t flags)
{
        partition_entry_t new_entry = { PARTITION_ENTRY_MAGIC, type, 0xFF, flags, start_addr,
                                                                                        size };
        ...
        for (;;) {
                ...
                if (entry.type == type && entry.magic == PARTITION_ENTRY_MAGIC &&
                        ...
                        if (entry.size == size && entry.start_address == start_addr) {
                                return;
                        }
                }
...
static void init_default_partitions(void)
{
        ...
        for (i = 0; i < sizeof(part_init) / sizeof(part_init[0]); ++i) {
                ...
                if (size == 0 && (i + 1 < sizeof(part_init) / sizeof(part_init[0]))) {
                        size = (int) part_init[i + 1].start_address - (int) part_init[i].start_address;
                }
                 ...
                 */
                if (size > 0) {
                        add_partition(part_init[i].id, part_init[i].start_address, size, part_init[i].flags);
                }
        }
}
#endif
...
{
        for (partition_t *part = partitions; part != NULL; part = part->next) {
...
size_t ad_nvms_erase_size(nvms_t handle)
{
        ...
        return ad_flash_erase_size(part->data.start_address);
}
...
bool ad_nvms_no_cache_flushing(nvms_t handle, uint32_t base, uint32_t size)
{
        ...
        addr = part->data.start_address + base;
