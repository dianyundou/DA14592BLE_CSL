...
static void handle_read_req(ble_service_t *svc, const ble_evt_gatts_read_req_t *evt)
{
        ...
        if (handle == hids->protocol_mode_val_h) {
                ble_gatts_read_cfm(evt->conn_idx, evt->handle, ATT_ERROR_OK,
                                                sizeof(hids->protocol_mode), &hids->protocol_mode);
        } else if (handle == hids->boot_keyboard_input_ccc_h ||
                                                        handle == hids->boot_mouse_input_ccc_h ||
...
static void add_ccc(uint16_t *ccc_h)
{
        ...
        ble_gatts_add_descriptor(&uuid, ATT_PERM_RW, 2, 0, ccc_h);
}
                                                                ...
                                                                const hids_callbacks_t *callbacks)
{
        ...
        if (config->num_reports > 0) {
                ...
                for (uint8_t i = 0; i < hids->num_reports; i++) {
...
bool hids_notify_boot_mouse_input_report(ble_service_t *svc, uint16_t length, const uint8_t *data, ble_error_t *ble_err)
{
        ...
        *ble_err = ble_gatts_send_event(hids->conn_idx, hids->boot_mouse_input_val_h,
                                                        GATT_EVENT_NOTIFICATION, length, data);

        return *ble_err == BLE_STATUS_OK;
}
...
bool hids_notify_boot_keyboard_input_report(ble_service_t *svc, uint16_t length,
                                            const uint8_t *data, ble_error_t *ble_err)
{
        hid_service_t *hids = (hid_service_t *) svc;
        uint16_t ccc = 0x0000;
        ...
        *ble_err = ble_gatts_send_event(hids->conn_idx, hids->boot_keyboard_input_val_h,
                                                        GATT_EVENT_NOTIFICATION, length, data);
                              ...
                              const uint8_t *data, ble_error_t *ble_err)
{
        ...
        *ble_err = ble_gatts_send_event(hids->conn_idx, report->val_h, GATT_EVENT_NOTIFICATION,
                                                                                length, data);
