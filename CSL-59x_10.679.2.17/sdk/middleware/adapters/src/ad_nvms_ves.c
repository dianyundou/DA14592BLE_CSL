...
#define AD_NVMS_VES_SECTOR_SIZE(addr)                   get_sector_size(addr)
...
typedef struct {
        ...
        uint32_t start_address;         /**< Partition start sector */
...
__STATIC_INLINE uint32_t get_sector_size(uint32_t addr)
{
        bool is_valid_virtual_addr;

        is_valid_virtual_addr = eflash_automode_is_valid_virtual_addr(addr);
#if (dg_configQSPI_AUTOMODE_ENABLE == 1)
        is_valid_virtual_addr |= qspi_automode_is_valid_virtual_addr(addr);
#endif
        OS_ASSERT(is_valid_virtual_addr);

        return AD_FLASH_GET_SECTOR_SIZE(addr);
}

__STATIC_INLINE uint32_t container_addr(ves_driver_data_t *ves, sec_ix_t sector, con_ix_t container)
{
        return ves->start_address + (sector * AD_NVMS_VES_SECTOR_SIZE(ves->start_address))
                + container * ves->container_size;
}

__STATIC_INLINE uint32_t container_data_addr(ves_driver_data_t *ves, sec_ix_t sector,
                                                                con_ix_t container, uint8_t offset)
{
        return ves->start_address + (sector * AD_NVMS_VES_SECTOR_SIZE(ves->start_address)) + offset
                + container * ves->container_size + offsetof(container_t, data);
}

__STATIC_INLINE bool current_sector_full(ves_driver_data_t *ves)
{
...
#ifdef CONFIG_NVMS_USE_CRC
/* Update container crc filed on flash */
static void ves_write_crc(ves_driver_data_t *ves, sec_ix_t sector, con_ix_t container,
                                                                                uint16_t crc)
{
        uint32_t addr = container_addr(ves, sector, container);
        ad_flash_write(addr + sizeof(uint16_t), (const uint8_t *) &crc, 2);
}
#endif
...
static void ves_init_sector(ves_driver_data_t *ves, sec_ix_t sector, bool check_dirty)
{
        ...
        if (erase_needed) {
                /* Erase whole sector */
                ad_flash_erase_region(addr, AD_NVMS_VES_SECTOR_SIZE(addr));
        }
 ...
 * - recalculate crc for this data container
...
#ifdef CONFIG_NVMS_USE_CRC
        /* Store new container CRC of the data */
        ves_write_crc(ves, new_sector, new_container,
                                        crc16_calculate(&cont->data[0], ves->container_data_size));
#endif
...
static void ves_mark_unused_as_dirty(ves_driver_data_t *ves, sec_ix_t sector)
{
        ...
        for (i = 0; i < ves->containers_per_sector; ++i) {
                const container_t *cont = ad_flash_get_ptr(container_addr(ves, sector, i));
                if (cont->index == CONTAINER_UNUSED) {
...
static void ves_read_cat(ves_driver_data_t *ves)
{
        ...
        cat_ix_t cat_ix;
        const container_t *cont;
        int32_t first_free_sector = -1;
        int32_t current_sector = -1;
        ves->last_erased_sector = 0;
        ves->free_container = ves->containers_per_sector;

        for (i = 0; i < ves->sector_count; ++i) {
                con_cnt_t uninitialized_count = 0;
                con_ix_t old_container = 0;
                con_cnt_t unused_count = 0;
                con_cnt_t dirty_count = 0;
...
#ifdef CONFIG_NVMS_USE_CRC
        data = ad_flash_get_ptr(container_data_addr(ves, new_sector, new_container, 0));
        crc = crc16_calculate(data, ves->container_data_size);
        ves_write_crc(ves, new_sector, new_container, crc);
#endif
...
void ves_init(struct partition_t *part)
{
        ...
        ves->start_address = part->data.start_address;
        ves->sector_count = part->data.size / AD_NVMS_VES_SECTOR_SIZE(part->data.start_address);
        ...
        ves->containers_per_sector = AD_NVMS_VES_SECTOR_SIZE(part->data.start_address) / ves->container_size;
        ...
        ves->cat_size = (ves->container_data_size - 1
                + ves->sector_count * AD_NVMS_VES_SECTOR_SIZE(part->data.start_address))
        ...
        while (offset < size) {
                ...
                if (container == CAT_ENTRY_NONE) {
                        ...
                        read = ad_flash_read(container_data_addr(ves, sector, container, offset_in_container),
                                                                        buf + offset, chunk);
#ifdef CONFIG_NVMS_USE_CRC
                        cont = ad_flash_get_ptr(container_addr(ves, sector, container));
                        crc = crc16_calculate(cont->data, ves->container_data_size);

                        /*
                         * 0xFFFF is the initial CRC value of the CRC16-CCITT algorithm and cannot
                         * be checked.
                         */
                        if (crc != cont->crc16 && cont->crc16 != 0xFFFF) {
                                return -1;
                        }
#endif
                }
                                                                                ...
                                                                                uint32_t size)
{
        ...
        while (offset < size) {
                ...
                size_t chunk = ves->container_data_size - offset_in_container;
