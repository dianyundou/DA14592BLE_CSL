...
__STATIC_INLINE void hw_src_enable(HW_SRC_ID id)
{
         ...
         */
        SRCBA(id)->SRC1_CTRL_REG |= REG_MSK(SRC1, SRC1_CTRL_REG, SRC_IN_FLOWCLR) |
                                    REG_MSK(SRC1, SRC1_CTRL_REG, SRC_OUT_FLOWCLR) |
                                    REG_MSK(SRC1, SRC1_CTRL_REG, SRC_EN);

        while (!(SRCBA(id)->SRC1_CTRL_REG & (REG_MSK(SRC1, SRC1_CTRL_REG, SRC_IN_OK) |
                                             REG_MSK(SRC1, SRC1_CTRL_REG, SRC_OUT_OK))));
...
__STATIC_INLINE void hw_src_enable_fifo(HW_SRC_ID id, HW_SRC_DIRECTION direction)
{
        ...
        switch (direction) {
        case HW_SRC_IN:
                REG_CLR_FIELD(SRC1, SRC1_CTRL_REG, SRC_FIFO_DIRECTION, src1_ctrl_reg);
                break;
        case HW_SRC_OUT:
                src1_ctrl_reg |= REG_MSK(SRC1, SRC1_CTRL_REG, SRC_FIFO_DIRECTION);
                break;
        default:
                ASSERT_WARNING(0);
        }

        src1_ctrl_reg |= REG_MSK(SRC1, SRC1_CTRL_REG, SRC_FIFO_ENABLE);
