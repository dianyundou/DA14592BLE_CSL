 ...
 */
typedef enum {
        CRYPTO_ECDH_RET_OK = 0,       /**< No error. */
        CRYPTO_ECDH_RET_TO = 1,       /**< Operation timed out. */
        CRYPTO_ECDH_RET_EE = 2,       /**< ECC operation error. */
        CRYPTO_ECDH_RET_MP = 3,       /**< Missing peer public key. */
        CRYPTO_ECDH_RET_IP = 4,       /**< Invalid peer public key. */
        CRYPTO_ECDH_RET_ER = 5        /**< Other error. */
} CRYPTO_ECDH_RET;
...
/**
 * \brief Initialize context for use with crypto_ecdh_compute.
 *
 * \param [in,out] ctx     The ECDH context where the result will be stored. It must be properly initialized with INIT_ECDH_CTX()
 *                         and one of the \link CURVES supported curves \endlink.
 * \param [in]     curve_type Curve type to be used with the created ecdh context.
 *
 * \returns The function returns one of the return codes defined in ::CRYPTO_ECDH_RET.
 */
