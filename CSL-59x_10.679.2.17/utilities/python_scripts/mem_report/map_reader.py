...
def section_groups(lines):
    section_group = []
    double_line_section_name = False
    double_line_section_str = ''
    for l in lines:
        if re.match(r"^\.[a-zA-Z]", l):
            if section_group:
                yield section_group
            if re.match(r"^\S+$", l):
                double_line_section_name = True
                double_line_section_str = l
            else:
                section_group = [l]
        elif section_group:
            if double_line_section_name == True:
                double_line_section_name = False
                double_line_section_str = double_line_section_str[:-1]
                double_line_section_str = double_line_section_str + l.replace(' ', "", 10)
                section_group = [double_line_section_str]
            else:
...
def read_capacity_of_cs_prod_sections(map_file_name):
    lines = read_lines(map_file_name)
    end_points = []
    for l in lines:
        if re.match(r"^\.init_text.", l) or re.match(r"^\.cs_and_keys.", l):
            end_points.append(l)
    if len(end_points) != 2:
        print("Warning - Size of CS and product headers is not parsed")
        return 0
    raw_begin = end_points[1].split()
    raw_end = end_points[0].split()
    if len(raw_begin) != 6 or len(raw_end) != 6:
        print("Warning - Size of CS and product headers is not parsed")
        return 0
    return int(raw_end[5], 16) - int(raw_begin[5], 16)


def get_symb_with_same_addr(list):
    symbol_addr = []
    symbol_name = []
    symbol_size = []
    previous_addr =''
    ...
    for sections in list:
        for symbol_content in sections:
            if previous_addr == symbol_content[1] and previous_size != '0x0':
                symbol_name.append([previous_name])
                symbol_name.append([symbol_content[0]])
                symbol_addr.append([previous_addr])
                symbol_addr.append([symbol_content[1]])
                symbol_size.append([int(previous_size, 16)])
                symbol_size.append([int(symbol_content[2], 16)])
            previous_name = symbol_content[0]
            previous_addr = symbol_content[1]
            previous_size = symbol_content[2]

    return (symbol_addr, symbol_name, symbol_size)
    ...
    for section_group in section_groups(lines):
        try:
            ...
            if name is not None and address is not None and size is not None:
                if name != '.cs_and_keys' and name != '.prod_head' and name != '.prod_head_backup' and name != '.img_head':
                    add_section(section_data, name, address, size, remaining_lines)
        except IndexError:
            banner = '*'*160
            print(banner + '\nERROR! Failed to read map_file: {}\n'.format(map_file_name) + banner)
    return section_data
